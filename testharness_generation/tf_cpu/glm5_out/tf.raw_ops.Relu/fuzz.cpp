...
bool isReluSupportedType(tensorflow::DataType dtype) {
    switch (dtype) {
        case tensorflow::DT_FLOAT:
        case tensorflow::DT_DOUBLE:
        case tensorflow::DT_INT32:
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
    try {
        ...
        std::cout << "Input: dtype=" << dtype << ", rank=" << static_cast<int>(rank) << ", shape=[";
        ...
        if (!isReluSupportedType(dtype)) {
            std::cout << "Unsupported dtype for Relu: " << dtype << std::endl;
            return 0;
        }
        ...
        // Build Graph: Const -> Relu
        tensorflow::Scope root = tensorflow::Scope::NewRootScope();

        auto input_op = tensorflow::ops::Const(root.WithOpName("input"), tensor);
        ...
        // Run graph
        tensorflow::Status status = session.Run({relu_op}, &outputs);
        if (!status.ok()) {
        ...
        std::cout << "Relu operation complete" << std::endl;
