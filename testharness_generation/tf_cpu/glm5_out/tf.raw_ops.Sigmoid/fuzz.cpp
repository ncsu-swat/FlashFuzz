...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
    ...
    try {
        ...
        if (tensor_shape.num_elements() > 2000000) return 0;

        tensorflow::Tensor tensor(dtype, tensor_shape);

        fillTensorWithDataByType(tensor, dtype, Data, offset, Size);

        std::cout << "Input dtype: " << tensorflow::DataType_Name(dtype) << std::endl;
        std::cout << "Input shape: " << tensor_shape.DebugString() << std::endl;
        std::cout << "Input tensor: " << tensor.DebugString(10) << std::endl;

        // Build Graph using Scope + ClientSession pattern
        tensorflow::Scope root = tensorflow::Scope::NewRootScope();

        auto input = tensorflow::ops::Const(root, tensor);
        auto sigmoid = tensorflow::ops::Sigmoid(root, input);

        // Execute Graph
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({sigmoid}, &outputs);

        if (!status.ok()) {
            std::cout << "Run failed: " << status.ToString() << std::endl;
        } else {
