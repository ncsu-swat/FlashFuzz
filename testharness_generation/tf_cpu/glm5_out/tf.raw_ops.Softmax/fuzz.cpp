...
void printTensorInfo(const tensorflow::Tensor& tensor, const std::string& name) {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
    try {
        ...
        if (tensor_shape.num_elements() > 1000000) return 0;

        tensorflow::Tensor logits_tensor(dtype, tensor_shape);
        fillTensorWithDataByType(logits_tensor, dtype, Data, offset, Size);

        printTensorInfo(logits_tensor, "Input logits");

        // Build Graph using Scope + ClientSession pattern
        tensorflow::Scope root = tensorflow::Scope::NewRootScope();

        auto input = tensorflow::ops::Const(root, logits_tensor);
        auto softmax = tensorflow::ops::Softmax(root, input);

        // Execute Graph
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({softmax}, &outputs);

        if (!status.ok()) {
            std::cout << "Softmax error: " << status.ToString() << std::endl;
        } else {
            if (!outputs.empty()) {
                printTensorInfo(outputs[0], "Output softmax");
            }
        }
