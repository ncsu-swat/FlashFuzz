...
Uses the NVIDIA NIM OpenAI-compatible Chat Completions endpoint
at https://integrate.api.nvidia.com with model `z-ai/glm5`.
...
def call_nim(
    ...
    *,
    ...
    """Call NVIDIA NIM (OpenAI-compatible) with retries and endpoint fallback.
    ...
    if api_key:
        # NVIDIA NIM keys require "nvapi-" prefix in the bearer token
        token = api_key if api_key.startswith("nvapi-") else f"nvapi-{api_key}"
        ...
        for attempt in range(1, max(1, retries) + 1):
            try:
                ...
                try:
                    ...
                    snippet = ""
                print(f"[ERROR] NIM call failed for {api_name}: HTTP {status} at {base}")
                if snippet:
                ...
                if retriable and attempt < max(1, retries):
                    ...
                    delay *= 1.6
                    continue
                print(f"[ERROR] NIM call failed for {api_name}: {e}")
                break
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate C++ test harnesses via NVIDIA NIM (GLM-5)")
    ...
    p.add_argument("--model", default=os.environ.get("NIM_MODEL", "z-ai/glm5"))
    p.add_argument(
        "--api-base",
        default=os.environ.get("NIM_API_BASE", "https://integrate.api.nvidia.com"),
        help=(
            "Base URL for NVIDIA NIM API (no /v1 suffix). "
            "Comma-separate multiple bases to enable fallback."
        ),
    )
    p.add_argument("--api-key", default=os.environ.get("NIM_API_KEY"), help="Bearer token for NIM API")
