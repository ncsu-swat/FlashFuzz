...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
{
    try
    {
        ...
        if (operation_type == 0) {
            ...
            if (offset >= Size) {
                torch::amax(input_tensor, c10::nullopt, keepdim);
                return 0;
            }
            ...
            if (tensor_rank == 0) {
                ...
                int64_t dim = static_cast<int64_t>(static_cast<int8_t>(dim_selector)) % (tensor_rank * 2 + 1) - tensor_rank;
                torch::amax(input_tensor, dim, keepdim);
            }
        }
        else {
            if (offset + 1 >= Size) {
            ...
            int64_t tensor_rank = input_tensor.dim();
            if (tensor_rank == 0) {
                torch::amax(input_tensor, {}, keepdim);
                return 0;
            }
            
            uint8_t num_dims = (num_dims_selector % static_cast<uint8_t>(tensor_rank + 1));
            ...
            torch::amax(input_tensor, dims, keepdim);
        }
    }
