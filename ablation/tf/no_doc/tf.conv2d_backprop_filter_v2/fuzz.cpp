...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto input_shape = std::vector<int64_t>{1, 4, 4, 3};
        auto out_backprop_shape = std::vector<int64_t>{1, 2, 2, 2};
        auto filter_sizes_shape = std::vector<int64_t>{4};
        ...
        for (int i = 0; i < filter_sizes_tensor.shape().dims(); ++i) {
            std::cout << filter_sizes_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
