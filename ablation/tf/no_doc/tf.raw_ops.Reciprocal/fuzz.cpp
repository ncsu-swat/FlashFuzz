...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        tensorflow::Status status = session.Run({reciprocal_op}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "Reciprocal operation completed successfully" << std::endl;
            std::cout << "Output tensor shape: " << outputs[0].shape().DebugString() << std::endl;
        } else {
            std::cout << "Reciprocal operation failed: " << status.ToString() << std::endl;
        }
