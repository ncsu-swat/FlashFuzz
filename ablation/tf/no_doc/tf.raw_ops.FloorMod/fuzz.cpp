...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto floor_mod_op = tensorflow::ops::FloorMod(root, x_placeholder, y_placeholder);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({{x_placeholder, x_tensor}, {y_placeholder, y_tensor}}, 
                                               {floor_mod_op}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "FloorMod operation completed successfully" << std::endl;
