...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto rsqrt_op = tensorflow::ops::Rsqrt(root, input_placeholder);
        ...
        tensorflow::Status run_status = session->Run({{input_placeholder.node()->name(), input_tensor}}, 
                                                     {rsqrt_op.node()->name()}, {}, &outputs);
        
        if (run_status.ok() && !outputs.empty()) {
            std::cout << "Rsqrt operation completed successfully" << std::endl;
            std::cout << "Output tensor shape: " << outputs[0].shape().DebugString() << std::endl;
        } else {
            std::cout << "Rsqrt operation failed: " << run_status.ToString() << std::endl;
        }
