...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        tensorflow::Tensor filter_sizes_tensor(tensorflow::DT_INT32, filter_sizes_shape);
        ...
        for (int i = 0; i < 5; ++i) {
            filter_sizes_flat(i) = static_cast<int32_t>(filter_sizes[i]);
        }
        ...
        auto conv3d_backprop_filter = tensorflow::ops::Conv3DBackpropFilterV2(
            ...
            strides, tensorflow::ops::Conv3DBackpropFilterV2::Padding(padding));
        ...
        tensorflow::Status status = session.Run(
            ...
            {conv3d_backprop_filter}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "Conv3DBackpropFilterV2 output shape: ";
            for (int i = 0; i < outputs[0].dims(); ++i) {
                std::cout << outputs[0].dim_size(i) << " ";
            }
            std::cout << std::endl;
        } else {
            std::cout << "Conv3DBackpropFilterV2 failed: " << status.ToString() << std::endl;
        }
