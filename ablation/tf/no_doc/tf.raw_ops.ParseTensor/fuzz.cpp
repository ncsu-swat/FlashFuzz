...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (remaining_data > 0) {
            std::string serialized_data(reinterpret_cast<const char*>(data + offset), remaining_data);
            serialized_tensor.scalar<tensorflow::tstring>()() = serialized_data;
        } else {
            serialized_tensor.scalar<tensorflow::tstring>()() = "";
        }

        std::cout << "Input serialized tensor: " << serialized_tensor.DebugString() << std::endl;
        std::cout << "Output type: " << tensorflow::DataTypeString(out_type) << std::endl;

        tensorflow::GraphDefBuilder builder(tensorflow::GraphDefBuilder::kFailImmediately);
        
        tensorflow::Node* serialized_node = tensorflow::ops::Const(serialized_tensor, builder.opts());
        
        tensorflow::Node* parse_tensor_node = tensorflow::ops::UnaryOp("ParseTensor", serialized_node,
                                                                      builder.opts().WithAttr("out_type", out_type));
        ...
        status = session->Run({}, {parse_tensor_node->name() + ":0"}, {}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "ParseTensor output: " << outputs[0].DebugString() << std::endl;
        } else {
            std::cout << "ParseTensor failed or no output: " << status.ToString() << std::endl;
        }
