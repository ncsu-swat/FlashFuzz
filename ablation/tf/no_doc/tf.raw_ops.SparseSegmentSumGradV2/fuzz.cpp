...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (offset + sizeof(int32_t) <= size) {
            std::memcpy(&num_segments, data + offset, sizeof(int32_t));
            offset += sizeof(int32_t);
            num_segments = std::abs(num_segments) % 100 + 1;
        } else {
            num_segments = 1;
        }
        ...
        auto sparse_segment_sum_grad = tensorflow::ops::SparseSegmentSumGradV2(
            root, grad_placeholder, indices_placeholder, segment_ids_placeholder, num_segments_placeholder);
        ...
        tensorflow::Status status = session.Run(
            ...
            {sparse_segment_sum_grad}, &outputs);
