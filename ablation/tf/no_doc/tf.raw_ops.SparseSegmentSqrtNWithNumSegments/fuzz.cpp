...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        fillTensorWithDataByType(num_segments_tensor, tensorflow::DT_INT32, data, offset, size);

        std::cout << "Data tensor shape: ";
        for (int i = 0; i < data_tensor.shape().dims(); ++i) {
            std::cout << data_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
        ...
        for (int i = 0; i < segment_ids_tensor.shape().dims(); ++i) {
            std::cout << segment_ids_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;

        std::cout << "Num segments: " << num_segments_tensor.scalar<int32_t>()() << std::endl;
        ...
        auto sparse_segment_sqrt_n = tensorflow::ops::SparseSegmentSqrtNWithNumSegments(
            root, data_placeholder, indices_placeholder, segment_ids_placeholder, num_segments_placeholder);
