...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        std::cout << std::endl;
        std::cout << "Data tensor type: " << tensorflow::DataTypeString(data_dtype) << std::endl;
        std::cout << "Predicate value: " << pred_value << std::endl;

        tensorflow::Scope root = tensorflow::Scope::NewRootScope();
        
        auto data_placeholder = tensorflow::ops::Placeholder(root, data_dtype);
        auto pred_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_BOOL);
        
        auto ref_switch_op = tensorflow::ops::RefSwitch(root, data_placeholder, pred_placeholder);

        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({{data_placeholder, data_tensor}, {pred_placeholder, pred_tensor}}, 
                                               {ref_switch_op.output_false, ref_switch_op.output_true}, &outputs);
        ...
        if (status.ok()) {
            ...
            for (size_t i = 0; i < outputs.size(); ++i) {
                ...
                for (int j = 0; j < outputs[i].shape().dims(); ++j) {
                    std::cout << outputs[i].shape().dim_size(j) << " ";
                }
                std::cout << std::endl;
            }
        } else {
