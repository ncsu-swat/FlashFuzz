...
#include <tensorflow/core/kernels/inplace_ops.h>
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        tensorflow::Tensor x_tensor(dtype, tensor_shape);
        tensorflow::Tensor v_tensor(dtype, tensor_shape);
        
        fillTensorWithDataByType(x_tensor, dtype, data, offset, size);
        fillTensorWithDataByType(v_tensor, dtype, data, offset, size);
        
        std::cout << "Input tensor x shape: ";
        ...
        if (offset < size) {
            int32_t i_value;
            std::memcpy(&i_value, data + offset, std::min(sizeof(int32_t), size - offset));
            i_value = std::abs(i_value) % tensor_shape.num_elements();
            
            tensorflow::Tensor i_tensor(tensorflow::DT_INT32, tensorflow::TensorShape({}));
            i_tensor.scalar<int32_t>()() = i_value;
            
            std::cout << "Index i: " << i_value << std::endl;
            ...
            auto v_node = tensorflow::ops::Const(v_tensor, builder.opts().WithName("v"));
            auto i_node = tensorflow::ops::Const(i_tensor, builder.opts().WithName("i"));
            
            tensorflow::NodeDef inplace_add_node;
            ...
            if (!status.ok()) {
                std::cout << "Graph building failed: " << status.ToString() << std::endl;
                return 0;
            }
            
            *graph_def.add_node() = inplace_add_node;
            ...
            status = session->Run({}, {"inplace_add:0"}, {}, &outputs);
            if (!status.ok()) {
