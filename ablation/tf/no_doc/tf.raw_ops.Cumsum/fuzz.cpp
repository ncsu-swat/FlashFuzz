...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (rank > 0 && offset < size) {
            axis = static_cast<int32_t>(data[offset++] % rank);
        }
        ...
        auto axis_tensor = tensorflow::ops::Const(root, axis);
        
        auto cumsum_op = tensorflow::ops::Cumsum(root, input_placeholder, axis_tensor,
                                               tensorflow::ops::Cumsum::Exclusive(exclusive).Reverse(reverse));
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}}, {cumsum_op}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "Cumsum operation completed successfully" << std::endl;
            ...
            std::cout << "Cumsum operation failed: " << status.ToString() << std::endl;
        }
