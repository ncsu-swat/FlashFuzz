...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (tensor_rank == 0 || indices_rank == 0 || updates_rank == 0) {
            return 0;
        }
        ...
        if (tensor_shape.empty() || indices_shape.empty() || updates_shape.empty()) {
            return 0;
        }
        ...
        tensorflow::Tensor tensor_tensor(dtype, tensor_tensor_shape);
        ...
        fillTensorWithDataByType(tensor_tensor, dtype, data, offset, size);
        ...
        tensorflow::Status status = session.Run({{tensor_placeholder, tensor_tensor}, 
                                                 {indices_placeholder, indices_tensor}, 
                                                 {updates_placeholder, updates_tensor}}, 
                                                {tensor_scatter_max}, &outputs);

        if (!status.ok()) {
            std::cout << "TensorScatterMax operation failed: " << status.ToString() << std::endl;
            return 0;
        }

        std::cout << "TensorScatterMax operation completed successfully" << std::endl;
