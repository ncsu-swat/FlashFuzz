...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        input_sizes_flat(0) = 1;
        input_sizes_flat(1) = 4;
        input_sizes_flat(2) = 4;
        input_sizes_flat(3) = 4;
        input_sizes_flat(4) = 2;
        ...
        std::cout << "Input sizes tensor shape: ";
        for (int i = 0; i < input_sizes_tensor.shape().dims(); ++i) {
            std::cout << input_sizes_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;

        std::cout << "Filter tensor shape: ";
        for (int i = 0; i < filter_tensor.shape().dims(); ++i) {
            std::cout << filter_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
        ...
        auto input_sizes_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_INT32);
        ...
        tensorflow::Status status = session.Run(
            {{input_sizes_placeholder, input_sizes_tensor},
             {filter_placeholder, filter_tensor},
             {out_backprop_placeholder, out_backprop_tensor}},
            {conv3d_backprop_input}, &outputs);

        if (status.ok() && !outputs.empty()) {
            std::cout << "Conv3DBackpropInput executed successfully" << std::endl;
            ...
            std::cout << "Conv3DBackpropInput failed: " << status.ToString() << std::endl;
        }
