...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (ref_shape.empty()) ref_shape = {1};
        if (indices_shape.empty()) indices_shape = {1};
        if (updates_shape.empty()) updates_shape = {1};
        ...
        auto scatter_div = tensorflow::ops::ScatterDiv(root, ref_placeholder, indices_placeholder, updates_placeholder);
        ...
        if (!status.ok()) {
            std::cout << "ScatterDiv operation failed: " << status.ToString() << std::endl;
            return 0;
        }
