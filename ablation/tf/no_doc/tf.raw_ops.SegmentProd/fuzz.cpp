...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (offset + sizeof(int64_t) <= size) {
            std::memcpy(&num_segments, data + offset, sizeof(int64_t));
            offset += sizeof(int64_t);
            num_segments = std::abs(num_segments) % 10 + 1;
        }
        
        int64_t total_elements = data_tensor_shape.num_elements();
        tensorflow::TensorShape segment_ids_shape;
        segment_ids_shape.AddDim(total_elements);
        
        tensorflow::Tensor segment_ids_tensor(tensorflow::DT_INT32, segment_ids_shape);
        ...
        for (int64_t i = 0; i < total_elements; ++i) {
            if (offset + sizeof(int32_t) <= size) {
                ...
                segment_ids_flat(i) = static_cast<int32_t>(i % num_segments);
            }
        }
        
        std::cout << "Segment IDs tensor shape: ";
        for (int i = 0; i < segment_ids_shape.dims(); ++i) {
            std::cout << segment_ids_shape.dim_size(i) << " ";
        }
        std::cout << std::endl;
        
        std::cout << "Data dtype: " << tensorflow::DataTypeString(data_dtype) << std::endl;
        ...
        auto segment_prod = tensorflow::ops::SegmentProd(root, data_placeholder, segment_ids_placeholder);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({{data_placeholder, data_tensor}, 
                                                 {segment_ids_placeholder, segment_ids_tensor}}, 
                                                {segment_prod}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "SegmentProd operation completed successfully" << std::endl;
            ...
            std::cout << "SegmentProd operation failed: " << status.ToString() << std::endl;
        }
