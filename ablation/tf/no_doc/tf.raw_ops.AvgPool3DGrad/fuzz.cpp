...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        uint8_t orig_output_shape_rank = parseRank(data[offset++]);
        std::vector<int64_t> orig_output_shape = parseShape(data, offset, size, orig_output_shape_rank);
        ...
        std::vector<int32_t> ksize(5, 1);
        std::vector<int32_t> strides(5, 1);
        ...
        for (int i = 0; i < 5 && offset < size; ++i) {
            if (offset + sizeof(int32_t) <= size) {
                ...
                std::memcpy(&val, data + offset, sizeof(int32_t));
                offset += sizeof(int32_t);
                strides[i] = std::abs(val) % 5 + 1;
            }
        }

        std::string padding = (offset < size && data[offset++] % 2 == 0) ? "VALID" : "SAME";

        tensorflow::TensorShape orig_output_tensor_shape(orig_output_shape);
        tensorflow::Tensor orig_output_tensor(dtype, orig_output_tensor_shape);
        ...
        std::cout << "orig_output_shape: ";
        for (auto dim : orig_output_shape) {
            std::cout << dim << " ";
        }
        std::cout << std::endl;
        ...
        tensorflow::Status status = session.Run(
            {{orig_output_placeholder, orig_output_tensor}, {grad_placeholder, grad_tensor}},
            {avg_pool_3d_grad},
            &outputs
        );

        if (status.ok() && !outputs.empty()) {
            std::cout << "AvgPool3DGrad operation completed successfully" << std::endl;
            ...
            std::cout << "AvgPool3DGrad operation failed: " << status.ToString() << std::endl;
        }
