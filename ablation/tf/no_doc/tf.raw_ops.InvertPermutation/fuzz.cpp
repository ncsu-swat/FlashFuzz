...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto invert_permutation_op = tensorflow::ops::InvertPermutation(root, x_placeholder);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({{x_placeholder, input_tensor}}, {invert_permutation_op}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "InvertPermutation operation succeeded" << std::endl;
            ...
            if (dtype == tensorflow::DT_INT32) {
                ...
                auto flat = outputs[0].flat<int64_t>();
                for (int i = 0; i < flat.size(); ++i) {
                    std::cout << flat(i) << " ";
                }
            }
            ...
            std::cout << "InvertPermutation operation failed: " << status.ToString() << std::endl;
        }
