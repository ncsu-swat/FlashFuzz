...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        std::cout << "Input tensor created with " << input_tensor.NumElements() << " elements" << std::endl;
        
        tensorflow::NodeDef node_def;
        node_def.set_name("serialize_tensor");
        ...
        std::vector<tensorflow::AllocatorAttributes> alloc_attrs;
        alloc_attrs.resize(1);
        params.output_alloc_attr = &alloc_attrs;
        
        tensorflow::gtl::InlinedVector<tensorflow::TensorValue, 4> inputs;
        inputs.push_back(tensorflow::TensorValue(&input_tensor));
        params.inputs = &inputs;
        
        tensorflow::OpKernelConstruction construction(
            tensorflow::DeviceType("CPU"), &device, 
            tensorflow::cpu_allocator(), &node_def,
            tensorflow::OpDef(), nullptr);
        
        std::unique_ptr<tensorflow::OpKernel> kernel;
        auto status = tensorflow::CreateOpKernel("CPU", &device, tensorflow::cpu_allocator(), 
                                                node_def, TF_GRAPH_DEF_VERSION, &kernel);
        
        if (!status.ok()) {
            std::cout << "Failed to create kernel: " << status.ToString() << std::endl;
            return 0;
        }
        ...
        std::cout << "SerializeTensor operation completed successfully" << std::endl;
