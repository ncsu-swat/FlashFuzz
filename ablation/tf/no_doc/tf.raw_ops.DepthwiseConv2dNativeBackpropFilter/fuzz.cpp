...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        uint8_t filter_size_rank = 1;
        uint8_t out_backprop_rank = 4;
        
        std::vector<int64_t> input_shape = {1, 8, 8, 3};
        std::vector<int64_t> filter_size_shape = {2};
        std::vector<int64_t> out_backprop_shape = {1, 4, 4, 6};
        ...
        if (offset + 8 * 3 <= size) {
            ...
            filter_size_shape = parseShape(data, offset, size, filter_size_rank);
            if (filter_size_shape.size() != 1) {
                filter_size_shape = {2};
            }
            
            out_backprop_shape = parseShape(data, offset, size, out_backprop_rank);
        ...
        tensorflow::TensorShape filter_size_tensor_shape(filter_size_shape);
        ...
        tensorflow::Tensor filter_size_tensor(tensorflow::DT_INT32, filter_size_tensor_shape);
        ...
        for (int i = 0; i < filter_size_flat.size(); ++i) {
            if (offset + sizeof(int32_t) <= size) {
                int32_t val;
                std::memcpy(&val, data + offset, sizeof(int32_t));
                offset += sizeof(int32_t);
                filter_size_flat(i) = std::abs(val) % 5 + 1;
            } else {
                filter_size_flat(i) = 3;
            }
        }
        ...
        std::cout << "Filter size tensor shape: ";
        for (int i = 0; i < filter_size_tensor.shape().dims(); ++i) {
            std::cout << filter_size_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
        
        std::cout << "Out backprop tensor shape: ";
        for (int i = 0; i < out_backprop_tensor.shape().dims(); ++i) {
            std::cout << out_backprop_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
        ...
        auto depthwise_conv2d_backprop_filter = tensorflow::ops::DepthwiseConv2dNativeBackpropFilter(
            ...
            strides, padding,
            tensorflow::ops::DepthwiseConv2dNativeBackpropFilter::Attrs().Dilations(dilations)
        );
