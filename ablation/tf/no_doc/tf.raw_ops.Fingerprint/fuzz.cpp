...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto fingerprint_op = tensorflow::ops::Fingerprint(root, input_placeholder);
        ...
        TF_CHECK_OK(session->Run(inputs, {fingerprint_op.node()->name()}, {}, &outputs));
        
        if (!outputs.empty()) {
            std::cout << "Fingerprint output shape: " << outputs[0].shape().DebugString() << std::endl;
            auto output_flat = outputs[0].flat<unsigned char>();
            std::cout << "Fingerprint bytes: ";
            for (int i = 0; i < std::min(16, static_cast<int>(output_flat.size())); ++i) {
                std::cout << static_cast<int>(output_flat(i)) << " ";
            }
            std::cout << std::endl;
        }
