...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        std::vector<int32_t> ksize(4);
        std::vector<int32_t> strides(4);
        for (int i = 0; i < 4; ++i) {
        ...
        auto maxpool_op = tensorflow::ops::MaxPoolWithArgmax(
            root, 
            input_placeholder, 
            ksize, 
            strides, 
            padding,
            tensorflow::ops::MaxPoolWithArgmax::IncludeBatchInIndex(include_batch_in_index)
        );
        ...
        std::vector<std::string> output_names = {
            maxpool_op.output.node()->name(),
            maxpool_op.argmax.node()->name()
        };
