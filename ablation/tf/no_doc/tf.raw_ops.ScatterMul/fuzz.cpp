...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        tensorflow::DataType updates_dtype = ref_dtype;
        ...
        auto updates_placeholder = tensorflow::ops::Placeholder(root, updates_dtype);
        
        auto scatter_mul = tensorflow::ops::ScatterMul(root, ref_placeholder, indices_placeholder, updates_placeholder);
        ...
        if (!status.ok()) {
            std::cout << "ScatterMul operation failed: " << status.ToString() << std::endl;
            return 0;
        }
