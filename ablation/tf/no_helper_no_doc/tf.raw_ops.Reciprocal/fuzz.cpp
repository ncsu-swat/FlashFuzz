        ...
        if (offset + required_bytes > size) {
            ...
            switch (dtype) {
                case tensorflow::DT_FLOAT: {
                    ...
                    for (int i = 0; i < total_elements; i++) {
                        float val = (offset < size) ? 
                            static_cast<float>(data[offset % size]) / 255.0f + 0.1f : 1.0f;
                        flat(i) = val;
                        offset++;
                    }
                    break;
                ...
                case tensorflow::DT_DOUBLE: {
                    ...
                    for (int i = 0; i < total_elements; i++) {
                        double val = (offset < size) ? 
                            static_cast<double>(data[offset % size]) / 255.0 + 0.1 : 1.0;
                        flat(i) = val;
                        offset++;
                    }
                    break;
            ...
            switch (dtype) {
                ...
                default: {
                    auto flat = input_tensor.flat<float>();
                    for (int i = 0; i < total_elements; i++) {
                        float val = static_cast<float>(data[offset]) / 255.0f + 0.1f;
                        flat(i) = val;
                        offset++;
                    }
                    break;
