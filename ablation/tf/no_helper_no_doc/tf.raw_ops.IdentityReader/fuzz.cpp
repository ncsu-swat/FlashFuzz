...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        int32_t container_len = 0;
        if (offset + sizeof(int32_t) <= size) {
            memcpy(&container_len, data + offset, sizeof(int32_t));
            offset += sizeof(int32_t);
            container_len = std::abs(container_len) % 256; // Limit length
        }
        ...
        int32_t shared_name_len = 0;
        if (offset + sizeof(int32_t) <= size) {
            memcpy(&shared_name_len, data + offset, sizeof(int32_t));
            offset += sizeof(int32_t);
            shared_name_len = std::abs(shared_name_len) % 256; // Limit length
        }
        ...
        auto identity_reader = tensorflow::ops::SourceOp("IdentityReader", 
            builder.opts()
