...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        int32_t dense_dim0_value = static_cast<int32_t>(grad_shape[0]);
        if (offset < size) {
            std::memcpy(&dense_dim0_value, data + offset, std::min(sizeof(int32_t), size - offset));
            dense_dim0_value = std::abs(dense_dim0_value) % 100 + 1;
        }
        dense_output_dim0_tensor.scalar<int32_t>()() = dense_dim0_value;

        auto indices_flat = indices_tensor.flat<int32_t>();
        if (indices_dtype == tensorflow::DT_INT32) {
            for (int i = 0; i < indices_flat.size(); ++i) {
                indices_flat(i) = std::abs(indices_flat(i)) % dense_dim0_value;
            }
        } else {
            auto indices_flat_64 = indices_tensor.flat<int64_t>();
            for (int i = 0; i < indices_flat_64.size(); ++i) {
                indices_flat_64(i) = std::abs(indices_flat_64(i)) % dense_dim0_value;
            }
        }
        ...
        auto sparse_segment_sum_grad_v2 = tensorflow::ops::SparseSegmentSumGradV2(
        ...
        }, {sparse_segment_sum_grad_v2.output, sparse_segment_sum_grad_v2.sorted_unique_indices}, &outputs);
