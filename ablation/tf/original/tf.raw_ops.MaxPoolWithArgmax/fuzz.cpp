...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        tensorflow::DataType targmax_dtype = (offset < size && data[offset++] % 2 == 0) ? 
                                           tensorflow::DT_INT32 : tensorflow::DT_INT64;
        
        bool include_batch_in_index = (offset < size && data[offset++] % 2 == 0);
        ...
        auto max_pool_op = tensorflow::ops::MaxPoolWithArgmax(
            root, input_placeholder, ksize, strides, padding,
            tensorflow::ops::MaxPoolWithArgmax::Targmax(targmax_dtype)
        ...
        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}}, 
                                               {max_pool_op.output, max_pool_op.argmax}, &outputs);

        if (status.ok() && outputs.size() == 2) {
            std::cout << "MaxPoolWithArgmax executed successfully" << std::endl;
