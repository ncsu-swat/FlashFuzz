...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        for (int i = 0; i < tensor_x.shape().dims(); ++i) {
            std::cout << tensor_x.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;

        std::cout << "Input tensor y shape: ";
        for (int i = 0; i < tensor_y.shape().dims(); ++i) {
            std::cout << tensor_y.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
