...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        uint8_t a_nnz_byte = data[offset++];
        int64_t a_nnz = 1 + (a_nnz_byte % 5);
        
        tensorflow::TensorShape a_indices_shape({a_nnz, static_cast<int64_t>(a_rank)});
        tensorflow::Tensor a_indices(tensorflow::DT_INT64, a_indices_shape);
        fillTensorWithData<int64_t>(a_indices, data, offset, size);
        
        tensorflow::TensorShape a_values_shape({a_nnz});
        tensorflow::Tensor a_values(value_dtype, a_values_shape);
        fillTensorWithDataByType(a_values, value_dtype, data, offset, size);
        
        tensorflow::TensorShape a_shape_tensor_shape({static_cast<int64_t>(a_rank)});
        ...
        for (int i = 0; i < a_rank; ++i) {
            a_shape_flat(i) = a_shape[i];
        }
        
        if (offset >= size) return 0;
        
        uint8_t b_nnz_byte = data[offset++];
        int64_t b_nnz = 1 + (b_nnz_byte % 5);
        
        tensorflow::TensorShape b_indices_shape({b_nnz, static_cast<int64_t>(a_rank)});
        tensorflow::Tensor b_indices(tensorflow::DT_INT64, b_indices_shape);
        fillTensorWithData<int64_t>(b_indices, data, offset, size);
        
        tensorflow::TensorShape b_values_shape({b_nnz});
        tensorflow::Tensor b_values(value_dtype, b_values_shape);
        fillTensorWithDataByType(b_values, value_dtype, data, offset, size);
        
        tensorflow::TensorShape b_shape_tensor_shape({static_cast<int64_t>(a_rank)});
        tensorflow::Tensor b_shape_tensor(tensorflow::DT_INT64, b_shape_tensor_shape);
        auto b_shape_flat = b_shape_tensor.flat<int64_t>();
        for (int i = 0; i < a_rank; ++i) {
            b_shape_flat(i) = a_shape[i];
        }
        
        std::cout << "a_indices shape: " << a_indices.shape().DebugString() << std::endl;
        std::cout << "a_values shape: " << a_values.shape().DebugString() << std::endl;
        std::cout << "a_shape shape: " << a_shape_tensor.shape().DebugString() << std::endl;
        std::cout << "b_indices shape: " << b_indices.shape().DebugString() << std::endl;
        std::cout << "b_values shape: " << b_values.shape().DebugString() << std::endl;
        std::cout << "b_shape shape: " << b_shape_tensor.shape().DebugString() << std::endl;
        ...
        tensorflow::Status status = session.Run({sparse_sparse_maximum.output_indices, 
                                                sparse_sparse_maximum.output_values}, &outputs);
