    ...
    try {
        ...
        if (offset < size) {
            ...
            for (uint8_t i = 0; i < handle_len && offset < size; ++i) {
                handle_str += static_cast<char>(data[offset]);
                offset++;
            }
        ...
        std::cout << "Handle: " << handle_tensor.scalar<tensorflow::tstring>()() << std::endl;
        ...
        auto get_session_tensor_op = tensorflow::ops::GetSessionTensor(
            root.WithOpName("get_session_tensor"),
            handle_placeholder,
            dtype
        );
        ...
        if (!status.ok()) {
            std::cout << "Failed to create session with graph: " << status.ToString() << std::endl;
            return 0;
        }
        ...
        std::vector<std::string> output_names = {"get_session_tensor"};

        status = session->Run(inputs, output_names, {}, &outputs);
        if (!status.ok()) {
            std::cout << "Session run failed: " << status.ToString() << std::endl;
        } else {
            std::cout << "GetSessionTensor operation completed successfully" << std::endl;
            if (!outputs.empty()) {
