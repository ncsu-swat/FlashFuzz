...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        std::cout << "Method: farmhash64" << std::endl;
        
        tensorflow::Scope root = tensorflow::Scope::NewRootScope();
        
        auto data_placeholder = tensorflow::ops::Placeholder(root.WithOpName("data"), dtype);
        auto method_placeholder = tensorflow::ops::Placeholder(root.WithOpName("method"), tensorflow::DT_STRING);
        
        auto fingerprint_op = tensorflow::ops::internal::Fingerprint(
            root.WithOpName("fingerprint"), 
            data_placeholder, 
            method_placeholder
        );
        ...
        tensorflow::Status status = session->Run(inputs, {"fingerprint"}, {}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "Fingerprint operation successful" << std::endl;
            std::cout << "Output shape: " << outputs[0].shape().DebugString() << std::endl;
            std::cout << "Output type: " << tensorflow::DataTypeString(outputs[0].dtype()) << std::endl;
        } else {
            std::cout << "Fingerprint operation failed: " << status.ToString() << std::endl;
        }
