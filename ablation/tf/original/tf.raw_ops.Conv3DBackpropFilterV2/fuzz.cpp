...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        std::vector<int64_t> input_shape = {2, 4, 4, 4, 3};
        std::vector<int64_t> filter_sizes_shape = {5};
        std::vector<int64_t> out_backprop_shape = {2, 4, 4, 4, 2};
        ...
        filter_sizes_flat(3) = 3;
        filter_sizes_flat(4) = 2;
        ...
        auto conv3d_backprop_filter = tensorflow::ops::Conv3DBackpropFilterV2(
            ...
            strides,
            padding,
            tensorflow::ops::Conv3DBackpropFilterV2::DataFormat(data_format).Dilations(dilations)
        );
        ...
        status = session->Run(inputs, {conv3d_backprop_filter.node()->name()}, {}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "Conv3DBackpropFilterV2 executed successfully" << std::endl;
