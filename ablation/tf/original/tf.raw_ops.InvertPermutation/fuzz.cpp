...
void makeValidPermutation(tensorflow::Tensor& tensor, tensorflow::DataType dtype) {
    if (dtype == tensorflow::DT_INT32) {
        auto flat = tensor.flat<int32_t>();
        const size_t num_elements = flat.size();
        for (size_t i = 0; i < num_elements; ++i) {
            flat(i) = static_cast<int32_t>(i);
        }
        std::random_shuffle(flat.data(), flat.data() + num_elements);
    } else if (dtype == tensorflow::DT_INT64) {
        auto flat = tensor.flat<int64_t>();
        const size_t num_elements = flat.size();
        for (size_t i = 0; i < num_elements; ++i) {
            flat(i) = static_cast<int64_t>(i);
        }
        std::random_shuffle(flat.data(), flat.data() + num_elements);
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        makeValidPermutation(input_tensor, dtype);
        ...
        std::cout << "Input tensor data type: " << tensorflow::DataTypeString(dtype) << std::endl;
        
        tensorflow::Scope root = tensorflow::Scope::NewRootScope();
        
        auto x_placeholder = tensorflow::ops::Placeholder(root, dtype);
        auto invert_perm_op = tensorflow::ops::InvertPermutation(root, x_placeholder);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({{x_placeholder, input_tensor}}, {invert_perm_op}, &outputs);
