...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        int64_t bias_size = value_shape.back();
        std::vector<int64_t> bias_shape = {bias_size};
        
        uint8_t data_format_selector = data[offset++] % 2;
        std::string data_format = (data_format_selector == 0) ? "NHWC" : "NCHW";
        
        if (data_format == "NCHW" && value_shape.size() >= 3) {
            bias_size = value_shape[value_shape.size() - 3];
            bias_shape = {bias_size};
        }

        tensorflow::TensorShape value_tensor_shape(value_shape);
        tensorflow::TensorShape bias_tensor_shape(bias_shape);
        
        tensorflow::Tensor value_tensor(dtype, value_tensor_shape);
        tensorflow::Tensor bias_tensor(dtype, bias_tensor_shape);
        
        fillTensorWithDataByType(value_tensor, dtype, data, offset, size);
        fillTensorWithDataByType(bias_tensor, dtype, data, offset, size);
        
        std::cout << "Value tensor shape: ";
        for (int i = 0; i < value_tensor.dims(); ++i) {
            std::cout << value_tensor.dim_size(i) << " ";
        }
        std::cout << std::endl;
        
        std::cout << "Bias tensor shape: ";
        for (int i = 0; i < bias_tensor.dims(); ++i) {
            std::cout << bias_tensor.dim_size(i) << " ";
        }
        std::cout << std::endl;
        ...
        tensorflow::ops::BiasAdd::Attrs attrs;
        attrs = attrs.DataFormat(data_format);
        
        auto bias_add_op = tensorflow::ops::BiasAdd(root, value_placeholder, bias_placeholder, attrs);
        ...
        std::vector<std::pair<std::string, tensorflow::Tensor>> inputs = {
            {value_placeholder.node()->name(), value_tensor},
            {bias_placeholder.node()->name(), bias_tensor}
        };
        
        std::vector<tensorflow::Tensor> outputs;
        std::vector<std::string> output_names = {bias_add_op.node()->name()};
        
        tensorflow::Status status = session->Run(inputs, output_names, {}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "BiasAdd operation completed successfully" << std::endl;
