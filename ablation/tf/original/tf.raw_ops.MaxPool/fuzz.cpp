    ...
    if (padding == "EXPLICIT") {
        ...
        for (int i = 0; i < 8; ++i) {
            if (offset + sizeof(int) <= total_size) {
                ...
                std::memcpy(&val, data + offset, sizeof(int));
                offset += sizeof(int);
                explicit_paddings[i] = std::abs(val) % 5;
            } else {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto maxpool_op = tensorflow::ops::MaxPool(root, input_placeholder, ksize, strides, padding, attrs);
        ...
        status = session->Run(inputs, {maxpool_op.node()->name()}, {}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "MaxPool operation successful. Output shape: ";
