...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (!tensorflow::TensorShape::BuildTensorShape(var_shape, &var_tensor_shape).ok()) {
            return 0;
        }
        if (!tensorflow::TensorShape::BuildTensorShape(grad_shape, &grad_tensor_shape).ok()) {
            return 0;
        }
        ...
        tensorflow::Tensor l1_tensor(lr_dtype, tensorflow::TensorShape({}));
        tensorflow::Tensor l2_tensor(lr_dtype, tensorflow::TensorShape({}));
        ...
        fillTensorWithDataByType(l1_tensor, lr_dtype, data, offset, size);
        fillTensorWithDataByType(l2_tensor, lr_dtype, data, offset, size);
        ...
        std::cout << "lr_dtype: " << tensorflow::DataTypeString(lr_dtype) << std::endl;
        std::cout << "indices_dtype: " << tensorflow::DataTypeString(indices_dtype) << std::endl;
        std::cout << "var_shape: ";
        ...
        auto var_node = tensorflow::ops::Variable(var_tensor_shape, lr_dtype, 
                                                 tensorflow::ops::Variable::Attrs().Container("").SharedName("var"));
        auto accum_node = tensorflow::ops::Variable(var_tensor_shape, lr_dtype,
                                                   tensorflow::ops::Variable::Attrs().Container("").SharedName("accum"));
        ...
        auto init_var = tensorflow::ops::Assign(builder.opts().WithName("init_var"), 
                                               var_node, 
                                               tensorflow::ops::Const(builder.opts(), 
                                                                     tensorflow::Tensor(lr_dtype, var_tensor_shape)));
        auto init_accum = tensorflow::ops::Assign(builder.opts().WithName("init_accum"), 
                                                 accum_node, 
                                                 tensorflow::ops::Const(builder.opts(), 
                                                                       tensorflow::Tensor(lr_dtype, var_tensor_shape)));
        ...
        status = session->Run({}, {"init_var", "init_accum"}, {}, &init_outputs);
        if (!status.ok()) {
            std::cout << "Failed to initialize variables: " << status.ToString() << std::endl;
            return 0;
        }

        std::cout << "Successfully created and initialized TensorFlow session" << std::endl;
