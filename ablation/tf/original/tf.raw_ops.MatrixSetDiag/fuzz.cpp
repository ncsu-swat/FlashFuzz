...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        diagonal_shape[diagonal_shape.size() - 2] = min_MN;
        ...
        for (int i = 0; i < diagonal_tensor.shape().dims(); ++i) {
            std::cout << diagonal_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
        ...
        auto diagonal_placeholder = tensorflow::ops::Placeholder(root.WithOpName("diagonal"), dtype);
        
        tensorflow::Node* matrix_set_diag_node;
        tensorflow::NodeBuilder("matrix_set_diag", "MatrixSetDiag")
            .Input(input_placeholder.node())
            .Input(diagonal_placeholder.node())
            .Finalize(root.graph(), &matrix_set_diag_node);
