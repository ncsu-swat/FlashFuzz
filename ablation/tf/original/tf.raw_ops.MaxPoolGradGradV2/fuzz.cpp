...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (offset + 8 <= size) {
            int32_t ksize_val, strides_val;
            std::memcpy(&ksize_val, data + offset, sizeof(int32_t));
            offset += sizeof(int32_t);
            std::memcpy(&strides_val, data + offset, sizeof(int32_t));
            offset += sizeof(int32_t);
            
            ksize_val = std::abs(ksize_val) % 5 + 1;
            strides_val = std::abs(strides_val) % 3 + 1;
            
            ksize_data[1] = ksize_val;
            ksize_data[2] = ksize_val;
            strides_data[1] = strides_val;
            strides_data[2] = strides_val;
        }
        ...
        std::cout << "orig_input shape: ";
        for (auto dim : orig_input_shape) {
            std::cout << dim << " ";
        }
        std::cout << std::endl;
        ...
        auto orig_input_placeholder = tensorflow::ops::Placeholder(root, dtype);
        ...
        tensorflow::Status status = session.Run({
            {orig_input_placeholder, orig_input_tensor},
            {orig_output_placeholder, orig_output_tensor},
            {grad_placeholder, grad_tensor},
            {ksize_placeholder, ksize_tensor},
            {strides_placeholder, strides_tensor}
        }, {max_pool_grad_grad_v2}, &outputs);
