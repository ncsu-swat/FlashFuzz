...
std::string parseDataFormat(uint8_t selector) {
    return (selector % 2 == 0) ? "NDHWC" : "NCDHW";
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        std::string data_format = parseDataFormat(data[(offset + 1) % size]);
        ...
        tensorflow::ops::MaxPool3D::Attrs attrs;
        attrs = attrs.DataFormat(data_format);
        
        auto maxpool3d_op = tensorflow::ops::MaxPool3D(
            root, input_placeholder, ksize, strides, padding, attrs);
        ...
        if (status.ok() && !outputs.empty()) {
            std::cout << "MaxPool3D operation successful" << std::endl;
