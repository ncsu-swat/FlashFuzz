...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        std::cout << "Min tensor: " << min_tensor.scalar<float>()() << std::endl;
        std::cout << "Max tensor: " << max_tensor.scalar<float>()() << std::endl;
        ...
        tensorflow::AttrValue gradients_dtype_attr;
        gradients_dtype_attr.set_type(tensorflow::DT_FLOAT);
        (*gradients_node->mutable_attr())["dtype"] = gradients_dtype_attr;

        tensorflow::NodeDef* inputs_node = graph_def.add_node();
        inputs_node->set_name("inputs");
        inputs_node->set_op("Placeholder");
        tensorflow::AttrValue inputs_dtype_attr;
        inputs_dtype_attr.set_type(tensorflow::DT_FLOAT);
        (*inputs_node->mutable_attr())["dtype"] = inputs_dtype_attr;

        tensorflow::NodeDef* min_node = graph_def.add_node();
        min_node->set_name("min");
        min_node->set_op("Placeholder");
        tensorflow::AttrValue min_dtype_attr;
        min_dtype_attr.set_type(tensorflow::DT_FLOAT);
        (*min_node->mutable_attr())["dtype"] = min_dtype_attr;

        tensorflow::NodeDef* max_node = graph_def.add_node();
        max_node->set_name("max");
        max_node->set_op("Placeholder");
        tensorflow::AttrValue max_dtype_attr;
        max_dtype_attr.set_type(tensorflow::DT_FLOAT);
        (*max_node->mutable_attr())["dtype"] = max_dtype_attr;
