...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto ref_var = tensorflow::ops::Variable(root, tensor_shape, dtype);
        ...
        tensorflow::ops::AssignSub::Attrs attrs;
        attrs = attrs.UseLocking(use_locking);
        auto assign_sub_op = tensorflow::ops::AssignSub(root, ref_var, value_const, attrs);
        ...
        TF_CHECK_OK(session->Run({}, {assign_ref.node()->name()}, {}, &outputs));
        
        outputs.clear();
        TF_CHECK_OK(session->Run({}, {assign_sub_op.output.node()->name()}, {}, &outputs));
