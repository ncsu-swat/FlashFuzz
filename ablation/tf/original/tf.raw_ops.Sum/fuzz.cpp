...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        offset++;
        if (axis_rank > input_rank) {
            axis_rank = input_rank;
        }
        ...
        for (int i = 0; i < axis_flat.size(); ++i) {
            if (offset < size) {
                int32_t axis_val = static_cast<int32_t>(data[offset++] % (2 * input_rank));
                if (axis_val >= input_rank) {
                    axis_val = axis_val - 2 * input_rank;
                }
        ...
        auto input_placeholder = tensorflow::ops::Placeholder(root.WithOpName("input"), input_dtype);
        auto axis_placeholder = tensorflow::ops::Placeholder(root.WithOpName("axis"), tensorflow::DT_INT32);
        
        auto sum_op = tensorflow::ops::Sum(root.WithOpName("sum"), input_placeholder, axis_placeholder, 
        ...
        tensorflow::Status status = session->Run(inputs, {"sum"}, {}, &outputs);
