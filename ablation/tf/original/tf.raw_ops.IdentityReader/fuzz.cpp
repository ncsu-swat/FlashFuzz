...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (offset < size) {
            size_t container_len = data[offset] % 16;
            offset++;
        ...
        if (offset < size) {
            size_t shared_name_len = data[offset] % 16;
            offset++;
        ...
        auto identity_reader = tensorflow::ops::IdentityReader(root.WithOpName("identity_reader"),
                                                              tensorflow::ops::IdentityReader::Container(container)
                                                              .SharedName(shared_name));

        std::cout << "IdentityReader operation created successfully" << std::endl;
        ...
        if (status.ok()) {
            std::cout << "IdentityReader executed successfully" << std::endl;
            if (!outputs.empty()) {
            ...
            std::cout << "IdentityReader execution failed: " << status.ToString() << std::endl;
        }
