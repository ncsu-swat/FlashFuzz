...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        tensorflow::Tensor axis_tensor(axis_dtype, tensorflow::TensorShape({}));
        ...
        if (axis_dtype == tensorflow::DT_INT32) {
            ...
            if (x_rank > 0) {
                axis_val = axis_val % static_cast<int32_t>(x_rank);
                if (axis_val < 0) {
                    axis_val += static_cast<int32_t>(x_rank);
                }
            } else {
                axis_val = 0;
            }
            ...
            if (offset + sizeof(int64_t) <= size) {
                std::memcpy(&axis_val, data + offset, sizeof(int64_t));
                offset += sizeof(int64_t);
            }
            if (x_rank > 0) {
                axis_val = axis_val % static_cast<int64_t>(x_rank);
                if (axis_val < 0) {
                    axis_val += static_cast<int64_t>(x_rank);
                }
            } else {
                axis_val = 0;
            }
            axis_tensor.scalar<int64_t>()() = axis_val;
        }
        ...
        std::cout << "Axis dtype: " << tensorflow::DataTypeString(axis_dtype) << std::endl;
        ...
        auto cumsum_op = tensorflow::ops::Cumsum(root, x_placeholder, axis_placeholder,
                                               tensorflow::ops::Cumsum::Exclusive(exclusive).Reverse(reverse));
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({{x_placeholder, x_tensor}, {axis_placeholder, axis_tensor}}, 
                                               {cumsum_op}, &outputs);
