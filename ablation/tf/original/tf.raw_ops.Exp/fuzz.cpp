...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto exp_op = tensorflow::ops::Exp(root, x);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({{x, input_tensor}}, {exp_op}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "Exp operation completed successfully" << std::endl;
            std::cout << "Output tensor shape: " << outputs[0].shape().DebugString() << std::endl;
        } else {
            std::cout << "Exp operation failed: " << status.ToString() << std::endl;
        }
