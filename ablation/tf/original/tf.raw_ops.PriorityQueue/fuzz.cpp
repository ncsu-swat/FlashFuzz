...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        uint8_t num_shapes = data[offset] % 5;
        offset++;
        ...
        auto priority_queue_op = tensorflow::ops::PriorityQueue(
            root.WithOpName("priority_queue"),
            shapes,
            tensorflow::ops::PriorityQueue::ComponentTypes(component_types)
                .Capacity(capacity)
                .Container(container)
                .SharedName(shared_name)
        );

        std::cout << "PriorityQueue created with:" << std::endl;
        ...
        tensorflow::Status status = session.Run({priority_queue_op}, &outputs);
        
        if (!status.ok()) {
            std::cout << "Session run failed: " << status.ToString() << std::endl;
        } else {
            std::cout << "PriorityQueue operation executed successfully" << std::endl;
            if (!outputs.empty()) {
