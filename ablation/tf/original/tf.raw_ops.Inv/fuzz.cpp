...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto inv_op = tensorflow::ops::Inv(root, x_placeholder);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({{x_placeholder, input_tensor}}, {inv_op}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "Inv operation completed successfully" << std::endl;
            std::cout << "Output tensor shape: " << outputs[0].shape().DebugString() << std::endl;
        } else {
            std::cout << "Inv operation failed: " << status.ToString() << std::endl;
        }
