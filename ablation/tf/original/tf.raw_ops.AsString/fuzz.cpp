...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        if (offset < size) {
            precision = static_cast<int32_t>(data[offset++] % 10) - 1;
        }
        if (offset < size) {
        ...
        if (offset < size) {
            width = static_cast<int32_t>(data[offset++] % 20) - 1;
        }
        if (offset < size) {
            char fill_char = static_cast<char>(data[offset++] % 128);
            if (fill_char >= 32 && fill_char <= 126) {
                fill = std::string(1, fill_char);
            }
        }
        
        std::cout << "Parameters - precision: " << precision 
        ...
        (*as_string_node->mutable_attr())["T"].set_type(dtype);
        ...
        (*as_string_node->mutable_attr())["fill"].set_s(fill);
        ...
        status = session->Run(inputs, {"as_string"}, {}, &outputs);
        ...
        if (status.ok() && !outputs.empty()) {
            ...
            for (int i = 0; i < std::min(5, static_cast<int>(output_flat.size())); ++i) {
