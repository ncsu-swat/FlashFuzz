...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        std::cout << "Alpha tensor: " << alpha_tensor.DebugString() << std::endl;
        std::cout << "Seed: " << seed << ", Seed2: " << seed2 << std::endl;
        
        tensorflow::Scope root = tensorflow::Scope::NewRootScope();
        
        auto shape_placeholder = tensorflow::ops::Placeholder(root.WithOpName("shape"), shape_dtype);
        auto alpha_placeholder = tensorflow::ops::Placeholder(root.WithOpName("alpha"), alpha_dtype);
        
        auto random_gamma = tensorflow::ops::RandomGamma(root.WithOpName("random_gamma"), 
                                                        shape_placeholder, 
                                                        alpha_placeholder,
        ...
        tensorflow::Status status = session.Run({{shape_placeholder, shape_tensor}, 
                                                 {alpha_placeholder, alpha_tensor}}, 
                                                {random_gamma}, &outputs);
        
        if (status.ok() && !outputs.empty()) {
            std::cout << "RandomGamma output: " << outputs[0].DebugString() << std::endl;
        } else {
            std::cout << "RandomGamma failed: " << status.ToString() << std::endl;
        }
