...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        auto floor_mod = tensorflow::ops::FloorMod(root.WithOpName("floor_mod"), x_placeholder, y_placeholder);
        ...
        if (!status.ok()) {
            std::cout << "FloorMod operation failed: " << status.ToString() << std::endl;
            return 0;
        }
