...
tensorflow::DataType parseIndicesType(uint8_t selector) {
    tensorflow::DataType dtype;
    switch (selector % 2) {
        case 0:
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        tensorflow::DataType indices_dtype = parseIndicesType(data[offset++]);
        tensorflow::DataType segment_ids_dtype = parseIndicesType(data[offset++]);
        ...
        uint8_t segment_ids_size_byte = data[offset++];
        int64_t segment_ids_size = 1 + (segment_ids_size_byte % 10);
        ...
        tensorflow::TensorShape segment_ids_shape({segment_ids_size});
        ...
        if (indices_dtype == tensorflow::DT_INT32) {
            ...
            for (int i = 0; i < indices_flat.size(); ++i) {
                indices_flat(i) = std::abs(indices_flat(i)) % static_cast<int32_t>(data_tensor_shape.dim_size(0));
            }
        } else {
            auto indices_flat = indices_tensor.flat<int64_t>();
            for (int i = 0; i < indices_flat.size(); ++i) {
                indices_flat(i) = std::abs(indices_flat(i)) % data_tensor_shape.dim_size(0);
            }
        }
        ...
        if (segment_ids_dtype == tensorflow::DT_INT32) {
            ...
            for (int i = 0; i < segment_ids_flat.size(); ++i) {
                segment_ids_flat(i) = std::abs(segment_ids_flat(i)) % 5;
            }
        } else {
        ...
        std::cout << "Indices tensor shape: " << indices_tensor.shape().DebugString() << std::endl;
        std::cout << "Segment IDs tensor shape: " << segment_ids_tensor.shape().DebugString() << std::endl;
