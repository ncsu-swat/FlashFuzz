...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        int64_t first_dim = data_tensor_shape.dim_size(0);
        ...
        if (segment_ids_dtype == tensorflow::DT_INT32) {
            ...
            for (int64_t i = 0; i < first_dim; ++i) {
                if (offset < size) {
                    int32_t seg_id = static_cast<int32_t>(data[offset++] % 5);
                    flat(i) = seg_id;
                } else {
            ...
            for (int64_t i = 0; i < first_dim; ++i) {
                if (offset < size) {
                    int64_t seg_id = static_cast<int64_t>(data[offset++] % 5);
                    flat(i) = seg_id;
                } else {
        ...
        auto segment_ids_placeholder = tensorflow::ops::Placeholder(root.WithOpName("segment_ids"), segment_ids_dtype);
        
        auto segment_prod = tensorflow::ops::SegmentProd(root.WithOpName("segment_prod"), 
                                                        data_placeholder, segment_ids_placeholder);
