...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        tensorflow::Tensor accum_init_tensor(lr_dtype, var_tensor_shape);
        ...
        fillTensorWithDataByType(accum_init_tensor, lr_dtype, data, offset, size);
        ...
        std::cout << "var_init_tensor shape: ";
        for (int i = 0; i < var_init_tensor.shape().dims(); ++i) {
            std::cout << var_init_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
        ...
        auto accum_init = tensorflow::ops::AssignVariableOp(root, accum, accum_init_tensor);
        ...
        tensorflow::ops::ResourceSparseApplyMomentum::Attrs attrs;
        attrs = attrs.UseLocking(use_locking);
        attrs = attrs.UseNesterov(use_nesterov);

        auto apply_momentum = tensorflow::ops::ResourceSparseApplyMomentum(
            root, var, accum, lr_const, grad_const, indices_const, momentum_const, attrs);

        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({var_init, accum_init}, &outputs);
        if (!status.ok()) {
            std::cout << "Initialization failed: " << status.ToString() << std::endl;
            return 0;
        }

        status = session.Run({apply_momentum}, &outputs);
        if (!status.ok()) {
            std::cout << "ResourceSparseApplyMomentum failed: " << status.ToString() << std::endl;
            return 0;
        }

        std::cout << "ResourceSparseApplyMomentum operation completed successfully" << std::endl;
