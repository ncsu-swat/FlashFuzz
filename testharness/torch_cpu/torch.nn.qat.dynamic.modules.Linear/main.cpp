#include "fuzzer_utils.h" // General fuzzing utilities
#include <iostream>       // For cerr
#include <tuple>          // For std::get with lu_unpack result
#include <torch/torch.h>

// --- Fuzzer Entry Point ---
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
{
    std::cout << "Start Fuzzing" << std::endl;
    try
    {
        size_t offset = 0;
        
        // Need at least a few bytes to create meaningful input
        if (Size < 4) {
            return 0;
        }
        
        // Create input tensor
        torch::Tensor input_tensor = fuzzer_utils::createTensor(Data, Size, offset);
        
        // Extract parameters for Linear module
        int64_t in_features = 0;
        int64_t out_features = 0;
        bool bias = true;
        
        // Determine in_features from input tensor
        if (input_tensor.dim() >= 1) {
            in_features = input_tensor.size(-1);
        } else {
            // For scalar tensors, use a default value
            in_features = 4;
        }
        
        // Extract out_features from remaining data
        if (offset + sizeof(int64_t) <= Size) {
            int64_t raw_out_features;
            std::memcpy(&raw_out_features, Data + offset, sizeof(int64_t));
            offset += sizeof(int64_t);
            
            // Ensure out_features is positive but not too large
            out_features = std::abs(raw_out_features) % 32 + 1;
        } else {
            out_features = 4; // Default value
        }
        
        // Extract bias parameter
        if (offset < Size) {
            bias = Data[offset++] & 0x1; // Use lowest bit to determine bias
        }
        
        // Create regular linear module (QAT dynamic linear is not available in standard PyTorch C++ API)
        torch::nn::Linear linear_module(torch::nn::LinearOptions(in_features, out_features).bias(bias));
        
        // Reshape input tensor if needed to match expected input shape
        if (input_tensor.dim() == 0) {
            // For scalar tensors, reshape to [1, in_features]
            input_tensor = input_tensor.reshape({1, in_features});
        } else if (input_tensor.dim() == 1) {
            // For 1D tensors, ensure last dimension is in_features
            if (input_tensor.size(0) != in_features) {
                input_tensor = input_tensor.reshape({1, in_features});
            } else {
                input_tensor = input_tensor.reshape({1, in_features});
            }
        } else {
            // For higher dimensional tensors, ensure last dimension is in_features
            auto sizes = input_tensor.sizes().vec();
            if (sizes.back() != in_features) {
                sizes.back() = in_features;
                input_tensor = input_tensor.reshape(sizes);
            }
        }
        
        // Apply the linear module
        torch::Tensor output = linear_module(input_tensor);
        
        // Verify output shape
        auto output_sizes = output.sizes();
        auto input_sizes = input_tensor.sizes();
        
        // Last dimension of output should be out_features
        if (output_sizes.back() != out_features) {
            throw std::runtime_error("Output tensor has incorrect last dimension");
        }
        
        // All other dimensions should match input
        for (int64_t i = 0; i < input_sizes.size() - 1; i++) {
            if (output_sizes[i] != input_sizes[i]) {
                throw std::runtime_error("Output tensor has incorrect batch dimensions");
            }
        }
        
        // Test with different input types if possible
        if (offset + 1 < Size) {
            torch::ScalarType dtype = fuzzer_utils::parseDataType(Data[offset++]);
            try {
                torch::Tensor converted_input = input_tensor.to(dtype);
                torch::Tensor converted_output = linear_module(converted_input);
            } catch (const std::exception& e) {
                // Silently catch exceptions from type conversion
            }
        }
        
        // Test with zero-sized batch dimension
        try {
            std::vector<int64_t> zero_batch_shape = input_tensor.sizes().vec();
            if (zero_batch_shape.size() > 1) {
                zero_batch_shape[0] = 0;
                torch::Tensor zero_batch_input = torch::empty(zero_batch_shape, input_tensor.options());
                torch::Tensor zero_batch_output = linear_module(zero_batch_input);
            }
        } catch (const std::exception& e) {
            // Silently catch exceptions from zero batch
        }
    }
    catch (const std::exception &e)
    {
        std::cerr << "Exception caught: " << e.what() << std::endl;
        return -1; // discard the input
    }
    return 0; // keep the input
}
