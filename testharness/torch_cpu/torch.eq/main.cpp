...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
{
    ...
    try
    {
        ...
        if (Size > offset) {
            ...
            torch::Tensor result2 = torch::eq(tensor1, static_cast<int64_t>(scalar_value));
        }
        ...
        try {
            ...
            // This might throw due to shape mismatch, which is fine
        }
        ...
        if (Size > offset) {
            uint8_t scalar_value = Data[offset % Size];
            torch::Tensor result9 = tensor1.eq(static_cast<int64_t>(scalar_value));
        }
        
        // 9. Test with double scalar
        if (offset + sizeof(double) <= Size) {
            double dval;
            memcpy(&dval, Data + (offset % (Size - sizeof(double) + 1)), sizeof(double));
            if (std::isfinite(dval)) {
                torch::Tensor result10 = torch::eq(tensor1, dval);
            }
        }
    }
