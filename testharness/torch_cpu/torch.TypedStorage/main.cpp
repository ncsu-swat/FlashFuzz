...
// Target API: torch.TypedStorage (via c10::Storage in C++)
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
{
    ...
    try
    {
        ...
        // Get the tensor's storage (TypedStorage equivalent in C++)
        ...
        if (element_size > 0 && storage_bytes >= element_size) {
            try {
                ...
                // from_blob may fail for certain dtypes, ignore
            }
        }
        ...
        if (offset < Size) {
            ...
            try {
                ...
                volatile double check = fresh_view.to(torch::kFloat).sum().item<double>();
                (void)check;
            } catch (...) {
                // from_blob may fail, ignore
            }
        }

        // Test storage resizing if supported
        try {
            c10::Storage resizable_storage(
                c10::Storage::use_byte_size_t{}, 64, 
                c10::GetAllocator(c10::DeviceType::CPU), /*resizable=*/true);
            // Access to verify it's valid
            volatile void* ptr = resizable_storage.mutable_data();
            (void)ptr;
        } catch (...) {
            // Resize may not be supported, ignore
        }
        ...
        if (torch::cuda::is_available()) {
            try {
                ...
                // CUDA operations may fail, ignore
            }
        }
        #endif
    }
