...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
{
    ...
    try
    {
        ...
        if (offset + 4 <= Size) {
            ...
            // Parse dropout_p as a double between 0 and 1
            ...
            // Parse scale as a double
            if (offset + sizeof(float) <= Size) {
                float raw_scale;
                std::memcpy(&raw_scale, Data + offset, sizeof(float));
                offset += sizeof(float);
                scale = static_cast<double>(raw_scale);
            }
        }
        
        // Call the can_use_efficient_attention function
        bool result = torch::can_use_efficient_attention(
            query, key, value, c10::nullopt, need_weights, dropout_p, is_causal, scale);
