...
tensorflow::DataType parseUpdatesDataType(uint8_t selector) {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        tensorflow::DataType updates_dtype = parseUpdatesDataType(data[offset++]);
        ...
        auto var = tensorflow::ops::VarHandleOp(root, updates_dtype, ref_tensor_shape);
        
        tensorflow::Tensor init_tensor(updates_dtype, ref_tensor_shape);
        fillTensorWithDataByType(init_tensor, updates_dtype, data, offset, size);
        auto init_op = tensorflow::ops::AssignVariableOp(root, var, init_tensor);
        ...
        tensorflow::Tensor updates_tensor(updates_dtype, updates_tensor_shape);
        ...
        auto scatter_min_op = tensorflow::ops::ResourceScatterMin(root, var, indices_input, updates_input);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Operation> ops_to_run = {init_op};
        tensorflow::Status init_status = session.Run(tensorflow::ClientSession::FeedType(), std::vector<tensorflow::Output>(), ops_to_run, nullptr);
        if (!init_status.ok()) {
            return -1;
        }
        
        ops_to_run = {scatter_min_op.operation};
        tensorflow::Status status = session.Run(tensorflow::ClientSession::FeedType(), std::vector<tensorflow::Output>(), ops_to_run, nullptr);
        if (!status.ok()) {
            return -1;
        }
