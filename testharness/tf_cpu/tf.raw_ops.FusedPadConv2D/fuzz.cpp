...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (int i = 0; i < 8; ++i) {
            if (offset < size) {
                paddings_flat(i) = static_cast<int32_t>(data[offset++] % 3);
            } else {
                paddings_flat(i) = 0;
            }
        }
        ...
        std::string mode = parseMode(data[offset % size]);
        offset++;
        ...
        auto fused_pad_conv2d = tensorflow::ops::FusedPadConv2D(
            root, input_op, paddings_op, filter_op, mode, strides, padding);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({fused_pad_conv2d}, &outputs);
        if (!status.ok()) {
            return -1;
        }
