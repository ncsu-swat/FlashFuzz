...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        fillTensorWithData<int64_t>(key_tensor, data, offset, size);
        ...
        uint8_t num_dtypes = (offset < size) ? (data[offset++] % 5 + 1) : 1;
        ...
        int capacity = (offset < size) ? static_cast<int>(data[offset++] % 100) : 0;
        int memory_limit = (offset < size) ? static_cast<int>(data[offset++] % 1000) : 0;
        ...
        auto unstage_op = tensorflow::ops::OrderedMapUnstage(
            root, key_input, indices_input, dtypes,
            tensorflow::ops::OrderedMapUnstage::Capacity(capacity)
        ...
        for (int i = 0; i < key_tensor.shape().dims(); ++i) {
            std::cout << key_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
