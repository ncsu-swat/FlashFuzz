...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto driver_flat = driver_name_tensor.scalar<tensorflow::tstring>();
        driver_flat() = tensorflow::tstring("sqlite");

        tensorflow::Tensor data_source_name_tensor(tensorflow::DT_STRING, tensorflow::TensorShape({}));
        auto data_source_flat = data_source_name_tensor.scalar<tensorflow::tstring>();
        data_source_flat() = tensorflow::tstring(":memory:");

        tensorflow::Tensor query_tensor(tensorflow::DT_STRING, tensorflow::TensorShape({}));
        auto query_flat = query_tensor.scalar<tensorflow::tstring>();
        query_flat() = tensorflow::tstring("SELECT 1 as col1, 'test' as col2");
        ...
        if (output_types.empty()) {
            output_types.push_back(tensorflow::DT_INT32);
