...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::TensorShape row_shape_tensor_shape;
        row_shape_tensor_shape.AddDim(row_shape_dims.size());
        ...
        output_shapes.push_back(tensorflow::PartialTensorShape({-1, -1}));
        output_shapes.push_back(tensorflow::PartialTensorShape({-1, static_cast<int64_t>(row_shape_dims.size())}));
        output_shapes.push_back(tensorflow::PartialTensorShape({static_cast<int64_t>(row_shape_dims.size())}));
        ...
        auto row_shape_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);
        
        // Use raw_ops namespace for ExperimentalDenseToSparseBatchDataset
        auto experimental_dense_to_sparse_batch_dataset = tensorflow::ops::_ExperimentalDenseToSparseBatchDataset(
            root,
            input_dataset_placeholder,
            batch_size_placeholder,
            row_shape_placeholder,
            output_types,
            output_shapes
        );
        ...
        std::vector<std::pair<std::string, tensorflow::Tensor>> feed_dict = {
            ...
            {row_shape_placeholder.node()->name(), row_shape_tensor}
        };
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run(feed_dict, {experimental_dense_to_sparse_batch_dataset}, {}, &outputs);
