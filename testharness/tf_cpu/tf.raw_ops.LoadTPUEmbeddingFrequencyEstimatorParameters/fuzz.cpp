...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor last_hit_step_tensor(tensorflow::DT_FLOAT, tensor_shape2);
        fillTensorWithDataByType(last_hit_step_tensor, tensorflow::DT_FLOAT, data, offset, size);
        ...
        if (offset < size) {
            size_t name_len = data[offset++] % 10;
        ...
        std::string config = "";
        if (offset < size) {
            size_t config_len = data[offset++] % 10;
        ...
        auto last_hit_step_input = tensorflow::ops::Const(root, last_hit_step_tensor);

        // Use raw_ops API instead of the missing tpu_ops.h
        tensorflow::NodeDef node_def;
        node_def.set_op("LoadTPUEmbeddingFrequencyEstimatorParameters");
        node_def.set_name("LoadTPUEmbeddingFrequencyEstimatorParameters");
        
        // Add inputs
        tensorflow::NodeDefBuilder::NodeOut parameters_node(parameters_input.node()->name(), 0, tensorflow::DT_FLOAT);
        tensorflow::NodeDefBuilder::NodeOut last_hit_step_node(last_hit_step_input.node()->name(), 0, tensorflow::DT_FLOAT);
        
        tensorflow::NodeDefBuilder builder("LoadTPUEmbeddingFrequencyEstimatorParameters", "LoadTPUEmbeddingFrequencyEstimatorParameters");
        builder.Input(parameters_node);
        builder.Input(last_hit_step_node);
        ...
        auto load_op = root.AddNode(node_def);

        tensorflow::ClientSession session(root);
        status = session.Run({load_op}, nullptr);
        if (!status.ok()) {
            return -1;
        }
