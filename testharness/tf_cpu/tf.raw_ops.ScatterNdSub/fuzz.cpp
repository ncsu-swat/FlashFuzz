...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto assign_ref = tensorflow::ops::Assign(root, ref_var, tensorflow::Input(ref_tensor));
        
        bool use_locking = (data[offset % size] % 2) == 0;
        
        auto scatter_nd_sub = tensorflow::ops::ScatterNdSub(
            root, ref_var, tensorflow::Input(indices_tensor), tensorflow::Input(updates_tensor),
            tensorflow::ops::ScatterNdSub::UseLocking(use_locking)
        );
        ...
        tensorflow::Status status = session.Run({scatter_nd_sub}, &outputs);
        if (!status.ok()) {
            return -1;
        }
