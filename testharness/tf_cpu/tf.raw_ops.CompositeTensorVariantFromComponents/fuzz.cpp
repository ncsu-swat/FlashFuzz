...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (components.empty()) {
            tensorflow::Tensor dummy_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({}));
            dummy_tensor.scalar<float>()() = 1.0f;
        ...
        auto composite_op = tensorflow::ops::RawOp(
            root.WithOpName("CompositeTensorVariantFromComponents"),
            "CompositeTensorVariantFromComponents",
            components,
            tensorflow::ops::RawOp::Attrs().SetAttr("metadata", metadata));

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({composite_op}, &outputs);
