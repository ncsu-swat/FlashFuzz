    ...
    try {
        ...
        if (offset < size) {
            ...
            for (uint8_t i = 0; i < name_len && offset < size; ++i) {
                tensor_name += static_cast<char>((data[offset++] % 26) + 'a');
            }
        }
        
        std::vector<std::string> debug_ops_spec;
        if (offset < size) {
            uint8_t num_debug_ops = data[offset++] % 3;
            for (uint8_t i = 0; i < num_debug_ops && offset < size; ++i) {
                std::string debug_op = "DebugIdentity;grpc://localhost:";
                debug_op += std::to_string(3000 + (data[offset++] % 1000));
                debug_op += ";";
                debug_op += std::to_string(data[offset++] % 2);
                debug_ops_spec.push_back(debug_op);
            }
        }
        
        // Use raw_ops::CopyHost instead of ops::CopyHost
        auto copy_host_op = tensorflow::ops::internal::CopyHost(
            root, 
            input_placeholder,
            tensor_name,
            debug_ops_spec
