...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            int8_t raw_axis;
            std::memcpy(&raw_axis, data + offset, sizeof(int8_t));
            offset += sizeof(int8_t);
            
            int max_axis = static_cast<int>(input_rank);
            int min_axis = -(max_axis + 1);
            
            if (raw_axis >= 0) {
                axis_value = raw_axis % (max_axis + 1);
            } else {
                axis_value = min_axis + ((-raw_axis - 1) % (max_axis + 1));
            }
        }
        ...
        auto expand_dims_op = tensorflow::ops::ExpandDims(root, input_op, axis_op);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({expand_dims_op}, &outputs);
