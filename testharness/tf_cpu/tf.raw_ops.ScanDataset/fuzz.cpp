...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t num_other_args = (data[offset++] % 3) + 1;
        uint8_t num_output_types = (data[offset++] % 3) + 1;
        
        std::vector<tensorflow::Tensor> initial_state_tensors;
        ...
        for (uint8_t i = 0; i < num_initial_state; ++i) {
            ...
            initial_state_tensors.push_back(tensor);
        }
        ...
        for (uint8_t i = 0; i < num_other_args; ++i) {
            ...
            tensorflow::Tensor tensor(dtype, tensor_shape);
            fillTensorWithDataByType(tensor, dtype, data, offset, size);
            other_arg_tensors.push_back(tensor);
        }
        ...
        for (uint8_t i = 0; i < num_output_types; ++i) {
            ...
            tensorflow::PartialTensorShape::MakePartialShape(dims.data(), dims.size(), &tensor_shape);
            output_shapes.push_back(tensor_shape);
        }
        
        if (initial_state_tensors.empty() || output_types.empty() || output_shapes.empty()) {
            return 0;
        }
        
        bool preserve_cardinality = (offset < size) ? (data[offset++] % 2 == 0) : false;
        bool use_default_device = (offset < size) ? (data[offset++] % 2 == 0) : true;
        std::string metadata = "";
        ...
        // Convert tensors to outputs
        std::vector<tensorflow::Output> initial_state_outputs;
        for (const auto& tensor : initial_state_tensors) {
            auto placeholder = tensorflow::ops::Placeholder(root, tensor.dtype());
            initial_state_outputs.push_back(placeholder);
        }
        
        std::vector<tensorflow::Output> other_args_outputs;
        for (const auto& tensor : other_arg_tensors) {
            auto placeholder = tensorflow::ops::Placeholder(root, tensor.dtype());
            other_args_outputs.push_back(placeholder);
        }
        
        // Create ScanDataset operation
        auto scan_dataset = tensorflow::ops::ScanDataset(
            root,
            input_dataset,
            initial_state_outputs,
            other_args_outputs,
            f_attr,
            output_types,
            output_shapes,
