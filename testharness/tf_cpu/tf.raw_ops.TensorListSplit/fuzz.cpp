...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::TensorShape element_shape_tf;
        for (int64_t dim : element_shape_dims) {
            element_shape_tf.AddDim(dim);
        }
        
        tensorflow::Tensor element_shape_tensor(tensorflow::DT_INT64, tensorflow::TensorShape({static_cast<int64_t>(element_shape_dims.size())}));
        ...
        uint8_t num_lengths = (offset < size) ? (data[offset++] % 5 + 1) : 1;
        tensorflow::Tensor lengths_tensor(tensorflow::DT_INT64, tensorflow::TensorShape({num_lengths}));
        ...
        int64_t sum_lengths = 0;
        for (int i = 0; i < num_lengths; ++i) {
            int64_t length = (offset < size) ? (data[offset++] % 5 + 1) : 1;
            lengths_flat(i) = length;
            sum_lengths += length;
        }
        
        if (sum_lengths == 0 || total_elements % sum_lengths != 0) {
            return 0;
        }
        
        auto input_placeholder = tensorflow::ops::Placeholder(root, tensor_dtype);
        auto element_shape_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);
        auto lengths_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);
        
        auto tensor_list_split = tensorflow::ops::TensorListSplit(
            root, 
            input_placeholder, 
            element_shape_placeholder, 
            lengths_placeholder
        );
        ...
        tensorflow::Status status = session.Run(
            {{input_placeholder, input_tensor}, 
             {element_shape_placeholder, element_shape_tensor}, 
             {lengths_placeholder, lengths_tensor}},
            {tensor_list_split}, 
            &outputs
        );
