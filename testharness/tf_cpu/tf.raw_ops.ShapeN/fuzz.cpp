...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t num_tensors = (num_tensors_byte % (MAX_NUM_TENSORS - MIN_NUM_TENSORS + 1)) + MIN_NUM_TENSORS;
        
        uint8_t out_type_byte = data[offset++];
        tensorflow::DataType out_type = (out_type_byte % 2 == 0) ? tensorflow::DT_INT32 : tensorflow::DT_INT64;
        ...
        auto shape_n_op = tensorflow::ops::ShapeN(root, input_tensors);
        shape_n_op = tensorflow::ops::ShapeN(root.WithOpName("ShapeN"), input_tensors, 
            tensorflow::ops::ShapeN::Attrs().OutType(out_type));
        
        tensorflow::ClientSession session(root);
        
        std::vector<std::pair<std::string, tensorflow::Tensor>> feed_dict;
        for (size_t i = 0; i < input_tensors.size(); ++i) {
            tensorflow::DataType dtype = parseDataType(data[2 + i * 2]);
            uint8_t rank = parseRank(data[3 + i * 2]);
            std::vector<int64_t> shape = parseShape(data, offset, size, rank);
        ...
        tensorflow::Status status = session.Run(feed_dict, {shape_n_op.output}, {}, &outputs);
