...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto retrieve_op = tensorflow::ops::Raw::RetrieveTPUEmbeddingFrequencyEstimatorParameters(
            root.WithOpName("RetrieveTPUEmbeddingFrequencyEstimatorParameters"),
            num_shards_tensor,
            shard_id_tensor,
            op_attrs);
        ...
        tensorflow::Status status = session.Run({retrieve_op.parameters, retrieve_op.last_hit_step}, &outputs);
        if (!status.ok()) {
        ...
        if (outputs.size() >= 2) {
            ...
            std::cout << "Last hit step tensor shape: ";
