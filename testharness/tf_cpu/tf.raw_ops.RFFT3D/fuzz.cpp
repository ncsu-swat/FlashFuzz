...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::vector<int32_t> fft_length_data = {
            static_cast<int32_t>(shape[rank-3] > 0 ? shape[rank-3] : 1),
            static_cast<int32_t>(shape[rank-2] > 0 ? shape[rank-2] : 1), 
            static_cast<int32_t>(shape[rank-1] > 0 ? shape[rank-1] : 1)
        };
        ...
        // Use raw_ops.RFFT3D instead of ops::RFFT3D
        auto rfft3d_op = tensorflow::ops::internal::RFFT3D(root, input_placeholder, fft_length_placeholder);
