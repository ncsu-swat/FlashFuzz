...
std::string parseAlign(uint8_t selector) {
    switch (selector % 4) {
        case 0:
            return "RIGHT_LEFT";
        case 1:
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::TensorShape k_shape({2});
        tensorflow::Tensor k_tensor(tensorflow::DT_INT32, k_shape);
        auto k_flat = k_tensor.flat<int32_t>();
        k_flat(0) = k_low;
        k_flat(1) = k_high;
        
        tensorflow::Tensor padding_value_tensor(input_dtype, tensorflow::TensorShape({}));
        
        switch (input_dtype) {
            case tensorflow::DT_FLOAT:
                padding_value_tensor.scalar<float>()() = 0.0f;
                break;
            case tensorflow::DT_DOUBLE:
                padding_value_tensor.scalar<double>()() = 0.0;
                break;
            case tensorflow::DT_INT32:
                padding_value_tensor.scalar<int32_t>()() = 0;
                break;
            case tensorflow::DT_INT64:
                padding_value_tensor.scalar<int64_t>()() = 0;
                break;
            case tensorflow::DT_BOOL:
                padding_value_tensor.scalar<bool>()() = false;
                break;
            default:
                padding_value_tensor.scalar<float>()() = 0.0f;
                break;
        ...
        std::string align = parseAlign(data[offset % size]);
        
        auto input_op = tensorflow::ops::Const(root, input_tensor);
        auto k_op = tensorflow::ops::Const(root, k_tensor);
        auto padding_value_op = tensorflow::ops::Const(root, padding_value_tensor);
        
        auto matrix_diag_part_v3 = tensorflow::ops::MatrixDiagPartV3(
            root, input_op, k_op, padding_value_op,
            tensorflow::ops::MatrixDiagPartV3::Align(align)
        );
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({matrix_diag_part_v3}, &outputs);
        if (!status.ok()) {
            return -1;
        }
