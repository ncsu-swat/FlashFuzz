...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        tensorflow::Output writer_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_RESOURCE);
        ...
        tensorflow::Output step_input = tensorflow::ops::Const(root, step_tensor);

        uint8_t tensor_rank = parseRank(data[offset++]);
        std::vector<int64_t> tensor_shape = parseShape(data, offset, size, tensor_rank);
        tensorflow::Tensor tensor_tensor(tensorflow::DT_STRING, tensorflow::TensorShape(tensor_shape));
        fillTensorWithDataByType(tensor_tensor, tensorflow::DT_STRING, data, offset, size);
        tensorflow::Output tensor_input = tensorflow::ops::Const(root, tensor_tensor);

        // Use raw op directly since summary_ops.h is not available
        auto write_graph_summary = tensorflow::ops::Operation(
            root.WithOpName("WriteGraphSummary"),
            "WriteGraphSummary",
            {writer_placeholder, step_input, tensor_input});
