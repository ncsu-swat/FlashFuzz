...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        tensorflow::DataType main_dtype = parseDataType(data[offset++]);
        ...
        std::vector<int64_t> reserve_space_shape = {channel_dim};
        
        tensorflow::Tensor y_backprop_tensor(main_dtype, tensorflow::TensorShape(y_backprop_shape));
        tensorflow::Tensor x_tensor(main_dtype, tensorflow::TensorShape(x_shape));
        tensorflow::Tensor scale_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape(scale_shape));
        tensorflow::Tensor reserve_space_1_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape(reserve_space_shape));
        tensorflow::Tensor reserve_space_2_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape(reserve_space_shape));
        
        fillTensorWithDataByType(y_backprop_tensor, main_dtype, data, offset, size);
        fillTensorWithDataByType(x_tensor, main_dtype, data, offset, size);
        ...
        if (offset < size) {
            float eps_val;
            std::memcpy(&eps_val, data + offset, std::min(sizeof(float), size - offset));
            epsilon = std::abs(eps_val);
            if (epsilon == 0.0f) epsilon = 0.0001f;
            offset += sizeof(float);
        }
        
        std::string data_format = (data[offset % size] % 2 == 0) ? "NHWC" : "NCHW";
        offset++;
        
        bool is_training = (data[offset % size] % 2 == 0);
        offset++;
        
        auto fused_batch_norm_grad = tensorflow::ops::FusedBatchNormGradV2(
            root,
            y_backprop_op,
            x_op,
            scale_op,
            reserve_space_1_op,
            reserve_space_2_op,
            tensorflow::ops::FusedBatchNormGradV2::Epsilon(epsilon)
