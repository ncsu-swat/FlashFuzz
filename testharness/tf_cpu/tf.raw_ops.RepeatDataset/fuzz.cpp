...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (count_value < -1) count_value = -1;
        if (count_value > 1000) count_value = 1000;
        ...
        auto repeat_dataset_op = tensorflow::ops::internal::RepeatDataset(
            root,
            input_dataset,
            count_op,
            output_types,
        ...
        tensorflow::Status status = session.Run({repeat_dataset_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
