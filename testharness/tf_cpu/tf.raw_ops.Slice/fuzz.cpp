...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::vector<int32_t> begin_values;
        std::vector<int32_t> size_values;
        
        for (int i = 0; i < input_rank; ++i) {
            if (offset < size) {
                int32_t begin_val = static_cast<int32_t>(data[offset++] % (input_shape[i] + 1));
                begin_values.push_back(begin_val);
                
                if (offset < size) {
                    int32_t remaining = input_shape[i] - begin_val;
                    int32_t size_val;
                    if (data[offset] % 10 == 0) {
                        size_val = -1;
                    } else {
                        size_val = static_cast<int32_t>((data[offset] % (remaining + 1)) + 1);
                    }
                    size_values.push_back(size_val);
                    offset++;
                } else {
                    size_values.push_back(1);
                }
            } else {
        ...
        if (begin_values.empty()) {
            begin_values.push_back(0);
            size_values.push_back(1);
        }
        
        tensorflow::TensorShape begin_shape({static_cast<int64_t>(begin_values.size())});
        tensorflow::Tensor begin_tensor(tensorflow::DT_INT32, begin_shape);
        auto begin_flat = begin_tensor.flat<int32_t>();
        for (size_t i = 0; i < begin_values.size(); ++i) {
            begin_flat(i) = begin_values[i];
        }
        
        tensorflow::TensorShape size_shape({static_cast<int64_t>(size_values.size())});
        tensorflow::Tensor size_tensor(tensorflow::DT_INT32, size_shape);
        auto size_flat = size_tensor.flat<int32_t>();
        for (size_t i = 0; i < size_values.size(); ++i) {
            size_flat(i) = size_values[i];
        }
        
        auto begin_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_INT32);
        auto size_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_INT32);
        
        auto slice_op = tensorflow::ops::Slice(root, input_placeholder, begin_placeholder, size_placeholder);
