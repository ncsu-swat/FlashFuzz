...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto op = tensorflow::Operation(root.WithOpName("RetrieveTPUEmbeddingProximalYogiParameters")
        ...
        auto parameters = tensorflow::Output(op, 0);
        auto v = tensorflow::Output(op, 1);
        auto m = tensorflow::Output(op, 2);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({parameters, v, m}, &outputs);
        if (!status.ok()) {
        ...
        if (outputs.size() >= 3) {
            ...
            std::cout << "V shape: " << outputs[1].shape().DebugString() << std::endl;
            std::cout << "M shape: " << outputs[2].shape().DebugString() << std::endl;
        }
