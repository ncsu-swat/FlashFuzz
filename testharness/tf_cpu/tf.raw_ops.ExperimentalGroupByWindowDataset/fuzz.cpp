...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t num_window_args = data[offset++] % 3;
        uint8_t num_output_types = data[offset++] % 3 + 1;
        
        std::vector<tensorflow::Input> key_func_other_arguments;
        for (uint8_t i = 0; i < num_key_args; ++i) {
            ...
            key_func_other_arguments.push_back(input);
        }
        
        std::vector<tensorflow::Input> reduce_func_other_arguments;
        for (uint8_t i = 0; i < num_reduce_args; ++i) {
            ...
            auto input = tensorflow::ops::Const(root, tensor);
            reduce_func_other_arguments.push_back(input);
        }
        
        std::vector<tensorflow::Input> window_size_func_other_arguments;
        for (uint8_t i = 0; i < num_window_args; ++i) {
            if (offset >= size) break;
            tensorflow::DataType dtype = parseDataType(data[offset++]);
            if (offset >= size) break;
            uint8_t rank = parseRank(data[offset++]);
            ...
            auto input = tensorflow::ops::Const(root, tensor);
            window_size_func_other_arguments.push_back(input);
        }
        ...
        auto scope = root.WithOpName("ExperimentalGroupByWindowDataset");
        auto experimental_group_by_window_dataset = tensorflow::ops::_GroupByWindowDataset(
            scope,
            input_dataset,
