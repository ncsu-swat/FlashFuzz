...
std::string parsePattern(const uint8_t* data, size_t& offset, size_t total_size) {
    if (offset >= total_size) {
        return ".*";
    }
    
    size_t pattern_len = std::min(static_cast<size_t>(data[offset] % 10 + 1), total_size - offset - 1);
    offset++;
    
    if (offset + pattern_len <= total_size) {
        std::string pattern(reinterpret_cast<const char*>(data + offset), pattern_len);
        offset += pattern_len;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::string pattern = parsePattern(data, offset, size);
        ...
        auto static_regex_op = tensorflow::ops::RegexFullMatch(
            root, 
            input_placeholder, 
            pattern
        );
