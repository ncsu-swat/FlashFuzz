...
#define MAX_OUTPUT_TYPES 5
#define MIN_OUTPUT_TYPES 1
...
std::vector<tensorflow::DataType> parseOutputTypes(const uint8_t* data, size_t& offset, size_t total_size) {
    if (offset >= total_size) {
        return {tensorflow::DT_FLOAT};
    }
    
    uint8_t num_types = (data[offset] % (MAX_OUTPUT_TYPES - MIN_OUTPUT_TYPES + 1)) + MIN_OUTPUT_TYPES;
    offset++;
...
std::vector<tensorflow::TensorShape> parseOutputShapes(const uint8_t* data, size_t& offset, size_t total_size, size_t num_shapes) {
    ...
    for (size_t i = 0; i < num_shapes; ++i) {
        if (offset < total_size) {
            ...
            output_shapes.push_back(scalar_shape);
        }
    }
