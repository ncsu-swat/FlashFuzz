...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::string tag_str = "test_tag";
        if (offset + tag_len <= size) {
            tag_str = std::string(reinterpret_cast<const char*>(data + offset), tag_len);
            offset += tag_len;
        }
        
        tensorflow::Tensor tag_tensor(tensorflow::DT_STRING, tensorflow::TensorShape({}));
        tag_tensor.scalar<tensorflow::tstring>()() = tag_str;
        ...
        auto tag = tensorflow::ops::Placeholder(root.WithOpName("tag"), tensorflow::DT_STRING);
        
        tensorflow::NodeDef node_def;
        node_def.set_name("bytes_produced_stats");
        node_def.set_op("ExperimentalBytesProducedStatsDataset");
        
        tensorflow::AddNodeAttr("output_types", output_types, &node_def);
        tensorflow::AddNodeAttr("output_shapes", output_shapes, &node_def);
        
        tensorflow::Node* node;
        auto status = tensorflow::NodeBuilder("bytes_produced_stats", "ExperimentalBytesProducedStatsDataset")
        ...
        status = session.Run(feed_dict, {result}, &outputs);
