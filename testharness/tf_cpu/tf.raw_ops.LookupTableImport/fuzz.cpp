...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto table_handle = tensorflow::ops::HashTable(root, key_dtype, value_dtype);
        
        auto keys_input = tensorflow::ops::Const(root, key_tensor);
        auto values_input = tensorflow::ops::Const(root, value_tensor);
        
        auto lookup_import = tensorflow::ops::LookupTableImport(root, table_handle, keys_input, values_input);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        std::vector<tensorflow::Output> fetch_outputs = {lookup_import.operation};
