...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        bool validate_shape = (data[offset++] % 2) == 1;
        ...
        auto ref_var = tensorflow::ops::Variable(root, ref_tensor_shape, dtype);
        auto ref_init = tensorflow::ops::Assign(root, ref_var, tensorflow::ops::Const(root, ref_tensor));
        
        auto assign_op = tensorflow::ops::Assign(root.WithOpName("assign_op"), ref_var, 
                                                tensorflow::ops::Const(root, value_tensor),
                                                tensorflow::ops::Assign::ValidateShape(validate_shape)
                                                .UseLocking(use_locking));
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> init_outputs;
        tensorflow::Status init_status = session.Run({ref_init}, &init_outputs);
        if (!init_status.ok()) {
            return -1;
        }
