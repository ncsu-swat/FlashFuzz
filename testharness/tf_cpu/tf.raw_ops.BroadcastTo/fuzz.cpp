...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t target_rank = parseRank(data[offset++]);
        std::vector<int64_t> target_shape = parseShape(data, offset, size, target_rank);
        ...
        tensorflow::TensorShape target_tensor_shape;
        for (int64_t dim : target_shape) {
            target_tensor_shape.AddDim(dim);
        }
        
        tensorflow::Tensor shape_tensor(tensorflow::DT_INT64, tensorflow::TensorShape({static_cast<int64_t>(target_shape.size())}));
        auto shape_flat = shape_tensor.flat<int64_t>();
        for (size_t i = 0; i < target_shape.size(); ++i) {
            shape_flat(i) = target_shape[i];
        }
        ...
        auto broadcast_op = tensorflow::ops::BroadcastTo(root, input_placeholder, shape_placeholder);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}, {shape_placeholder, shape_tensor}}, 
                                               {broadcast_op}, &outputs);
