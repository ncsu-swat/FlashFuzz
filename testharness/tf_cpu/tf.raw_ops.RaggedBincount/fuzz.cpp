...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t splits_rank = 1;
        std::vector<int64_t> splits_shape = {static_cast<int64_t>((data[offset++] % 10) + 2)};
        
        uint8_t values_rank = parseRank(data[offset++]);
        if (values_rank == 0) values_rank = 1;
        ...
        if (weights_rank == 0) {
            ...
            weights_shape = parseShape(data, offset, size, weights_rank);
        }
        ...
        auto splits_flat = splits_tensor.flat<int64_t>();
        for (int i = 0; i < splits_flat.size(); ++i) {
            splits_flat(i) = std::abs(splits_flat(i)) % 100;
            if (i > 0 && splits_flat(i) < splits_flat(i-1)) {
                splits_flat(i) = splits_flat(i-1);
            }
        }
        ...
        if (values_dtype == tensorflow::DT_INT32) {
            ...
            for (int i = 0; i < values_flat.size(); ++i) {
                values_flat(i) = std::abs(values_flat(i)) % 50;
            }
        } else {
        ...
        auto ragged_bincount = tensorflow::ops::RaggedBincount(
            root,
            splits_input,
            values_input,
            size_input,
            weights_input,
            tensorflow::ops::RaggedBincount::BinaryOutput(binary_output)
        );
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({ragged_bincount}, &outputs);
        if (!status.ok()) {
            return -1;
        }
