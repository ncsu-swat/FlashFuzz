...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset >= size) return 0;
        bool drop_remainder_val = data[offset++] % 2;
        ...
        auto num_parallel_calls = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);
        auto drop_remainder = tensorflow::ops::Placeholder(root, tensorflow::DT_BOOL);
        ...
        // Use raw_ops API to create the ExperimentalMapAndBatchDataset
        auto map_and_batch_dataset = tensorflow::ops::internal::MapAndBatchDataset(
            root,
            input_dataset,
            {},  // other_arguments
            batch_size,
        ...
        tensorflow::Status status = session.Run(
            {{input_dataset.node()->name(), input_dataset_tensor},
             ...
             {num_parallel_calls.node()->name(), num_parallel_calls_tensor},
             {drop_remainder.node()->name(), drop_remainder_tensor}},
            {map_and_batch_dataset.node()->name()},
            {},
            &outputs
        );
