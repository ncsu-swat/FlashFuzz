...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Create a frame name for the Exit node
        std::string frame_name = "test_frame";
        
        // Use raw_ops to create the Exit operation
        auto exit_op = tensorflow::ops::internal::Exit(root.WithOpName("exit_op"), 
                                                      input_placeholder,
                                                      frame_name);
        ...
        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}}, {exit_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
