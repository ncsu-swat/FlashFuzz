...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            uint8_t fill_selector = data[offset] % 3;
            if (fill_selector == 1) fill_mode = "REFLECT";
            else if (fill_selector == 2) fill_mode = "WRAP";
        }
        offset++;
        ...
        // Use raw_ops namespace for ImageProjectiveTransformV2
        auto result = tensorflow::ops::ImageProjectiveTransform(
            root,
            images_placeholder,
