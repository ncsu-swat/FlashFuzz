...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::cout << "Creating Variable with:" << std::endl;
        ...
        auto variable_op = tensorflow::ops::Variable(
            root.WithOpName("test_variable"),
            tensor_shape,
            dtype,
            tensorflow::ops::Variable::Container(container).SharedName(shared_name)
        );

        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({variable_op}, &outputs);
        ...
        if (!outputs.empty()) {
            std::cout << "Variable created successfully" << std::endl;
