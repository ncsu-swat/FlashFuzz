...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor filenames_tensor(tensorflow::DT_STRING, 
                                          tensorflow::TensorShape(filenames_shape));
        ...
        std::cout << "Filenames tensor shape: ";
        for (int i = 0; i < filenames_tensor.shape().dims(); ++i) {
            std::cout << filenames_tensor.shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
        ...
        // Use raw_ops API to create the ExperimentalLMDBDataset
        auto dataset = tensorflow::ops::_LMDBDataset(
            root, filenames_input, output_types, output_shapes);
