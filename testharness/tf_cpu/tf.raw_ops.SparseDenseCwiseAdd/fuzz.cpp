...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        int64_t num_sparse_elements = 1 + (num_sparse_elements_byte % MAX_SPARSE_ELEMENTS);
        
        int64_t total_dense_elements = 1;
        for (auto dim : dense_shape) {
            total_dense_elements *= dim;
        }
        ...
        tensorflow::Tensor sp_indices_tensor(tensorflow::DT_INT64, 
                                           tensorflow::TensorShape({num_sparse_elements, static_cast<int64_t>(dense_rank)}));
        auto sp_indices_flat = sp_indices_tensor.flat<int64_t>();
        
        for (int64_t i = 0; i < num_sparse_elements; ++i) {
            for (int64_t j = 0; j < dense_rank; ++j) {
                if (offset + sizeof(int64_t) <= size) {
                    int64_t idx_val;
                    std::memcpy(&idx_val, data + offset, sizeof(int64_t));
                    offset += sizeof(int64_t);
                    idx_val = std::abs(idx_val) % dense_shape[j];
                    sp_indices_flat(i * dense_rank + j) = idx_val;
                } else {
                    sp_indices_flat(i * dense_rank + j) = 0;
                }
            }
        }
        
        tensorflow::Tensor sp_values_tensor(dtype, tensorflow::TensorShape({num_sparse_elements}));
        fillTensorWithDataByType(sp_values_tensor, dtype, data, offset, size);
        
        tensorflow::Tensor sp_shape_tensor(tensorflow::DT_INT64, tensorflow::TensorShape({static_cast<int64_t>(dense_rank)}));
        auto sp_shape_flat = sp_shape_tensor.flat<int64_t>();
        for (int64_t i = 0; i < dense_rank; ++i) {
        ...
        auto result = tensorflow::ops::SparseDenseCwiseAdd(root, sp_indices_input, sp_values_input, sp_shape_input, dense_input);
