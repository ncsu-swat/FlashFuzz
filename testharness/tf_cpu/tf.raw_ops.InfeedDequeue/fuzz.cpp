...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Create InfeedDequeue op using the raw op API
        tensorflow::NodeDef node_def;
        node_def.set_name("InfeedDequeue");
        node_def.set_op("InfeedDequeue");
        
        auto attr_dtype = node_def.mutable_attr()->insert({"dtype", tensorflow::AttrValue()}).first->second;
        attr_dtype.set_type(dtype);
        
        auto attr_shape = node_def.mutable_attr()->insert({"shape", tensorflow::AttrValue()}).first->second;
        auto* shape_proto = attr_shape.mutable_shape();
        ...
        if (!status.ok()) {
            std::cout << "Error creating InfeedDequeue op: " << status.ToString() << std::endl;
            return -1;
        }
