    ...
    try {
        ...
        for (uint8_t i = 0; i < num_output_types && offset < size; ++i) {
            ...
            if (offset < size) {
                ...
                std::vector<int64_t> shape = parseShape(data, offset, size, shape_rank);
        ...
        // Use raw_ops namespace to access LMDBDataset
        auto lmdb_dataset = tensorflow::ops::experimental::LMDBDataset(
            root, 
            filenames_input,
            output_types,
            output_shapes
        ...
        std::cout << "LMDBDataset operation created successfully" << std::endl;
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({lmdb_dataset.handle}, &outputs);
        if (!status.ok()) {
