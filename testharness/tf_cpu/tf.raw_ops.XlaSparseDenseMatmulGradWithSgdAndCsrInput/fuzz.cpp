...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            float temp_min;
            std::memcpy(&temp_min, data + offset, sizeof(float));
            offset += sizeof(float);
            if (std::isfinite(temp_min)) {
                clip_weight_min = temp_min;
            }
        }

        if (offset < size) {
            float temp_max;
            std::memcpy(&temp_max, data + offset, sizeof(float));
            offset += sizeof(float);
            if (std::isfinite(temp_max)) {
                clip_weight_max = temp_max;
            }
        }

        tensorflow::NodeDef node_def;
        node_def.set_name("XlaSparseDenseMatmulGradWithSgdAndCsrInput");
        node_def.set_op("XlaSparseDenseMatmulGradWithSgdAndCsrInput");
        ...
        auto op = tensorflow::Operation(root.WithOpName("XlaSparseDenseMatmulGradWithSgdAndCsrInput")
                                           .WithDevice("/cpu:0")
