...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto resource_var = tensorflow::ops::VarHandleOp(root, output_dtype, tensorflow::TensorShape({5, 3, 4}));
        
        tensorflow::Tensor init_tensor(output_dtype, tensorflow::TensorShape({5, 3, 4}));
        fillTensorWithDataByType(init_tensor, output_dtype, data, offset, size);
        
        auto init_op = tensorflow::ops::AssignVariableOp(root, resource_var, tensorflow::Input(init_tensor));
        
        auto indices_input = tensorflow::ops::Const(root, indices_tensor);
        
        auto gather_nd_op = tensorflow::ops::ResourceGatherNd(root, resource_var, indices_input);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> init_outputs;
        tensorflow::Status init_status = session.Run({}, {}, {init_op.operation}, &init_outputs);
        if (!init_status.ok()) {
            return -1;
        }
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({}, {gather_nd_op}, {}, &outputs);
        if (!status.ok()) {
            return -1;
        }
