...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t num_outputs = (offset < size) ? (data[offset] % 3) + 1 : 1;
        offset++;
        
        std::vector<tensorflow::DataType> output_types;
        for (uint8_t i = 0; i < num_outputs; ++i) {
            tensorflow::DataType out_dtype = (offset < size) ? parseDataType(data[offset]) : tensorflow::DT_FLOAT;
            if (offset < size) offset++;
            output_types.push_back(out_dtype);
        }
        
        // Create PyFuncStateless op using raw op creation
        tensorflow::NodeDef node_def;
        node_def.set_name("PyFuncStateless");
        node_def.set_op("PyFuncStateless");
        
        auto attr_token = node_def.mutable_attr()->insert({"token", tensorflow::AttrValue()});
        attr_token->second.set_s(token);
        
        auto attr_types = node_def.mutable_attr()->insert({"Tout", tensorflow::AttrValue()});
        for (const auto& dtype : output_types) {
            attr_types->second.mutable_list()->add_type(dtype);
        }
        
        std::vector<tensorflow::Output> inputs_vec;
        for (const auto& input : input_tensors) {
            inputs_vec.push_back(input.node()->output(input.index()));
        }
        
        auto pyfunc_op = tensorflow::Operation(root.WithOpName("PyFuncStateless"), 
                                              "PyFuncStateless", 
                                              inputs_vec);
        
        std::vector<tensorflow::Output> outputs;
        for (int i = 0; i < output_types.size(); i++) {
            outputs.push_back(tensorflow::Output(pyfunc_op.node(), i));
        }
