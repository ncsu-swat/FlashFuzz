...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        bool parallel_copy_val = false;
        if (offset < size) {
            parallel_copy_val = (data[offset++] % 2) == 1;
        }
        ...
        auto range_dataset = tensorflow::ops::RangeDataset(
            ...
            tensorflow::ops::Const(root, 100),
            tensorflow::ops::Const(root, 1),
            {tensorflow::DT_INT64},
            {tensorflow::PartialTensorShape({})});
        
        // Use BatchDataset instead of BatchDatasetV2
        auto batch_dataset = tensorflow::ops::BatchDataset(
            root, 
            range_dataset.handle, 
            batch_size, 
            {output_dtype},
        ...
        tensorflow::Status status = session.Run(
            ...
            {batch_dataset.handle.node()->name()},
            &outputs
        );
