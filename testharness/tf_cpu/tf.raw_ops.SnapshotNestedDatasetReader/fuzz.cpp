...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (uint8_t i = 0; i < num_inputs; ++i) {
            ...
            tensorflow::Tensor input_tensor(tensorflow::DT_VARIANT, tensor_shape);
            
            auto input_op = tensorflow::ops::Placeholder(root, tensorflow::DT_VARIANT);
        ...
        if (output_shapes.empty()) {
            tensorflow::PartialTensorShape default_shape({1});
            output_shapes.push_back(default_shape);
        }

        // Create a raw op using the NodeBuilder
        tensorflow::NodeBuilder node_builder("snapshot_nested_dataset_reader", "SnapshotNestedDatasetReader");
        ...
        node_builder.Input(input_nodes);
        ...
        // Finalize the node
        tensorflow::Node* snapshot_reader_node;
        tensorflow::Status status = node_builder.Finalize(root.graph(), &snapshot_reader_node);
        
        if (!status.ok()) {
            return -1;
        }
        
        tensorflow::Output snapshot_reader(snapshot_reader_node);
        ...
        for (size_t i = 0; i < inputs.size(); ++i) {
            ...
            feed_dict.push_back({inputs[i].node()->name(), variant_tensor});
        }

        std::vector<tensorflow::Tensor> outputs;
        status = session.Run(feed_dict, {snapshot_reader}, {}, &outputs);
