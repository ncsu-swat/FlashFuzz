...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t shift_rank = parseRank(data[offset++]);
        if (shift_rank > 1) shift_rank = 1;
        std::vector<int64_t> shift_shape = parseShape(data, offset, size, shift_rank);
        
        tensorflow::TensorShape shift_tensor_shape(shift_shape);
