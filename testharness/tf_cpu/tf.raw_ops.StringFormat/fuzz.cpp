...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (inputs.empty()) {
            tensorflow::Tensor default_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({2}));
        ...
        std::string placeholder_str = "%s";
        int64_t summarize = 3;
        
        if (offset < size) {
            uint8_t template_len = (data[offset] % 10) + 1;
            offset++;
            template_str = "";
            for (uint8_t i = 0; i < template_len && offset < size; ++i) {
                char c = static_cast<char>((data[offset] % 95) + 32);
                template_str += c;
                offset++;
            }
        ...
        if (offset < size) {
            uint8_t placeholder_len = (data[offset] % 5) + 1;
            offset++;
            placeholder_str = "";
            for (uint8_t i = 0; i < placeholder_len && offset < size; ++i) {
                char c = static_cast<char>((data[offset] % 95) + 32);
                placeholder_str += c;
                offset++;
            }
        ...
        auto string_format_op = tensorflow::ops::StringFormat(
            root, inputs,
            tensorflow::ops::StringFormat::Template(template_str)
        ...
        tensorflow::Status status = session.Run({string_format_op}, &outputs);
