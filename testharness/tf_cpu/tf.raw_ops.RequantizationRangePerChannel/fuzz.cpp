...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        int64_t num_channels = input_shape.empty() ? 1 : input_shape.back();
        
        tensorflow::TensorShape min_max_shape;
        min_max_shape.AddDim(num_channels);
        
        tensorflow::Tensor input_min_tensor(tensorflow::DT_FLOAT, min_max_shape);
        tensorflow::Tensor input_max_tensor(tensorflow::DT_FLOAT, min_max_shape);
        
        fillTensorWithDataByType(input_min_tensor, tensorflow::DT_FLOAT, data, offset, size);
        fillTensorWithDataByType(input_max_tensor, tensorflow::DT_FLOAT, data, offset, size);
        
        float clip_value_max = 6.0f;
        if (offset + sizeof(float) <= size) {
            std::memcpy(&clip_value_max, data + offset, sizeof(float));
            offset += sizeof(float);
            clip_value_max = std::abs(clip_value_max);
            if (clip_value_max == 0.0f) clip_value_max = 6.0f;
        }
        ...
        tensorflow::Output output_min, output_max;
        tensorflow::NodeBuilder node_builder("RequantizationRangePerChannel", "RequantizationRangePerChannel");
        node_builder.Input(input_op.node())
                   .Input(input_min_op.node())
                   .Input(input_max_op.node())
        ...
        output_min = tensorflow::Output(node, 0);
        output_max = tensorflow::Output(node, 1);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        status = session.Run({output_min, output_max}, &outputs);
        if (!status.ok()) {
            return -1;
        }
