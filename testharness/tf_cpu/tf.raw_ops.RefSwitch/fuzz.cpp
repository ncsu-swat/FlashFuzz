...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        bool pred_value = (offset < size) ? (data[offset++] % 2 == 1) : false;
        tensorflow::Tensor pred_tensor(tensorflow::DT_BOOL, tensorflow::TensorShape({}));
        pred_tensor.scalar<bool>()() = pred_value;
        
        auto data_var = tensorflow::ops::Variable(root, data_tensor_shape, data_dtype);
        auto assign_data = tensorflow::ops::Assign(root, data_var, tensorflow::ops::Const(root, data_tensor));
        
        auto pred_placeholder = tensorflow::ops::Const(root, pred_tensor);
        
        auto ref_switch = tensorflow::ops::RefSwitch(root, data_var, pred_placeholder);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({assign_data}, &outputs);
        if (!status.ok()) {
            return -1;
        }
        
        status = session.Run({ref_switch.output_false, ref_switch.output_true}, &outputs);
        if (!status.ok()) {
            return -1;
        }
