...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        // Use raw_ops directly since StatelessRandomGetAlg is not in tensorflow::ops namespace
        auto result = tensorflow::ops::StatelessRandomGetKeyCounter(root, 
        ...
        if (!outputs.empty()) {
            ...
            if (outputs[0].dtype() == tensorflow::DT_INT64) {
                ...
                for (int i = 0; i < std::min(static_cast<int>(flat.size()), 10); ++i) {
                    std::cout << flat(i) << " ";
                }
                std::cout << std::endl;
            }
