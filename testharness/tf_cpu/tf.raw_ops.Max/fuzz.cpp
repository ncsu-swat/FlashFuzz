...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (int i = 0; i < axis_flat.size(); ++i) {
            if (offset < size) {
                int32_t axis_val = static_cast<int32_t>(data[offset++] % (2 * input_rank + 1)) - input_rank;
        ...
        auto max_op = tensorflow::ops::Max(root, input_op, axis_op, 
                                          tensorflow::ops::Max::KeepDims(keep_dims));
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({max_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
