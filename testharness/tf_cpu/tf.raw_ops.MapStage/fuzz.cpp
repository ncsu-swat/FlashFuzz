...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor key_tensor(tensorflow::DT_INT64, tensorflow::TensorShape(key_shape));
        fillTensorWithDataByType(key_tensor, tensorflow::DT_INT64, data, offset, size);
        auto key = tensorflow::ops::Const(root, key_tensor);
        ...
        for (uint8_t i = 0; i < num_values; ++i) {
            ...
            tensorflow::Tensor value_tensor(dtype, tensorflow::TensorShape(value_shape));
            fillTensorWithDataByType(value_tensor, dtype, data, offset, size);
            values.push_back(tensorflow::ops::Const(root, value_tensor));
        }

        int64_t capacity = (offset < size) ? static_cast<int64_t>(data[offset++] % 100) : 0;
        int64_t memory_limit = (offset < size) ? static_cast<int64_t>(data[offset++] % 100) : 0;
        
        std::string container = "";
        std::string shared_name = "test_stage";

        auto map_stage = tensorflow::ops::MapStage(
            root,
            key,
            indices,
            values,
        ...
        tensorflow::Status status = session.Run({}, {}, {map_stage.operation}, &outputs);
        if (!status.ok()) {
            return -1;
        }
