...
std::vector<int> parseExplicitPaddings(const uint8_t* data, size_t& offset, size_t total_size, const std::string& padding) {
    ...
    std::vector<int> explicit_paddings(8);
    for (int i = 0; i < 8; ++i) {
        if (offset < total_size) {
            uint8_t pad_val = data[offset++];
            explicit_paddings[i] = pad_val % 5;
        } else {
...
std::vector<int> parseDilations(const uint8_t* data, size_t& offset, size_t total_size) {
    ...
    for (int i = 0; i < 4; ++i) {
        if (offset < total_size) {
            uint8_t dil_val = data[offset++];
            dilations[i] = (dil_val % 3) + 1;
        } else {
            dilations[i] = 1;
        }
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::vector<int64_t> out_backprop_shape = {1, 4, 4, 2};
        
        std::string data_format = parseDataFormat(data[offset++]);
        if (data_format == "NCHW") {
            input_shape = {1, 3, 4, 4};
            out_backprop_shape = {1, 2, 4, 4};
        }
        ...
        auto conv2d_backprop_input_attrs = tensorflow::ops::Conv2DBackpropInput::Attrs()
            .UseCudnnOnGpu(use_cudnn_on_gpu)
            .ExplicitPaddings(explicit_paddings)
            .DataFormat(data_format)
            .Dilations(dilations);
            
        auto conv2d_backprop_input = tensorflow::ops::Conv2DBackpropInput(
            root,
            input_placeholder,
            filter_placeholder,
