...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        offset++;
        std::vector<int64_t> fft_length_shape = parseShape(data, offset, size, 1);
        fft_length_shape[0] = std::min(fft_length_shape[0], static_cast<int64_t>(input_rank));
        
        tensorflow::TensorShape fft_length_tensor_shape;
        fft_length_tensor_shape.AddDim(fft_length_shape[0]);
        ...
        tensorflow::TensorShape axes_tensor_shape;
        axes_tensor_shape.AddDim(fft_length_shape[0]);
        
        tensorflow::Tensor axes_tensor(tensorflow::DT_INT32, axes_tensor_shape);
        ...
        for (int i = 0; i < fft_length_shape[0]; ++i) {
            if (offset + sizeof(int32_t) <= size) {
                ...
                std::memcpy(&value, data + offset, sizeof(int32_t));
                offset += sizeof(int32_t);
                value = std::abs(value) % input_rank;
                axes_flat(i) = value;
            } else {
        ...
        // Create FFTND operation using raw_ops
        tensorflow::Output fftnd_op = tensorflow::ops::FFTND(root, input_op, fft_length_op, axes_op);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({fftnd_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
