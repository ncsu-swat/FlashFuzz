...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Use raw operation instead of tensorflow::ops::TensorMapLookup
        auto tensor_map_lookup = tensorflow::ops::Operation(
            root.WithOpName("TensorMapLookup"),
            "TensorMapLookup",
            {input_handle.output, key.output},
        ...
        tensorflow::Status status = session.Run(
            {{input_handle, input_handle_tensor}, {key, key_tensor}},
            {tensorflow::Output(tensor_map_lookup.node(), 0)}, &outputs);
