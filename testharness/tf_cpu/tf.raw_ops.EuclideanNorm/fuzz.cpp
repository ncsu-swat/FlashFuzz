    ...
    try {
        ...
        if (axis_dtype == tensorflow::DT_INT32) {
            ...
            for (int i = 0; i < axis_flat.size(); ++i) {
                if (offset < size) {
                    int32_t axis_val;
                    std::memcpy(&axis_val, data + (offset % size), sizeof(int32_t));
            ...
            for (int i = 0; i < axis_flat.size(); ++i) {
                if (offset < size) {
                    int64_t axis_val;
                    std::memcpy(&axis_val, data + (offset % size), sizeof(int64_t));
        ...
        auto euclidean_norm = tensorflow::ops::EuclideanNorm(
            root, input_placeholder, axis_placeholder,
            tensorflow::ops::EuclideanNorm::Attrs().KeepDims(keep_dims)
        );
        ...
        tensorflow::Status status = session.Run(
            {{input_placeholder, input_tensor}, {axis_placeholder, axis_tensor}},
            {euclidean_norm},
            &outputs
        );
