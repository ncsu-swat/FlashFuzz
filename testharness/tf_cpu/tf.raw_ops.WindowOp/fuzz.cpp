...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        uint8_t num_inputs = (data[offset] % MAX_NUM_INPUTS) + 1;
        offset++;
        ...
        // Use raw_ops.Window instead of ops::WindowOp
        tensorflow::Node* window_node;
        tensorflow::Status status = tensorflow::NodeBuilder("Window", "Window")
            .Input(tensorflow::InputList(inputs))
            .Attr("output_types", output_types)
            .Attr("output_shapes", output_shapes)
            .Finalize(root.graph(), &window_node);
        ...
        status = session.Run({tensorflow::Output(window_node, 0)}, &outputs);
