...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::TensorShape sp_indices_shape({num_sparse_elements, static_cast<int64_t>(dense_rank)});
        ...
        for (int64_t i = 0; i < num_sparse_elements; ++i) {
            for (int64_t j = 0; j < dense_rank; ++j) {
                if (offset + sizeof(int64_t) <= size) {
                    ...
                    sp_indices_flat(i * dense_rank + j) = std::abs(idx) % dense_shape[j];
                } else {
        ...
        tensorflow::TensorShape sp_values_shape({num_sparse_elements});
        tensorflow::Tensor sp_values_tensor(dtype, sp_values_shape);
        ...
        for (size_t i = 0; i < dense_rank; ++i) {
            sp_shape_flat(i) = dense_shape[i];
        }
        
        tensorflow::TensorShape dense_tensor_shape(dense_shape);
        tensorflow::Tensor dense_tensor(dtype, dense_tensor_shape);
        ...
        auto sparse_dense_cwisemul = tensorflow::ops::SparseDenseCwiseMul(
            root, sp_indices_input, sp_values_input, sp_shape_input, dense_input);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({sparse_dense_cwisemul}, &outputs);
        if (!status.ok()) {
            return -1;
        }
