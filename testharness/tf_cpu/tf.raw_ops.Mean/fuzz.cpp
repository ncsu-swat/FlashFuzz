...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (int i = 0; i < axis_flat.size(); ++i) {
            if (offset < size) {
                int32_t axis_val = static_cast<int32_t>(data[offset++] % (input_rank > 0 ? input_rank : 1));
                if (axis_val >= input_rank) axis_val = input_rank - 1;
        ...
        auto mean_op = tensorflow::ops::Mean(root, input_op, axis_op, 
                                           tensorflow::ops::Mean::KeepDims(keep_dims));

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({mean_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
