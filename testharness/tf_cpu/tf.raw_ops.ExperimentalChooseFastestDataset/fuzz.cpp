...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t num_experiments_byte = data[offset++];
        int num_experiments = (num_experiments_byte % 10) + 1;
        ...
        for (int i = 0; i < num_datasets; ++i) {
            ...
            auto dataset = tensorflow::ops::TensorSliceDataset(root, {const_op});
            input_datasets.push_back(dataset);
        }
        
        if (input_datasets.size() < 2) {
        ...
        std::cout << "Number of input datasets: " << input_datasets.size() << std::endl;
        ...
        tensorflow::ops::ExperimentalChooseFastestDataset::Attrs attrs;
        auto experimental_choose_fastest = tensorflow::ops::ExperimentalChooseFastestDataset(
            root, input_datasets, num_experiments, output_types, output_shapes);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({experimental_choose_fastest}, &outputs);
        if (!status.ok()) {
