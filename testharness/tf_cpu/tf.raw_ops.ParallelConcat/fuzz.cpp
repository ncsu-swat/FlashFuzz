...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        output_shape.AddDim(num_tensors);
        for (size_t i = 1; i < base_shape.size(); ++i) {
            output_shape.AddDim(base_shape[i]);
        }
        
        auto parallel_concat_op = tensorflow::ops::ParallelConcat(root, input_tensors, output_shape);
        
        tensorflow::ClientSession session(root);
        
        std::unordered_map<tensorflow::Output, tensorflow::Input::Initializer> feed_dict;
        for (size_t i = 0; i < input_tensors.size(); ++i) {
            feed_dict.insert({input_tensors[i], tensor_values[i]});
        }
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run(feed_dict, {parallel_concat_op}, &outputs);
