...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto input_dataset = tensorflow::ops::Const(root, input_tensor);
        ...
        for (const auto& shape : {input_tensor_shape}) {
            ...
            tensorflow::PartialTensorShape::MakePartialShape(shape.dim_sizes().data(), shape.dims(), &partial_shape);
            output_shapes.push_back(partial_shape);
        }
        
        // Use raw_ops approach instead of ops namespace
        auto parallel_interleave = tensorflow::ops::RawOp(
            root.WithOpName("ParallelInterleaveDatasetV2"),
            "ParallelInterleaveDatasetV2",
            {input_dataset.output, cycle_length, block_length, num_parallel_calls},
            {
                {"f", f_attr},
                {"Targuments", {}},
        ...
        std::cout << "Input dataset created" << std::endl;
        ...
        tensorflow::Status status = session.Run({parallel_interleave}, &outputs);
        if (!status.ok()) {
