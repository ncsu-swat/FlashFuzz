...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t fft_length_rank = parseRank(data[offset++]);
        if (fft_length_rank > input_rank) fft_length_rank = input_rank;
        
        std::vector<int32_t> fft_length_data;
        for (uint8_t i = 0; i < fft_length_rank; ++i) {
            if (offset + sizeof(int32_t) <= size) {
                ...
                fft_length_data.push_back(1);
            }
        }
        
        tensorflow::TensorShape fft_length_shape;
        fft_length_shape.AddDim(fft_length_rank);
        ...
        for (uint8_t i = 0; i < fft_length_rank; ++i) {
            if (offset < size) {
                int32_t axis = data[offset++] % input_rank;
                axes_data.push_back(axis);
            } else {
                axes_data.push_back(i);
            }
        }
        
        tensorflow::TensorShape axes_shape;
        axes_shape.AddDim(fft_length_rank);
        ...
        tensorflow::Node* irfftnd_node;
        tensorflow::NodeBuilder builder("irfftnd", "IRFFTND");
        builder.Input(input_op.node())
               .Input(fft_length_op.node())
               .Input(axes_op.node())
               .Attr("Treal", output_dtype);
        
        tensorflow::Status build_status = builder.Finalize(root.graph(), &irfftnd_node);
        if (!build_status.ok()) {
            return -1;
        }
        ...
        tensorflow::Status status = session.Run({tensorflow::Output(irfftnd_node)}, &outputs);
        if (!status.ok()) {
            return -1;
        }
