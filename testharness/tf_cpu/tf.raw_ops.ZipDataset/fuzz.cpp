...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        uint8_t num_datasets = (data[offset] % MAX_DATASETS) + 1;
        offset++;
        ...
        for (uint8_t i = 0; i < num_datasets; ++i) {
            ...
            // Create a tensor slice dataset
            auto range_dataset = tensorflow::ops::TensorSliceDataset(root, {const_op});
            
            input_datasets.push_back(range_dataset);
        ...
        // Create a ZipDataset op
        tensorflow::ops::experimental::ZipDataset zip_dataset(
            root, 
            input_datasets,
            output_types,
            output_shapes
        ...
        tensorflow::Status status = session.Run({zip_dataset.handle}, &outputs);
