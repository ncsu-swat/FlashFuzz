...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor linears_tensor(tensorflow::DT_FLOAT, tensor_shape3);
        fillTensorWithDataByType(linears_tensor, tensorflow::DT_FLOAT, data, offset, size);
        ...
        auto linears_input = tensorflow::ops::Const(root, linears_tensor);

        // Use raw_ops API instead of the missing tpu_ops.h
        auto load_op = tensorflow::ops::Operation(
            root.WithOpName("LoadTPUEmbeddingFTRLParameters"),
            "LoadTPUEmbeddingFTRLParameters",
            {parameters_input.node(), accumulators_input.node(), linears_input.node()},
            {
