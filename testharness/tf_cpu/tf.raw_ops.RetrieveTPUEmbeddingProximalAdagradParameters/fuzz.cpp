...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Use raw_ops instead of ops::RetrieveTPUEmbeddingProximalAdagradParameters
        ...
        auto retrieve_op = tensorflow::Operation(
            root.WithOpName("RetrieveTPUEmbeddingProximalAdagradParameters")
                .WithDevice("/cpu:0"),
            "RetrieveTPUEmbeddingProximalAdagradParameters",
            {num_shards_tensor.node(), shard_id_tensor.node()},
            op_attrs);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({tensorflow::Output(retrieve_op, 0), tensorflow::Output(retrieve_op, 1)}, &outputs);
