...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Create SummaryWriter op using raw_ops
        auto writer = tensorflow::ops::Placeholder(root, tensorflow::DT_RESOURCE);
        
        tensorflow::NodeDef def;
        def.set_op("SummaryWriter");
        def.set_name(root.UniqueName("SummaryWriter"));
        
        if (!shared_name.empty()) {
            (*def.mutable_attr())["shared_name"].set_s(shared_name);
        }
        if (!container.empty()) {
            (*def.mutable_attr())["container"].set_s(container);
        }
        
        tensorflow::Status status;
        auto op = root.AddNode(def, &status);
        if (!status.ok()) {
            std::cout << "Error creating SummaryWriter node: " << status.ToString() << std::endl;
            return -1;
        }
        ...
        if (!outputs.empty()) {
            std::cout << "SummaryWriter output tensor shape: ";
