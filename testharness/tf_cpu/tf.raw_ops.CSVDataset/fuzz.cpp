...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::ops::DatasetOps::CsvDataset csv_dataset = tensorflow::ops::DatasetOps::CsvDataset(
            root,
            filenames_tensor,
            record_defaults,
            compression_type_tensor.scalar<tensorflow::tstring>()(),
            buffer_size_tensor.scalar<int64_t>()(),
            header_tensor.scalar<bool>()(),
            field_delim_tensor.scalar<tensorflow::tstring>()(),
            use_quote_delim_tensor.scalar<bool>()(),
            na_value_tensor.scalar<tensorflow::tstring>()(),
            select_cols_tensor
        );

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({csv_dataset.output}, &outputs);
