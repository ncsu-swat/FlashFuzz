...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto legacy_parallel_interleave = tensorflow::ops::internal::LegacyParallelInterleaveDatasetV2(
            root.WithOpName("legacy_parallel_interleave"),
            input_dataset,
            other_arguments,
            cycle_length,
            block_length,
        ...
        std::cout << "Input dataset shape: " << input_dataset_tensor.shape().DebugString() << std::endl;
        std::cout << "Cycle length: " << cycle_length_val << std::endl;
        std::cout << "Block length: " << block_length_val << std::endl;
        std::cout << "Buffer output elements: " << buffer_output_elements_val << std::endl;
        std::cout << "Prefetch input elements: " << prefetch_input_elements_val << std::endl;
        
        tensorflow::Status status = session.Run({legacy_parallel_interleave}, &outputs);
        if (!status.ok()) {
