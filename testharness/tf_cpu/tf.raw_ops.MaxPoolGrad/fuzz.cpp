...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (data_format == "NHWC") {
            if (orig_input_shape.size() == 4) {
                int64_t batch = orig_input_shape[0];
                int64_t height = orig_input_shape[1];
                int64_t width = orig_input_shape[2];
                int64_t channels = orig_input_shape[3];
                ...
                if (out_height > 0 && out_width > 0) {
                    orig_output_shape = {batch, out_height, out_width, channels};
                    grad_shape = orig_output_shape;
                }
            }
        } else {
            if (orig_input_shape.size() == 4) {
                int64_t batch = orig_input_shape[0];
                int64_t channels = orig_input_shape[1];
                int64_t height = orig_input_shape[2];
                int64_t width = orig_input_shape[3];
                ...
                if (out_height > 0 && out_width > 0) {
                    orig_output_shape = {batch, channels, out_height, out_width};
                    grad_shape = orig_output_shape;
                }
            }
        }
        ...
        // Use raw_ops namespace for MaxPoolGrad
        auto max_pool_grad = tensorflow::ops::internal::MaxPoolGrad(
            root,
            orig_input_op,
            orig_output_op,
            grad_op,
