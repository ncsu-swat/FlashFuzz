...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor input_dataset(tensorflow::DT_VARIANT, tensor_shape);
        ...
        // Create a NonSerializableDataset operation using raw ops
        tensorflow::NodeDef node_def;
        node_def.set_name("NonSerializableDataset");
        node_def.set_op("NonSerializableDataset");
        
        // Add input
        *node_def.add_input() = input_dataset_op.node()->name();
        
        // Add attributes
        auto attr_map = node_def.mutable_attr();
        
        // Set output_types attribute
        ...
        auto non_serializable_dataset = root.AddNode(node_def, &status);
        
        if (!status.ok()) {
            std::cout << "Error creating NonSerializableDataset node: " << status.ToString() << std::endl;
            return -1;
        }
