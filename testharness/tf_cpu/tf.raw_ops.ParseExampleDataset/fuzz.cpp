...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (size_t i = 0; i < sparse_keys.size(); ++i) {
            output_types.push_back(sparse_types[i]);
            output_shapes.push_back(tensorflow::PartialTensorShape({-1}));
        }
        ...
        auto parse_example_dataset_op = tensorflow::ops::experimental::ParseExampleDataset(
            root,
        ...
        tensorflow::Status status = session.Run({parse_example_dataset_op.handle}, &outputs);
