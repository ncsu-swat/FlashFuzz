...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto input_handle = tensorflow::ops::Placeholder(root, input_dtype);
        
        // Use raw operation instead of ops namespace
        auto wrap_dataset_variant_op = tensorflow::ops::Operation(
            root.WithOpName("WrapDatasetVariant"),
            "WrapDatasetVariant",
            {input_handle},
            {});
        ...
        tensorflow::Status status = session.Run({{input_handle, input_tensor}}, {wrap_dataset_variant_op.output(0)}, &outputs);
