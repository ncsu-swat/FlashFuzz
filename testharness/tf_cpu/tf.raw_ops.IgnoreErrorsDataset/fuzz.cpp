...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        bool log_warning = (offset < size) ? (data[offset] % 2 == 1) : false;
        offset++;
        ...
        // Create IgnoreErrorsDataset using raw_ops
        auto ignore_errors_dataset = tensorflow::ops::_IgnoreErrorsDataset(
            root,
            range_dataset,
            output_types,
            output_shapes,
        ...
        tensorflow::Status status = session.Run({ignore_errors_dataset}, &outputs);
        if (!status.ok()) {
            return -1;
        }
