...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor another_dataset_tensor(tensorflow::DT_VARIANT, tensorflow::TensorShape({}));
        
        auto input_dataset = tensorflow::ops::Placeholder(root, tensorflow::DT_VARIANT);
        auto another_dataset = tensorflow::ops::Placeholder(root, tensorflow::DT_VARIANT);
        
        // Use raw_ops approach for ConcatenateDataset
        auto concatenate_dataset = tensorflow::ops::internal::ConcatenateDataset(
            root, 
            input_dataset, 
            another_dataset,
            output_types,
        ...
        tensorflow::Status status = session.Run(
            {{input_dataset, input_dataset_tensor}, {another_dataset, another_dataset_tensor}},
            {concatenate_dataset},
            &outputs
        );
