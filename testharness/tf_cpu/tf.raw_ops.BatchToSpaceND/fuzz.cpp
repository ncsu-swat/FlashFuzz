...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t spatial_dims = input_rank - 1;
        if (spatial_dims < 1) spatial_dims = 1;
        if (spatial_dims > 3) spatial_dims = 3;
        
        std::vector<int32_t> block_shape_data;
        for (uint8_t i = 0; i < spatial_dims; ++i) {
            if (offset < size) {
                int32_t block_val = (data[offset++] % 4) + 1;
        ...
        for (size_t i = 0; i < block_shape_data.size(); ++i) {
            block_shape_flat(i) = block_shape_data[i];
        }
        
        std::vector<int32_t> crops_data;
        for (uint8_t i = 0; i < spatial_dims; ++i) {
            int32_t crop_start = 0;
            int32_t crop_end = 0;
            if (offset < size) {
                crop_start = data[offset++] % 2;
            }
            if (offset < size) {
                crop_end = data[offset++] % 2;
            }
            crops_data.push_back(crop_start);
            crops_data.push_back(crop_end);
        }
        
        tensorflow::TensorShape crops_tensor_shape;
        crops_tensor_shape.AddDim(spatial_dims);
        crops_tensor_shape.AddDim(2);
        tensorflow::Tensor crops_tensor(tensorflow::DT_INT32, crops_tensor_shape);
        auto crops_flat = crops_tensor.flat<int32_t>();
        for (size_t i = 0; i < crops_data.size(); ++i) {
            crops_flat(i) = crops_data[i];
        }
