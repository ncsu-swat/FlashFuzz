...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            axis_count = (data[offset++] % tensor_rank) + 1;
        }
        
        std::vector<int32_t> axis_values;
        for (uint8_t i = 0; i < axis_count && offset < size; ++i) {
            int32_t axis_val = static_cast<int32_t>(data[offset++] % (2 * tensor_rank)) - tensor_rank;
            axis_values.push_back(axis_val);
        }
        ...
        axis_shape.AddDim(axis_values.size());
