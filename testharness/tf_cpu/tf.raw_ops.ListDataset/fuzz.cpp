...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        uint8_t num_tensors = (data[offset] % MAX_TENSORS) + 1;
        offset++;

        std::vector<tensorflow::Output> tensor_inputs;
        ...
        for (uint8_t i = 0; i < num_tensors && offset < size; ++i) {
            ...
            tensor_inputs.push_back(const_op);
        ...
        // Use raw_ops.TensorSliceDataset as an alternative since ListDataset is not directly available
        auto dataset = tensorflow::ops::TensorSliceDataset(root, tensor_inputs, output_shapes);

        std::cout << "Created Dataset with " << tensor_inputs.size() << " tensors" << std::endl;
        std::cout << "Metadata: " << metadata << std::endl;

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({dataset}, &outputs);
