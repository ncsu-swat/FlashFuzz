...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor dummy_data(output_dtype, tensor_shape);
        fillTensorWithDataByType(dummy_data, output_dtype, data, offset, size);
        
        auto tensor_slice = tensorflow::ops::TensorSliceDataset(root, {dummy_data}, output_shape);
        ...
        auto seed_tensor = tensorflow::ops::Const(root, seed_val);
        auto seed2_tensor = tensorflow::ops::Const(root, seed2_val);
        
        bool reshuffle_each_iteration = (offset < size) ? (data[offset++] % 2 == 0) : true;
        ...
        // Create a ShuffleDataset as a substitute for GlobalShuffleDataset
        auto shuffle_dataset = tensorflow::ops::ShuffleDataset(
            ...
            tensorflow::ops::ShuffleDataset::ReshuffleEachIteration(reshuffle_each_iteration)
        );
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({shuffle_dataset.handle}, &outputs);
