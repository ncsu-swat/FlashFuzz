...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (indices_dtype == tensorflow::DT_INT32) {
            ...
            for (int i = 0; i < indices_flat.size(); ++i) {
                if (offset + sizeof(int32_t) <= size) {
                    ...
                    if (ref_shape.size() > 0) {
                        indices_flat(i) = std::abs(val) % static_cast<int32_t>(ref_shape[0]);
                    } else {
                        indices_flat(i) = 0;
                    }
                } else {
            ...
            for (int i = 0; i < indices_flat.size(); ++i) {
                if (offset + sizeof(int64_t) <= size) {
                    ...
                    if (ref_shape.size() > 0) {
                        indices_flat(i) = std::abs(val) % ref_shape[0];
                    } else {
                        indices_flat(i) = 0;
                    }
                } else {
        ...
        auto scatter_update = tensorflow::ops::ScatterUpdate(
            root, 
            ref_var, 
            tensorflow::ops::Const(root, indices_tensor),
            tensorflow::ops::Const(root, updates_tensor),
            tensorflow::ops::ScatterUpdate::UseLocking(use_locking)
        );

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status init_status = session.Run({assign_ref}, nullptr);
        if (!init_status.ok()) {
            return -1;
        }
        
        tensorflow::Status status = session.Run({scatter_update}, &outputs);
        if (!status.ok()) {
            return -1;
        }
