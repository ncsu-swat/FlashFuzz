...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Use raw_ops namespace for DeleteMemoryCache
        auto delete_memory_cache_op = tensorflow::ops::DeleteMemoryCache(
            root, handle_input, deleter_input);

        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run(
            {{handle_input, handle_tensor}, {deleter_input, deleter_tensor}},
            {delete_memory_cache_op.operation.node()->name()}, 
            {}, 
            &outputs);
