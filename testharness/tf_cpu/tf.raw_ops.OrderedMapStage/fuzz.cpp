...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset >= size) return 0;
        uint8_t num_values = (data[offset++] % 3) + 1;
        ...
        for (uint8_t i = 0; i < num_values; ++i) {
            ...
            values.push_back(tensorflow::Input(value_tensor));
        }
        ...
        auto key_input = tensorflow::Input(key_tensor);
        auto indices_input = tensorflow::Input(indices_tensor);
        
        auto stage_op = tensorflow::ops::OrderedMapStage(
            root,
            key_input,
            indices_input,
            values,
