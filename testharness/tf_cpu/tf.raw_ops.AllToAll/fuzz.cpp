...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        int32_t concat_dimension = static_cast<int32_t>(data[offset++]) % input_rank;
        int32_t split_dimension = static_cast<int32_t>(data[offset++]) % input_rank;
        int32_t split_count = static_cast<int32_t>(data[offset++]) % 8 + 1;
        
        auto input = tensorflow::ops::Const(root, input_tensor);
        auto group_assignment = tensorflow::ops::Const(root, group_assignment_tensor);
        
        // Use raw_ops.AllToAll through the NodeBuilder API
        tensorflow::NodeBuilder builder = tensorflow::NodeBuilder("all_to_all", "AllToAll")
            .Input(input.node())
            .Input(group_assignment.node())
            .Attr("T", input_dtype)
            .Attr("concat_dimension", concat_dimension)
            .Attr("split_dimension", split_dimension)
            .Attr("split_count", split_count);
        
        tensorflow::Node* all_to_all_node;
        tensorflow::Status status = builder.Finalize(root.graph(), &all_to_all_node);
        
        if (!status.ok()) {
            return -1;
        }
        
        tensorflow::Output all_to_all(all_to_all_node);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        status = session.Run({all_to_all}, &outputs);
