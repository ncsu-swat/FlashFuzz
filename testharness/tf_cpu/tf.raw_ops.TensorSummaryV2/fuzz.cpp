...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor input_tensor(tensor_dtype, tf_shape);
        ...
        uint8_t metadata_len = (offset < size) ? data[offset] % 50 + 1 : 10;
        if (offset < size) offset++;
        
        std::string metadata_str;
        for (uint8_t i = 0; i < metadata_len && offset < size; ++i) {
            metadata_str += static_cast<char>(data[offset] % 128);
            offset++;
        }
        
        tensorflow::Tensor metadata_tensor(tensorflow::DT_STRING, tensorflow::TensorShape({}));
        metadata_tensor.scalar<tensorflow::tstring>()() = tensorflow::tstring(metadata_str);
        auto metadata_input = tensorflow::ops::Const(root, metadata_tensor);

        std::cout << "Tag: " << tag_str << std::endl;
        std::cout << "Tensor dtype: " << tensorflow::DataTypeString(tensor_dtype) << std::endl;
        ...
        // Use raw_ops.TensorSummaryV2 instead of ops::TensorSummaryV2
        auto summary_op = tensorflow::ops::Operation(root.WithOpName("TensorSummaryV2")
            .WithAttr("T", tensor_dtype)
            .WithInput(tag_input)
            .WithInput(tensor_input)
            .WithInput(metadata_input));

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({summary_op}, &outputs);
        if (!status.ok()) {
        ...
        if (!outputs.empty()) {
            std::cout << "Summary output shape: " << outputs[0].shape().DebugString() << std::endl;
        }
