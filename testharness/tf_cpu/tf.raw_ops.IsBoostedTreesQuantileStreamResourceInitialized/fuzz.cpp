...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        resource_handle.set_container("");
        resource_handle.set_name("quantile_stream_resource");
        resource_handle.set_hash_code(12345);
        resource_handle.set_maybe_type_name("QuantileStreamResource");
        ...
        auto quantile_stream_resource_handle = tensorflow::ops::Const(root, resource_tensor);

        // Use raw_ops directly instead of the missing boosted_trees_ops.h
        auto result = tensorflow::ops::Operation(
            root.WithOpName("IsBoostedTreesQuantileStreamResourceInitialized"),
        ...
        tensorflow::Status status = session.Run({result.node()->name()}, &outputs);
        if (!status.ok()) {
            return -1;
        }
