...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor tensor_tensor(tensor_dtype, tensor_tf_shape);
        tensorflow::Tensor indices_tensor(indices_dtype, indices_tf_shape);
        tensorflow::Tensor updates_tensor(tensor_dtype, updates_tf_shape);
        ...
        std::string bad_indices_policy = "";
        if (offset < size) {
            uint8_t policy_selector = data[offset++];
            switch (policy_selector % 3) {
                case 0:
                    bad_indices_policy = "";
                    break;
                case 1:
        ...
        auto result = tensorflow::ops::TensorScatterUpdate(
            root, tensor_input, indices_input, updates_input,
            tensorflow::ops::TensorScatterUpdate::BadIndicesPolicy(bad_indices_policy));
