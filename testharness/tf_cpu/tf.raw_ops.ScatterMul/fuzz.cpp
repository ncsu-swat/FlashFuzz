...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (int64_t dim : ref_shape) {
            tensorflow::Status status = ref_tensor_shape.AddDimWithStatus(dim);
            if (!status.ok()) return 0;
        }
        
        tensorflow::TensorShape indices_tensor_shape;
        for (int64_t dim : indices_shape) {
            tensorflow::Status status = indices_tensor_shape.AddDimWithStatus(dim);
            if (!status.ok()) return 0;
        }
        
        tensorflow::TensorShape updates_tensor_shape;
        for (int64_t dim : updates_shape) {
            tensorflow::Status status = updates_tensor_shape.AddDimWithStatus(dim);
            if (!status.ok()) return 0;
        }
        ...
        auto scatter_mul = tensorflow::ops::ScatterMul(
            root,
            ref_var,
        ...
        tensorflow::Status assign_status = session.Run({assign_ref}, &outputs);
        if (!assign_status.ok()) {
            return -1;
        }
