...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        index_tensor.scalar<int32_t>()() = static_cast<int32_t>(index_val % num_inputs);
        
        auto index_op = tensorflow::ops::Const(root, index_tensor);
        
        std::vector<tensorflow::Output> input_ops;
        ...
        auto ref_select_op = tensorflow::ops::RefSelect(root, index_op, input_ops);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({ref_select_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
