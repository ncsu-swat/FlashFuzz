...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        uint8_t num_components = (data[offset] % MAX_COMPONENTS) + 1;
        offset++;

        std::vector<tensorflow::Output> components;
        ...
        for (uint8_t comp_idx = 0; comp_idx < num_components; ++comp_idx) {
            ...
            std::cout << "Component " << static_cast<int>(comp_idx) 
                      << " - Type: " << tensorflow::DataTypeString(dtype)
                      << ", Shape: " << tensor_shape.DebugString() << std::endl;

            auto const_op = tensorflow::ops::Const(root, tensor);
            components.push_back(const_op);
            output_types.push_back(dtype);
            output_shapes.push_back(tensorflow::PartialTensorShape(tensor_shape));
        }
        ...
        std::string metadata = "";
        if (offset < size) {
            uint8_t metadata_len = data[offset] % 20;
            offset++;
        ...
        std::cout << "Creating TensorDataset with " << components.size() 
                  << " components, metadata: '" << metadata << "'" << std::endl;

        // Use raw_ops.TensorDataset via the NodeBuilder approach
        tensorflow::NodeBuilder node_builder("tensor_dataset", "TensorDataset");
        
        // Add components as inputs
        for (const auto& component : components) {
            node_builder.Input(component.node());
        }
        
        // Add attributes
        node_builder.Attr("Toutput_types", output_types);
        node_builder.Attr("output_shapes", output_shapes);
        if (!metadata.empty()) {
            node_builder.Attr("metadata", metadata);
        }
        
        tensorflow::Node* tensor_dataset_node;
        tensorflow::Status status = node_builder.Finalize(root.graph(), &tensor_dataset_node);
        
        if (!status.ok()) {
            std::cout << "Error creating TensorDataset node: " << status.ToString() << std::endl;
            return -1;
        }
        
        tensorflow::Output tensor_dataset_output(tensor_dataset_node, 0);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        status = session.Run({tensor_dataset_output}, &outputs);
        ...
        std::cout << "TensorDataset operation completed successfully" << std::endl;
