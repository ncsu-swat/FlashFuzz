...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor input_tensor(tensorflow::DT_INT32, tensorflow::TensorShape({3}));
        ...
        auto input_dataset = tensorflow::ops::TensorSliceDataset(
            root, 
            {tensorflow::Input(tensorflow::ops::Const(root, input_tensor))},
            {tensorflow::TensorShape({3})}
        );

        std::vector<tensorflow::Input> other_arguments;
        if (offset < size) {
            ...
            other_arguments.push_back(tensorflow::Input(tensorflow::ops::Const(root, arg_tensor)));
        }
        ...
        if (offset + sizeof(int64_t) <= size) {
            ...
            offset += sizeof(int64_t);
            buffer_output_elements_val = std::abs(buffer_output_elements_val) % 10 + 1;
        }
        ...
        if (offset + sizeof(int64_t) <= size) {
            ...
            offset += sizeof(int64_t);
            prefetch_input_elements_val = std::abs(prefetch_input_elements_val) % 10 + 1;
        }
        ...
        // Use raw_ops to create ParallelInterleaveDatasetV4
        auto parallel_interleave = tensorflow::ops::experimental::ParallelInterleaveDatasetV4(
            root,
            input_dataset,
            other_arguments,
            cycle_length,
        ...
        tensorflow::Status status = session.Run({parallel_interleave.handle}, &outputs);
        if (!status.ok()) {
            return -1;
        }
