...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::vector<int64_t> input_shape = {data[offset] % 5 + 1, data[offset+1] % 10 + 1, data[offset+2] % 10 + 1, data[offset+3] % 5 + 1};
        offset += 4;
        
        std::vector<int64_t> filter_shape = {data[offset] % 5 + 1, data[offset+1] % 5 + 1, input_shape[3]};
        offset += 2;
        
        int stride_h = data[offset] % 3 + 1;
        int stride_w = data[offset+1] % 3 + 1;
        offset += 2;
        
        int rate_h = data[offset] % 3 + 1;
        int rate_w = data[offset+1] % 3 + 1;
        offset += 2;
        ...
        if (padding == "VALID") {
            out_height = (input_shape[1] - (filter_shape[0] - 1) * rate_h - 1) / stride_h + 1;
            out_width = (input_shape[2] - (filter_shape[1] - 1) * rate_w - 1) / stride_w + 1;
        } else {
            out_height = (input_shape[1] + stride_h - 1) / stride_h;
            out_width = (input_shape[2] + stride_w - 1) / stride_w;
        }
        
        if (out_height <= 0) out_height = 1;
        if (out_width <= 0) out_width = 1;
        
        std::vector<int64_t> out_backprop_shape = {input_shape[0], out_height, out_width, input_shape[3]};
        ...
        auto dilation_backprop_filter = tensorflow::ops::Dilation2DBackpropFilter(
            root, input_op, filter_op, out_backprop_op, strides, rates, padding);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({dilation_backprop_filter}, &outputs);
