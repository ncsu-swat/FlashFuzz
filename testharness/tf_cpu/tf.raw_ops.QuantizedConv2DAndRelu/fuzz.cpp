...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto input_op = tensorflow::ops::Placeholder(root.WithOpName("input"), input_dtype, 
                                                   tensorflow::ops::Placeholder::Shape(input_tensor_shape));
        auto filter_op = tensorflow::ops::Placeholder(root.WithOpName("filter"), filter_dtype,
                                                     tensorflow::ops::Placeholder::Shape(filter_tensor_shape));
        auto min_input_op = tensorflow::ops::Placeholder(root.WithOpName("min_input"), tensorflow::DT_FLOAT);
        auto max_input_op = tensorflow::ops::Placeholder(root.WithOpName("max_input"), tensorflow::DT_FLOAT);
        auto min_filter_op = tensorflow::ops::Placeholder(root.WithOpName("min_filter"), tensorflow::DT_FLOAT);
        auto max_filter_op = tensorflow::ops::Placeholder(root.WithOpName("max_filter"), tensorflow::DT_FLOAT);

        // Use raw_ops for QuantizedConv2DAndRelu since it's not in the C++ ops API
        auto quantized_conv2d_relu = tensorflow::ops::internal::QuantizedConv2DAndRelu(
            root.WithOpName("quantized_conv2d_relu"),
            input_op,
            filter_op,
            min_input_op,
            max_input_op,
        ...
        tensorflow::Status status = session.Run({
            ...
            {filter_op, filter_tensor},
            {min_input_op, min_input_tensor},
            {max_input_op, max_input_tensor},
            {min_filter_op, min_filter_tensor},
            {max_filter_op, max_filter_tensor}
        }, {quantized_conv2d_relu.output, quantized_conv2d_relu.min_output, quantized_conv2d_relu.max_output}, &outputs);
