...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (uint8_t i = 0; i < num_other_args && offset < size; ++i) {
            ...
            auto arg_const = tensorflow::ops::Const(root, arg_tensor);
            other_arguments.push_back(arg_const);
        ...
        predicate_func.set_name("simple_predicate");
        
        auto input_dataset = tensorflow::ops::Const(root, input_dataset_tensor);
        
        // Use NodeBuilder to create ExperimentalTakeWhileDataset since it's not in the C++ API
        tensorflow::NodeBuilder node_builder = 
            tensorflow::NodeBuilder("take_while_dataset", "ExperimentalTakeWhileDataset")
                .Input(tensorflow::ops::AsNodeOut(root, input_dataset))
                .Input(tensorflow::ops::AsNodeOutList(root, other_arguments));
        ...
        if (!status.ok()) {
            tf_fuzzer_utils::logError("Failed to create ExperimentalTakeWhileDataset node: " + status.ToString(), data, size);
            return -1;
        }
        ...
        std::cout << "Output types size: " << output_types.size() << std::endl;
        std::cout << "Output shapes size: " << output_shapes.size() << std::endl;
