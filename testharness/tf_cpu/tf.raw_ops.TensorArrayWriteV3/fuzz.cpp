...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto tensor_array = tensorflow::ops::TensorArray(root, array_size, value_dtype);
        ...
        auto index_input = tensorflow::ops::Const(root, index_val);
        auto flow_in_input = tensorflow::ops::Const(root, flow_in_val);
        
        auto write_op = tensorflow::ops::TensorArrayWrite(root, tensor_array.handle, index_input, value_input, tensor_array.flow);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({write_op}, &outputs);
