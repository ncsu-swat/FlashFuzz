...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto table_handle_op = tensorflow::ops::HashTable(root, key_dtype, value_dtype);
        
        uint8_t keys_rank = parseRank(data[offset++]);
        std::vector<int64_t> keys_shape = parseShape(data, offset, size, keys_rank);
        
        tensorflow::Tensor keys_tensor(key_dtype, tensorflow::TensorShape(keys_shape));
        fillTensorWithDataByType(keys_tensor, key_dtype, data, offset, size);
        
        auto keys_op = tensorflow::ops::Const(root, keys_tensor);
        ...
        auto lookup_op = tensorflow::ops::LookupTableFind(root, table_handle_op.table_handle, keys_op, default_value_op);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({lookup_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
