...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (auto dim : weights_shape) {
            weights_tensor_shape.AddDim(dim);
        }

        tensorflow::Tensor indices_tensor(tensorflow::DT_INT32, indices_tensor_shape);
        tensorflow::Tensor values_tensor(tensorflow::DT_INT32, values_tensor_shape);
        tensorflow::Tensor weights_tensor(tensorflow::DT_FLOAT, weights_tensor_shape);
        ...
        if (offset + sizeof(int32_t) <= size) {
            std::memcpy(&sample_count, data + offset, sizeof(int32_t));
            offset += sizeof(int32_t);
            sample_count = std::abs(sample_count) % 100 + 1;
        }
        ...
        // Use raw_ops to call ConvertToCooTensor
        auto convert_op = tensorflow::ops::_Internal::ConvertToCooTensor(
            root,
            indices_input,
            values_input,
            weights_input,
            tensorflow::ops::_Internal::ConvertToCooTensor::Attrs()
        ...
        tensorflow::Status status = session.Run({convert_op.row_ids, convert_op.col_ids, convert_op.gains}, &outputs);
        if (!status.ok()) {
            return -1;
        }
