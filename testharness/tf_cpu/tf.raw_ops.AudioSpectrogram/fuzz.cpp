...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset + sizeof(int) <= size) {
            std::memcpy(&window_size, data + offset, sizeof(int));
            offset += sizeof(int);
            window_size = std::abs(window_size) % 1024 + 1;
        }
        
        int stride = 128;
        if (offset + sizeof(int) <= size) {
            std::memcpy(&stride, data + offset, sizeof(int));
            offset += sizeof(int);
            stride = std::abs(stride) % 512 + 1;
        }
        
        bool magnitude_squared = false;
        if (offset < size) {
            magnitude_squared = (data[offset++] % 2) == 1;
        }
        
        auto audio_spectrogram = tensorflow::ops::AudioSpectrogram(
            root, 
            input_placeholder, 
            window_size, 
            stride,
            tensorflow::ops::AudioSpectrogram::MagnitudeSquared(magnitude_squared)
        );
