...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::cout << "Creating AnonymousIteratorV3 with " << num_types << " output types" << std::endl;
        ...
        // Create a vector of tensors for output_shapes
        ...
        if (!output_shapes.empty()) {
            auto output_shapes_matrix = output_shapes_tensor.matrix<int64_t>();
            for (size_t i = 0; i < output_shapes.size(); ++i) {
                for (int j = 0; j < output_shapes[i].dims(); ++j) {
                    output_shapes_matrix(i, j) = output_shapes[i].dim_size(j);
                }
            }
        }
        ...
        std::cout << "AnonymousIteratorV3 executed successfully" << std::endl;
        if (!outputs.empty()) {
