...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto tag = tensorflow::ops::Placeholder(root, tensorflow::DT_STRING);
        
        // Use raw_ops namespace for LatencyStatsDataset
        auto latency_stats_dataset = tensorflow::ops::experimental::LatencyStatsDataset(
            root, input_dataset, tag, output_types, output_shapes);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run(
            {{input_dataset, variant_tensor}, {tag, tag_tensor}},
