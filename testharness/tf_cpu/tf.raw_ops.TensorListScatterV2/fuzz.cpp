...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::TensorShape tf_indices_shape;
        for (int64_t dim : indices_shape) {
            tf_indices_shape.AddDim(dim);
        }
        
        tensorflow::Tensor indices_tensor(tensorflow::DT_INT32, tf_indices_shape);
        ...
        for (int i = 0; i < indices_flat.size(); ++i) {
            indices_flat(i) = std::abs(indices_flat(i)) % (tensor_shape.empty() ? 1 : tensor_shape[0]);
        }
        ...
        tensorflow::TensorShape tf_element_shape;
        for (int64_t dim : element_shape_dims) {
            tf_element_shape.AddDim(dim);
        }
        
        tensorflow::Tensor element_shape_tensor(tensorflow::DT_INT32, tf_element_shape);
        fillTensorWithData<int32_t>(element_shape_tensor, data, offset, size);
        
        int32_t num_elements_val = 10;
        if (offset < size) {
            std::memcpy(&num_elements_val, data + offset, sizeof(int32_t));
        ...
        auto element_shape_input = tensorflow::ops::Const(root, element_shape_tensor);
        auto num_elements_input = tensorflow::ops::Const(root, num_elements_tensor);
        
        auto tensor_list_scatter = tensorflow::ops::TensorListScatterV2(
            root, tensor_input, indices_input, element_shape_input, num_elements_input);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({tensor_list_scatter}, &outputs);
        if (!status.ok()) {
            return -1;
        }
