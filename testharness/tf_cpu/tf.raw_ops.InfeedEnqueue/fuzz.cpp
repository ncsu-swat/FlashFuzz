...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            uint8_t layout_size = data[offset++] % 5;
            for (uint8_t i = 0; i < layout_size && offset < size; ++i) {
                int layout_val = static_cast<int>(data[offset++]) - 1;
                layout.push_back(layout_val);
            }
        }

        int device_ordinal = -1;
        if (offset < size) {
            device_ordinal = static_cast<int>(data[offset++]) % 10 - 1;
        }

        // Create InfeedEnqueue operation using raw_ops
        auto attrs = tensorflow::ops::InfeedEnqueueV2::Attrs();
        if (!layout.empty()) {
            attrs = attrs.Layout(layout);
        }
        if (device_ordinal >= 0) {
            attrs = attrs.DeviceOrdinal(device_ordinal);
        }
        
        auto infeed_enqueue = tensorflow::ops::InfeedEnqueueV2(
        ...
        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}}, {}, {infeed_enqueue.operation}, nullptr);
        if (!status.ok()) {
