...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        fillTensorWithDataByType(bounding_boxes_tensor, tensorflow::DT_FLOAT, data, offset, size);
        
        tensorflow::TensorShape min_object_covered_shape({});
        tensorflow::Tensor min_object_covered_tensor(tensorflow::DT_FLOAT, min_object_covered_shape);
        fillTensorWithDataByType(min_object_covered_tensor, tensorflow::DT_FLOAT, data, offset, size);
        ...
        auto min_object_covered_input = tensorflow::ops::Const(root, min_object_covered_tensor);
        ...
        if (offset < size) {
            aspect_ratio_range[0] = static_cast<float>(data[offset++]) / 255.0f;
        }
        if (offset < size) {
            aspect_ratio_range[1] = static_cast<float>(data[offset++]) / 255.0f + 1.0f;
        }
        if (offset < size) {
            area_range[0] = static_cast<float>(data[offset++]) / 255.0f;
        }
        if (offset < size) {
            area_range[1] = static_cast<float>(data[offset++]) / 255.0f;
        }
        if (offset < size) {
            max_attempts = static_cast<int>(data[offset++]) + 1;
        }
        if (offset < size) {
            use_image_if_no_bounding_boxes = (data[offset++] % 2) == 1;
        }

        auto stateless_sample_op = tensorflow::ops::StatelessSampleDistortedBoundingBox(
            root,
            image_size_input,
            bounding_boxes_input,
            min_object_covered_input,
            seed_input,
            tensorflow::ops::StatelessSampleDistortedBoundingBox::AspectRatioRange(aspect_ratio_range)
        ...
        tensorflow::Status status = session.Run({stateless_sample_op.begin, stateless_sample_op.size, stateless_sample_op.bboxes}, &outputs);
        if (!status.ok()) {
            return -1;
        }
