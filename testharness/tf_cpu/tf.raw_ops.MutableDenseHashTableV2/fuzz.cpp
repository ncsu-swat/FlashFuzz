...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            uint8_t value_shape_size = data[offset++] % 5;
            for (uint8_t i = 0; i < value_shape_size && offset < size; ++i) {
                int64_t dim = static_cast<int64_t>(data[offset++] % 10 + 1);
                value_shape_vec.push_back(dim);
            }
        }
        
        tensorflow::PartialTensorShape value_shape;
        for (int64_t dim : value_shape_vec) {
        ...
        if (offset + sizeof(int32_t) <= size) {
            int32_t bucket_val;
            std::memcpy(&bucket_val, data + offset, sizeof(int32_t));
            offset += sizeof(int32_t);
            initial_num_buckets = 1 << (std::abs(bucket_val) % 20 + 1);
        }
        
        float max_load_factor = 0.8f;
        if (offset + sizeof(float) <= size) {
            float load_val;
            std::memcpy(&load_val, data + offset, sizeof(float));
            offset += sizeof(float);
            max_load_factor = std::abs(load_val);
            if (max_load_factor > 1.0f) max_load_factor = 0.8f;
            if (max_load_factor <= 0.0f) max_load_factor = 0.1f;
        }
        ...
        auto hash_table = tensorflow::ops::MutableDenseHashTable(
            ...
            deleted_key_op,
            value_dtype,
            tensorflow::ops::MutableDenseHashTable::Container(container)
                .SharedName(shared_name)
                .UseNodeNameSharing(use_node_name_sharing)
                .ValueShape(value_shape)
                .InitialNumBuckets(initial_num_buckets)
