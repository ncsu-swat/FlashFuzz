...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        size_t serialized_options_len = (offset < size) ? data[offset++] % 20 + 1 : 5;
        std::string serialized_options;
        for (size_t i = 0; i < serialized_options_len && offset < size; ++i) {
            serialized_options += static_cast<char>(data[offset++]);
        }
        ...
        for (uint8_t i = 0; i < num_output_types; ++i) {
            uint8_t shape_rank = (offset < size) ? parseRank(data[offset++]) : 1;
            std::vector<int64_t> shape_dims = parseShape(data, offset, size, shape_rank);
            tensorflow::PartialTensorShape shape(shape_dims);
            output_shapes.push_back(shape);
        }
        
        size_t metadata_len = (offset < size) ? data[offset++] % 10 : 0;
        std::string metadata;
        for (size_t i = 0; i < metadata_len && offset < size; ++i) {
            metadata += static_cast<char>(data[offset++]);
        }
        
        auto input_dataset = tensorflow::ops::Placeholder(root, tensorflow::DT_VARIANT);
        
        // Use raw_ops approach for OptionsDataset
        auto options_dataset = tensorflow::ops::internal::OptionsDataset(
            root,
            input_dataset,
            output_types,
            output_shapes,
