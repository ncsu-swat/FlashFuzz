...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor shape_tensor(tensorflow::DT_INT32, tensorflow::TensorShape({static_cast<int64_t>(shape_dims.size())}));
        auto shape_flat = shape_tensor.flat<int32_t>();
        for (size_t i = 0; i < shape_dims.size(); ++i) {
            shape_flat(i) = static_cast<int32_t>(shape_dims[i]);
        }

        auto resource_var = tensorflow::ops::VarHandleOp(root, tensorflow::DT_INT64, tensorflow::TensorShape({2}));
        ...
        auto assign_op = tensorflow::ops::AssignVariableOp(root, resource_var, init_value);

        auto stateful_uniform = tensorflow::ops::StatefulUniformFullInt(
            root, 
            resource_var, 
            tensorflow::Input(algorithm_tensor),
            tensorflow::Input(shape_tensor),
            output_dtype
        );
        ...
        tensorflow::Status status = session.Run({assign_op}, {});
        if (!status.ok()) {
            return -1;
        }
        
        status = session.Run({stateful_uniform}, &outputs);
        if (!status.ok()) {
            return -1;
        }
