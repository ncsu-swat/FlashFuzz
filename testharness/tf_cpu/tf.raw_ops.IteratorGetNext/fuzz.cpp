...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Create a range dataset as a simple data source
        auto start = tensorflow::ops::Const(root, 0);
        auto stop = tensorflow::ops::Const(root, 10);
        auto step = tensorflow::ops::Const(root, 1);
        auto range_dataset = tensorflow::ops::RangeDataset(root, start, stop, step);

        // Create tensors for output types and shapes
        ...
        // Create iterator
        auto iterator_v2 = tensorflow::ops::Iterator(
            root.WithOpName("iterator"),
            "",  // container
            "",  // shared_name
            output_types,
            output_shapes
        ...
        auto make_iterator = tensorflow::ops::MakeIterator(
            root.WithOpName("make_iterator"),
            range_dataset,
            iterator_v2.handle
        );

        // Get next element
        auto iterator_get_next = tensorflow::ops::IteratorGetNext(
            root.WithOpName("iterator_get_next"),
            iterator_v2.handle,
            output_types,
            output_shapes
        );
        ...
        TF_CHECK_OK(session.Run({make_iterator}, nullptr));
        
        // Then try to get next element
        std::vector<tensorflow::Tensor> outputs;
        auto status = session.Run({iterator_get_next.values[0]}, &outputs);
        
        std::cout << "Created IteratorGetNext with " << output_types.size() << " outputs" << std::endl;
        for (size_t i = 0; i < output_types.size(); ++i) {
            std::cout << "Output " << i << ": type=" << output_types[i] 
                      << ", shape=" << output_shapes[i].DebugString() << std::endl;
        }

        if (status.ok() && !outputs.empty()) {
            std::cout << "Successfully retrieved element: " << outputs[0].DebugString() << std::endl;
        } else {
            std::cout << "Failed to get next element: " << status.ToString() << std::endl;
        }
