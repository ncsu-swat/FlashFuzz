...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Create a raw op for ScatterNdMin since it's not directly available in the C++ API
        tensorflow::ops::Const use_locking_attr(root, tensorflow::Tensor(use_locking));
        
        auto scatter_nd_min = tensorflow::ops::Operation(
            root.WithOpName("ScatterNdMin"),
            "ScatterNdMin",
            {ref_placeholder, indices_placeholder, updates_placeholder},
            {tensorflow::Output::Attrs().WithAttr("use_locking", use_locking)}
        );
