...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset + sizeof(int32_t) <= size) {
            ...
            index_value = std::abs(index_value) % 10;
        }
        ...
        if (offset >= size) return 0;
        uint8_t num_dtypes = (data[offset] % 3) + 1;
        offset++;
        ...
        std::cout << "Index: " << index_value << std::endl;
        std::cout << "Dtypes count: " << dtypes.size() << std::endl;
        std::cout << "Capacity: " << capacity << std::endl;
        std::cout << "Memory limit: " << memory_limit << std::endl;
        std::cout << "Container: " << container << std::endl;
        std::cout << "Shared name: " << shared_name << std::endl;

        auto stage_peek_op = tensorflow::ops::StagePeek(
            root,
            tensorflow::ops::Const(root, index_tensor),
            dtypes,
            tensorflow::ops::StagePeek::Capacity(capacity)
        ...
        tensorflow::Status status = session.Run({stage_peek_op.values}, &outputs);
        ...
        std::cout << "StagePeek executed successfully, output count: " << outputs.size() << std::endl;
