...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        switch (format_selector) {
            case 0:
                data_format = "NHWC";
                if (shape[1] % block_size != 0 || shape[2] % block_size != 0) {
                    shape[1] = (shape[1] / block_size) * block_size;
                    shape[2] = (shape[2] / block_size) * block_size;
                    if (shape[1] == 0) shape[1] = block_size;
                    if (shape[2] == 0) shape[2] = block_size;
                }
                break;
            case 1:
                data_format = "NCHW";
                if (shape[2] % block_size != 0 || shape[3] % block_size != 0) {
                    shape[2] = (shape[2] / block_size) * block_size;
                    shape[3] = (shape[3] / block_size) * block_size;
                    if (shape[2] == 0) shape[2] = block_size;
                    if (shape[3] == 0) shape[3] = block_size;
                }
                break;
            case 2:
                data_format = "NCHW_VECT_C";
                if (dtype != tensorflow::DT_QINT8) {
                    dtype = tensorflow::DT_QINT8;
                }
                if (shape[1] % 4 != 0) {
                    shape[1] = (shape[1] / 4) * 4;
        ...
        auto space_to_depth_op = tensorflow::ops::SpaceToDepth(
            root, input_placeholder, block_size,
            tensorflow::ops::SpaceToDepth::Attrs().DataFormat(data_format)
        );
