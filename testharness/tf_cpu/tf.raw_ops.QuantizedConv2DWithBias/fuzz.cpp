...
std::vector<int> parseStrides(const uint8_t* data, size_t& offset, size_t total_size) {
    ...
    for (int i = 0; i < 4; ++i) {
        if (offset < total_size) {
            int stride_val = static_cast<int>(data[offset] % 5) + 1;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::TensorShape bias_shape({filter_shape[3]});
        ...
        // Use raw_ops namespace for QuantizedConv2DWithBias
        tensorflow::OutputList outputs;
        tensorflow::Status status = tensorflow::ops::internal::QuantizedConv2DWithBias(
            root.WithOpName("QuantizedConv2DWithBias"),
            input_op,
            filter_op,
            bias_op,
            min_input_op,
