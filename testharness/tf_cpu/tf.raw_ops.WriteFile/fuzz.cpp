...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        tensorflow::DataType filename_dtype = parseDataType(data[offset++]);
        uint8_t filename_rank = parseRank(data[offset++]);
        std::vector<int64_t> filename_shape = parseShape(data, offset, size, filename_rank);
        
        tensorflow::TensorShape filename_tensor_shape;
        for (int64_t dim : filename_shape) {
            filename_tensor_shape.AddDim(dim);
        }
        
        tensorflow::Tensor filename_tensor(filename_dtype, filename_tensor_shape);
        
        if (filename_dtype == tensorflow::DT_STRING) {
            fillStringTensor(filename_tensor, data, offset, size);
        }
        
        tensorflow::DataType contents_dtype = parseDataType(data[offset % size]);
        offset++;
        uint8_t contents_rank = parseRank(data[offset % size]);
        offset++;
        ...
        if (contents_dtype == tensorflow::DT_STRING) {
        ...
        auto write_file_op = tensorflow::ops::WriteFile(root, filename_input, contents_input);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({}, {}, {write_file_op.operation}, &outputs);
        if (!status.ok()) {
            return -1;
        }
