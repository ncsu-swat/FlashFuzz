...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        int32_t concat_dim = concat_dim_byte % (rank == 0 ? 1 : rank);
        ...
        std::vector<tensorflow::Output> value_ops;
        
        for (int i = 0; i < num_tensors; ++i) {
            std::vector<int64_t> shape = base_shape;
            
            if (rank > 0 && concat_dim < shape.size()) {
                if (offset < size) {
                    uint8_t dim_modifier = data[offset++];
                    shape[concat_dim] = 1 + (dim_modifier % 5);
                }
            }
            ...
            auto tensor_op = tensorflow::ops::Const(root, tensor);
            value_ops.push_back(tensor_op);
        }
        
        auto concat_op = tensorflow::ops::Concat(root, value_ops, concat_dim_op);
