...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        bool use_fallback = (data[offset % size] % 2) == 0;

        // Create a RebatchDataset operation using raw ops
        std::vector<tensorflow::Output> inputs = {input_dataset_placeholder, num_replicas_placeholder};
        tensorflow::NodeDef node_def;
        node_def.set_name("RebatchDataset");
        node_def.set_op("RebatchDataset");
        ...
        tensorflow::AttrValue use_fallback_attr;
        use_fallback_attr.set_b(use_fallback);
        (*node_def.mutable_attr())["use_fallback"] = use_fallback_attr;
        
        // Create the operation
        tensorflow::Status status;
        auto rebatch_dataset = root.AddNode(node_def, &status);
        ...
        status = session.Run(feed_dict, {rebatch_dataset}, {}, &outputs);
