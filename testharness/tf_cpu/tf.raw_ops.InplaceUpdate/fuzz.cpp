...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t i_rank = 1;
        int64_t i_size = std::min(static_cast<int64_t>(5), x_shape[0]);
        ...
        for (int64_t idx = 0; idx < i_size; ++idx) {
            if (offset + sizeof(int32_t) <= size) {
                int32_t val;
                std::memcpy(&val, data + offset, sizeof(int32_t));
                offset += sizeof(int32_t);
                i_flat(idx) = std::abs(val) % static_cast<int32_t>(x_shape[0]);
            } else {
                i_flat(idx) = idx % static_cast<int32_t>(x_shape[0]);
            }
        }
        ...
        auto inplace_update = tensorflow::ops::InplaceUpdate(root, x_input, i_input, v_input);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({inplace_update}, &outputs);
        if (!status.ok()) {
            return -1;
        }
