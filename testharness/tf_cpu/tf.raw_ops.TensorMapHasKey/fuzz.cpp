...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        tensorflow::Tensor input_handle_tensor(tensorflow::DT_VARIANT, tensorflow::TensorShape({}));
        ...
        auto input_handle = tensorflow::ops::Placeholder(root, tensorflow::DT_VARIANT);
        auto key = tensorflow::ops::Placeholder(root, key_dtype);

        // Use raw op directly since TensorMapHasKey is not available in tensorflow::ops namespace
        auto tensor_map_has_key = tensorflow::ops::Operation(
            root.WithOpName("TensorMapHasKey"),
            "TensorMapHasKey",
            {input_handle, key},
            {{"key_dtype", key_dtype}});
        ...
        tensorflow::Status status = session.Run({{input_handle, input_handle_tensor}, {key, key_tensor}}, 
                                               {tensorflow::Output(tensor_map_has_key, 0)}, &outputs);
