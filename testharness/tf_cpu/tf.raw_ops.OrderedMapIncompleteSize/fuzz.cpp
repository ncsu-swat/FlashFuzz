...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        size_t num_dtypes = (num_dtypes_byte % 5) + 1;
        
        std::vector<tensorflow::DataType> dtypes;
        for (size_t i = 0; i < num_dtypes && offset < size; ++i) {
        ...
        std::cout << "dtypes size: " << dtypes.size() << std::endl;
        std::cout << "capacity: " << capacity << std::endl;
        std::cout << "memory_limit: " << memory_limit << std::endl;
        std::cout << "container: " << container << std::endl;
        std::cout << "shared_name: " << shared_name << std::endl;

        auto ordered_map_incomplete_size_op = tensorflow::ops::OrderedMapIncompleteSize(
            root,
            dtypes,
        ...
        std::cout << "OrderedMapIncompleteSize operation created successfully" << std::endl;

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({ordered_map_incomplete_size_op}, &outputs);
        ...
        if (!outputs.empty()) {
            ...
            if (outputs[0].dtype() == tensorflow::DT_INT32) {
                auto flat = outputs[0].flat<int32_t>();
                if (flat.size() > 0) {
                    std::cout << "Output value: " << flat(0) << std::endl;
                }
            }
        }
