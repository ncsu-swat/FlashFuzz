...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        float contrast_factor_val = 1.0f;
        if (offset + sizeof(float) <= size) {
            std::memcpy(&contrast_factor_val, data + offset, sizeof(float));
            offset += sizeof(float);
        }
        auto contrast_factor = tensorflow::ops::Const(root, contrast_factor_val);
        
        auto adjust_contrast_op = tensorflow::ops::AdjustContrast(root, images_input, contrast_factor);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({adjust_contrast_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
