...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        node_def.set_op("RetrieveTPUEmbeddingStochasticGradientDescentParameters");
        node_def.set_name(root.UniqueName("RetrieveTPUEmbeddingStochasticGradientDescentParameters"));
        
        // Add inputs
        tensorflow::NodeDefBuilder node_def_builder(node_def.name(), node_def.op());
        node_def_builder.Input(num_shards_tensor.node()->name(), 0, tensorflow::DT_INT32);
        node_def_builder.Input(shard_id_tensor.node()->name(), 0, tensorflow::DT_INT32);
        ...
        if (!status.ok()) {
            std::cout << "Error building node: " << status.ToString() << std::endl;
            return -1;
        }
        ...
        // Connect inputs
        root.graph()->AddEdge(num_shards_tensor.node(), 0, node, 0);
        root.graph()->AddEdge(shard_id_tensor.node(), 0, node, 1);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        status = session.Run({tensorflow::Output(node, 0)}, &outputs);
