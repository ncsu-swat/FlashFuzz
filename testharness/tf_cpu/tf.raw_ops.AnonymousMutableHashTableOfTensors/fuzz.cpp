...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::cout << "Value shape rank: " << static_cast<int>(value_rank) << std::endl;
        std::cout << "Value shape: [";
        for (size_t i = 0; i < value_shape.size(); ++i) {
            std::cout << value_shape[i];
            if (i < value_shape.size() - 1) std::cout << ", ";
        }
        std::cout << "]" << std::endl;
        ...
        std::cout << "Successfully created hash table resource" << std::endl;
        if (!outputs.empty()) {
