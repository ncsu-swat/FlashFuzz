...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        int32_t fft_length_value = 1;
        if (offset + sizeof(int32_t) <= size) {
            std::memcpy(&fft_length_value, data + offset, sizeof(int32_t));
            offset += sizeof(int32_t);
            fft_length_value = std::abs(fft_length_value) % 1000 + 1;
        }
        
        tensorflow::TensorShape fft_length_shape({1});
        tensorflow::Tensor fft_length_tensor(tensorflow::DT_INT32, fft_length_shape);
        fft_length_tensor.flat<int32_t>()(0) = fft_length_value;
