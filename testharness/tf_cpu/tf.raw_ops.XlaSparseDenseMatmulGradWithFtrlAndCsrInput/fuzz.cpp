...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset + sizeof(float) <= size) {
            ...
            offset += sizeof(float);
            beta = std::max(0.0f, std::min(1.0f, std::abs(beta)));
        }
        ...
        if (offset + sizeof(float) <= size) {
            ...
            offset += sizeof(float);
            learning_rate_power = std::max(-2.0f, std::min(0.0f, learning_rate_power));
        }
        ...
        if (offset + sizeof(float) <= size) {
            ...
            offset += sizeof(float);
            l1_regularization_strength = std::max(0.0f, std::abs(l1_regularization_strength));
        }
        ...
        if (offset + sizeof(float) <= size) {
            ...
            offset += sizeof(float);
            l2_regularization_strength = std::max(0.0f, std::abs(l2_regularization_strength));
        }
        ...
        tensorflow::NodeBuilder builder("XlaSparseDenseMatmulGradWithFtrlAndCsrInput", "XlaSparseDenseMatmulGradWithFtrlAndCsrInput");
        builder.Input(row_pointers.node())
               ...
               .Attr("beta", beta)
               .Attr("learning_rate_power", learning_rate_power)
               .Attr("l1_regularization_strength", l1_regularization_strength)
               .Attr("l2_regularization_strength", l2_regularization_strength)
