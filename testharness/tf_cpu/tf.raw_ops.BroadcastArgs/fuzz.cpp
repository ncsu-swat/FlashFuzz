    ...
    for (size_t i = 0; i < num_elements; ++i) {
        if (offset + element_size <= total_size) {
            ...
            if (std::is_integral<T>::value) {
                value = static_cast<T>(std::abs(static_cast<int64_t>(value)) % 10 + 1);
            }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto s0_input = tensorflow::ops::Const(root, tensor_s0);
        auto s1_input = tensorflow::ops::Const(root, tensor_s1);
        
        // Use the raw op through the scope directly
        auto broadcast_args_op = tensorflow::ops::BroadcastTo(root.WithOpName("BroadcastArgs"), 
                                                             s0_input, 
                                                             s1_input);
        ...
        tensorflow::Status status = session.Run({broadcast_args_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
