...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto tensor_const = tensorflow::ops::Const(root, 
            tensorflow::Input::Initializer(1.0f, tensorflow::TensorShape({1})));
        ...
        // Create an iterator
        auto iterator = tensorflow::ops::Iterator(
            root, 
            "iterator", 
            "", 
            output_types, 
        ...
        auto get_next_optional = tensorflow::ops::IteratorGetNextAsOptional(
            root, 
            iterator, 
            output_types, 
            output_shapes);
        ...
        tensorflow::Status status = session.Run({make_iterator}, &outputs);
        if (!status.ok()) {
            return -1;
        }

        status = session.Run({get_next_optional}, &outputs);
        if (!status.ok()) {
            return -1;
        }
