...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        bool use_locking = (offset < size) ? (data[offset++] % 2 == 0) : true;
        
        auto var_handle = tensorflow::ops::VarHandleOp(root, ref_dtype, tensorflow::TensorShape(ref_shape));
        auto assign_var = tensorflow::ops::AssignVariableOp(root, var_handle, ref_tensor);
        
        auto scatter_update = tensorflow::ops::ResourceScatterNdUpdate(
            root.WithControlDependencies({assign_var.operation}),
            var_handle,
            indices_tensor,
            updates_tensor,
            tensorflow::ops::ResourceScatterNdUpdate::UseLocking(use_locking)
        );
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Operation> ops = {scatter_update.operation};
