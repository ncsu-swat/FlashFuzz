...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::vector<tensorflow::DataType> dtypes_list;
        for (uint8_t i = 0; i < num_dtypes; ++i) {
            if (offset >= size) return 0;
            tensorflow::DataType dtype = parseDataType(data[offset++]);
            dtypes_list.push_back(dtype);
        }
        
        if (offset >= size) return 0;
        int capacity = static_cast<int>(data[offset++] % 100);
        
        if (offset >= size) return 0;
        int memory_limit = static_cast<int>(data[offset++] % 100);
        ...
        std::cout << "Number of dtypes: " << num_dtypes << std::endl;
        std::cout << "Capacity: " << capacity << std::endl;
        std::cout << "Memory limit: " << memory_limit << std::endl;
        
        auto unstage_op = tensorflow::ops::OrderedMapUnstageNoKey(
            root,
            indices_input,
            dtypes_list,
        ...
        // Add key and values outputs to fetch_outputs
        fetch_outputs.push_back(unstage_op.key);
        for (const auto& value : unstage_op.values) {
            fetch_outputs.push_back(value);
        }
