...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::vector<int> ksize = {1, 2, 2, 1};
        std::vector<int> strides = {1, 1, 1, 1};
        std::string padding = "VALID";
        
        if (offset < size) {
            uint8_t ksize_selector = data[offset++];
            ksize[1] = 1 + (ksize_selector % 3);
            ksize[2] = 1 + (ksize_selector % 3);
        }
        
        if (offset < size) {
            uint8_t stride_selector = data[offset++];
            strides[1] = 1 + (stride_selector % 2);
            strides[2] = 1 + (stride_selector % 2);
        }
        
        if (offset < size) {
            uint8_t padding_selector = data[offset++];
            padding = (padding_selector % 2 == 0) ? "VALID" : "SAME";
        }
        
        tensorflow::DataType targmax_dtype = tensorflow::DT_INT64;
        if (offset < size) {
            uint8_t targmax_selector = data[offset++];
            targmax_dtype = (targmax_selector % 2 == 0) ? tensorflow::DT_INT32 : tensorflow::DT_INT64;
        }
        
        bool include_batch_in_index = false;
        if (offset < size) {
            uint8_t batch_selector = data[offset++];
            include_batch_in_index = (batch_selector % 2 == 1);
        }
        ...
        auto max_pool_op = tensorflow::ops::MaxPoolWithArgmax(
            ...
            strides, 
            padding,
            tensorflow::ops::MaxPoolWithArgmax::Targmax(targmax_dtype).IncludeBatchInIndex(include_batch_in_index)
        );
