...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        int num_inputs = (num_inputs_raw % MAX_NUM_INPUTS) + 1;
        ...
        for (int i = 0; i < rank; ++i) {
            if (offset < size) {
                int concat_val = (data[offset++] % 4) + 1;
                num_concats.push_back(concat_val);
            } else {
        ...
        for (int i = 0; i < rank; ++i) {
            if (offset < size) {
                int padding_val = data[offset++] % 3;
                paddings.push_back(padding_val);
            } else {
