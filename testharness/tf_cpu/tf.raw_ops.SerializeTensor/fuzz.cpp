...
int fillTensorWithDataByType(tensorflow::Tensor& tensor,
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (fillTensorWithDataByType(input_tensor, dtype, data, offset, size) != 0) {
            return -1;
        }
        ...
        auto serialize_op = tensorflow::ops::SerializeTensor(root, input_placeholder);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;

        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}}, {serialize_op}, &outputs);
        if (!status.ok()) {
        ...
        if (!outputs.empty()) {
            std::cout << "Serialized tensor size: " << outputs[0].NumElements() << std::endl;
        }
