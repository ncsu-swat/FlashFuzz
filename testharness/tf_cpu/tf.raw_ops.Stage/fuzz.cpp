...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::vector<tensorflow::Output> value_outputs;
        ...
        for (uint8_t i = 0; i < num_tensors && offset < size; ++i) {
            ...
            auto input_node = tensorflow::ops::Const(root, tensor);
            value_outputs.push_back(input_node);
            dtypes.push_back(dtype);
        }

        if (value_outputs.empty()) return 0;

        int64_t capacity = 0;
        if (offset < size) {
            capacity = static_cast<int64_t>(data[offset] % 100);
            offset++;
        }

        int64_t memory_limit = 0;
        if (offset < size) {
            memory_limit = static_cast<int64_t>(data[offset] % 100) * 1024;
            offset++;
        }
        ...
        std::string shared_name = "test_stage";
        if (offset < size) {
            shared_name += std::to_string(data[offset] % 10);
            offset++;
        }

        // Convert vector<Output> to InputList
        tensorflow::InputList value_inputs;
        for (const auto& output : value_outputs) {
            value_inputs.push_back(output);
        }

        auto stage_attrs = tensorflow::ops::Stage::Attrs()
            .Capacity(capacity)
            .MemoryLimit(memory_limit)
            .Container(container)
            .SharedName(shared_name);

        auto stage_op = tensorflow::ops::Stage(root, value_inputs, stage_attrs);

        tensorflow::ClientSession session(root);
        tensorflow::Status status = session.Run({stage_op}, nullptr);
