...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto init_table_op = tensorflow::ops::Operation(
            root.WithOpName("InitializeTableFromDataset"),
            "InitializeTableFromDataset",
            {table_handle.output, dataset.output},
            {}
        );
        ...
        std::vector<std::pair<std::string, tensorflow::Tensor>> feed_dict = {
            {table_handle.node()->name(), table_handle_tensor},
            {dataset.node()->name(), dataset_tensor}
        };
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run(feed_dict, {}, {init_table_op.node()->name()}, &outputs);
