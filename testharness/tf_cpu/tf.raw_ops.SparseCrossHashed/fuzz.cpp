...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor num_buckets_tensor(tensorflow::DT_INT64, tensorflow::TensorShape({}));
        num_buckets_tensor.scalar<int64_t>()() = 100;
        auto num_buckets = tensorflow::ops::Const(root, num_buckets_tensor);

        tensorflow::Tensor strong_hash_tensor(tensorflow::DT_BOOL, tensorflow::TensorShape({}));
        strong_hash_tensor.scalar<bool>()() = false;
        auto strong_hash = tensorflow::ops::Const(root, strong_hash_tensor);

        tensorflow::Tensor salt_tensor(tensorflow::DT_INT64, tensorflow::TensorShape({}));
        salt_tensor.scalar<int64_t>()() = 12345;
        auto salt = tensorflow::ops::Const(root, salt_tensor);

        auto sparse_cross_hashed = tensorflow::ops::SparseCrossHashed(
            root,
            indices_list,
            values_list,
            shapes_list,
        ...
        tensorflow::Status status = session.Run({sparse_cross_hashed.output_indices, 
                                                sparse_cross_hashed.output_values, 
                                                sparse_cross_hashed.output_shape}, &outputs);
