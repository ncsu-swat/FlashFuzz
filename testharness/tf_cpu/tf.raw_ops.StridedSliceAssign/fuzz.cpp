...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor ref_tensor(ref_dtype, tensorflow::TensorShape(ref_shape));
        ...
        tensorflow::Tensor begin_tensor(tensorflow::DT_INT32, tensorflow::TensorShape(slice_shape));
        tensorflow::Tensor end_tensor(tensorflow::DT_INT32, tensorflow::TensorShape(slice_shape));
        tensorflow::Tensor strides_tensor(tensorflow::DT_INT32, tensorflow::TensorShape(slice_shape));
        ...
        for (int i = 0; i < slice_rank; ++i) {
            if (offset + sizeof(int32_t) <= size) {
                ...
                if (i < ref_rank) {
                    begin_val = std::abs(begin_val) % ref_shape[i];
                    end_val = std::abs(end_val) % ref_shape[i];
                    if (end_val <= begin_val) end_val = begin_val + 1;
                ...
                begin_flat(i) = 0;
                end_flat(i) = (i < ref_rank) ? ref_shape[i] : 1;
                strides_flat(i) = 1;
            }
        }
        
        std::vector<int64_t> value_shape;
        for (int i = 0; i < slice_rank && i < ref_rank; ++i) {
        ...
        tensorflow::Tensor value_tensor(ref_dtype, tensorflow::TensorShape(value_shape));
        fillTensorWithDataByType(value_tensor, ref_dtype, data, offset, size);
        ...
        auto ref_var = tensorflow::ops::Variable(root, ref_tensor.shape(), ref_dtype);
        auto assign_init = tensorflow::ops::Assign(root, ref_var, tensorflow::ops::Const(root, ref_tensor));
        
        auto strided_slice_assign = tensorflow::ops::StridedSliceAssign(
            root, ref_var,
            ...
            tensorflow::ops::StridedSliceAssign::BeginMask(begin_mask)
                .EndMask(end_mask)
        ...
        tensorflow::Status status = session.Run({assign_init}, &outputs);
        if (!status.ok()) {
            return -1;
        }
        
        status = session.Run({strided_slice_assign}, &outputs);
        if (!status.ok()) {
            return -1;
        }
