...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset >= size) return 0;
        int num_replicas = (data[offset++] % 8) + 1;
        ...
        // Create TPUReplicatedOutput op using raw_ops
        std::vector<tensorflow::Output> outputs;
        auto op_builder = tensorflow::NodeBuilder("TPUReplicatedOutput", "TPUReplicatedOutput")
            .Input(input_placeholder)
            .Attr("num_replicas", num_replicas);
        
        tensorflow::Node* node;
        tensorflow::Status status = root.graph()->AddNode(op_builder, &node);
        if (!status.ok()) {
            return -1;
        }
        ...
        status = session.Run({{input_placeholder, input_tensor}}, outputs, &output_tensors);
