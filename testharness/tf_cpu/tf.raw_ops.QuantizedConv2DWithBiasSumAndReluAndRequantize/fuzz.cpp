...
tensorflow::DataType parseOutDataType(uint8_t selector) {
    tensorflow::DataType dtype;
    switch (selector % 5) {
        case 0:
            dtype = tensorflow::DT_QINT8;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::DataType out_dtype = parseOutDataType(data[offset++]);
        ...
        auto quantized_conv2d_op = tensorflow::ops::internal::QuantizedConv2DWithBiasSumAndReluAndRequantize(
            root,
            input_op,
            filter_op,
            bias_op,
