...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            algorithm = static_cast<int>(data[offset++]);
        }
        if (offset < size) {
            cpu_budget = static_cast<int>(data[offset++]);
        }
        if (offset < size) {
            ram_budget = static_cast<int>(data[offset++]);
        }
        
        auto input_dataset = tensorflow::ops::Placeholder(root, tensorflow::DT_VARIANT);
        
        tensorflow::NodeDef node_def;
        node_def.set_name("ModelDataset");
        node_def.set_op("ModelDataset");
        
        // Add inputs to NodeDef
        tensorflow::NodeDefBuilder builder("ModelDataset", "ModelDataset");
        ...
        builder.Attr("algorithm", algorithm);
        builder.Attr("cpu_budget", cpu_budget);
        builder.Attr("ram_budget", ram_budget);
        ...
        // Create operation
        tensorflow::Operation model_dataset_op = root.graph()->CreateOperation(node_def);
        auto model_dataset = tensorflow::Output(model_dataset_op, 0);
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        status = session.Run(
            {{input_dataset, input_tensor}},
            {model_dataset},
            &outputs
        );
