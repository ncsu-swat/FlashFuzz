...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            precision = static_cast<int>(data[offset++]) % 10 - 1;
        }
        if (offset < size) {
        ...
        if (offset < size) {
            width = static_cast<int>(data[offset++]) % 10 - 1;
        }
        if (offset < size) {
        ...
        auto as_string_op = tensorflow::ops::AsString(
            root,
            input_placeholder,
            tensorflow::ops::AsString::Precision(precision)
