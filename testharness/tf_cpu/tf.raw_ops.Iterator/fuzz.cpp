...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (uint8_t i = 0; i < num_types && offset < size; ++i) {
            ...
            output_shapes.push_back(tensorflow::TensorShape(shape_dims));
        }
        ...
        auto iterator_op = tensorflow::ops::Iterator(
            root,
            output_types,
            output_shapes,
            shared_name,
        ...
        tensorflow::Status status = session.Run({iterator_op.handle}, &outputs);
        if (!status.ok()) {
