...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (int64_t dim : resource_shape) {
            resource_tensor_shape.AddDim(dim);
        }
        ...
        int num_features = 1;
        if (offset < size) {
            num_features = static_cast<int>(data[offset++] % 10) + 1;
        }
        
        // Use raw_ops API instead of the missing boosted_trees_ops.h
        std::vector<tensorflow::Output> bucket_boundaries;
        auto op = tensorflow::ops::Operation(
            root.WithOpName("BoostedTreesQuantileStreamResourceGetBucketBoundaries"),
        ...
        for (int i = 0; i < num_features; i++) {
            bucket_boundaries.push_back(tensorflow::Output(op, i));
        }
