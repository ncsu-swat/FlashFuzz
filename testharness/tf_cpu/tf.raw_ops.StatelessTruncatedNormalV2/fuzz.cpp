...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        fillTensorWithDataByType(key_tensor, tensorflow::DT_UINT64, data, offset, size);
        
        tensorflow::TensorShape counter_shape({2});
        tensorflow::Tensor counter_tensor(tensorflow::DT_UINT64, counter_shape);
        fillTensorWithDataByType(counter_tensor, tensorflow::DT_UINT64, data, offset, size);
        ...
        // Use raw_ops namespace for StatelessTruncatedNormalV2
        auto stateless_truncated_normal = tensorflow::ops::StatelessRandomOps::TruncatedNormal(
            root, shape_input, key_input, counter_input, alg_input, output_dtype);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({stateless_truncated_normal}, &outputs);
        if (!status.ok()) {
            return -1;
        }
