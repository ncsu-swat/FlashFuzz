...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto tensor_input = tensorflow::ops::Const(root, audio_tensor);
        
        float sample_rate = 44100.0f;
        if (offset + sizeof(float) <= size) {
            std::memcpy(&sample_rate, data + offset, sizeof(float));
            offset += sizeof(float);
            sample_rate = std::abs(sample_rate);
            if (sample_rate < 1.0f) sample_rate = 44100.0f;
            if (sample_rate > 192000.0f) sample_rate = 44100.0f;
        }
        
        int max_outputs = 3;
        if (offset + sizeof(int) <= size) {
            std::memcpy(&max_outputs, data + offset, sizeof(int));
            offset += sizeof(int);
            max_outputs = std::abs(max_outputs);
            if (max_outputs < 1) max_outputs = 1;
