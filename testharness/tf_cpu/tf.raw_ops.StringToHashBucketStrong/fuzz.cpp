...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset + 2 * sizeof(int) <= size) {
            int key1, key2;
            std::memcpy(&key1, data + offset, sizeof(int));
            offset += sizeof(int);
            std::memcpy(&key2, data + offset, sizeof(int));
            offset += sizeof(int);
            key[0] = key1;
            key[1] = key2;
        }
        ...
        auto result = tensorflow::ops::StringToHashBucketStrong(
            root, 
            input_placeholder, 
            num_buckets, 
            key
