...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Status status = tensorflow::ops::internal::QuantizeAndDequantize(
            root.WithOpName("QuantizeAndDequantize"),
            {input_op},
            &output_list,
            tensorflow::ops::internal::QuantizeAndDequantize::Attrs()
        ...
        auto quantize_op = output_list[0];
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        status = session.Run({quantize_op}, &outputs);
