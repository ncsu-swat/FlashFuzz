...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto stats_handle_op = tensorflow::Operation();
        tensorflow::Status status = tensorflow::NodeBuilder("stats_handle", "ExperimentalStatsAggregatorHandle")
            .Attr("container", container)
            .Attr("shared_name", shared_name)
            .Finalize(root.graph(), &stats_handle_op);
        ...
        auto stats_handle = tensorflow::Output(stats_handle_op, 0);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        status = session.Run({stats_handle}, &outputs);
        if (!status.ok()) {
