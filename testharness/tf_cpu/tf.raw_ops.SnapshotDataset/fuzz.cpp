...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::string path_str = "/tmp/snapshot_test";
        if (offset < size) {
        ...
        std::vector<tensorflow::PartialTensorShape> output_shapes = {input_tensor_shape};
        
        std::string compression = "";
        std::string reader_path_prefix = "";
        std::string writer_path_prefix = "";
        int64_t shard_size_bytes = 10737418240;
        int64_t pending_snapshot_expiry_seconds = 86400;
        int64_t num_reader_threads = 1;
        int64_t reader_buffer_size = 1;
        int64_t num_writer_threads = 1;
        int64_t writer_buffer_size = 1;
        ...
        if (offset < size) {
            compression = (data[offset++] % 2) ? "GZIP" : "";
        }
        if (offset < size) {
            num_reader_threads = (data[offset++] % 4) + 1;
        }
        if (offset < size) {
            num_writer_threads = (data[offset++] % 4) + 1;
        }
        if (offset < size) {
            shuffle_on_read = data[offset++] % 2;
        }
        ...
        auto snapshot_dataset_op = tensorflow::ops::experimental::SnapshotDataset(
            root,
            input_dataset_placeholder,
            path_placeholder,
            output_types,
            output_shapes,
            tensorflow::ops::experimental::SnapshotDataset::Compression(compression)
                .ReaderPathPrefix(reader_path_prefix)
                .WriterPathPrefix(writer_path_prefix)
                .ShardSizeBytes(shard_size_bytes)
                .PendingSnapshotExpirySeconds(pending_snapshot_expiry_seconds)
                .NumReaderThreads(num_reader_threads)
                .ReaderBufferSize(reader_buffer_size)
                .NumWriterThreads(num_writer_threads)
                .WriterBufferSize(writer_buffer_size)
                .ShuffleOnRead(shuffle_on_read)
                .Seed(seed)
                .Seed2(seed2)
                .Mode(mode)
        ...
        tensorflow::Status status = session.Run(
            {{input_dataset_placeholder, input_dataset_tensor}, {path_placeholder, path_tensor}},
            {snapshot_dataset_op},
            &outputs
        );
