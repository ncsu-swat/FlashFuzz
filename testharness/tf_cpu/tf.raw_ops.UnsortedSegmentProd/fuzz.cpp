...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t segment_ids_rank = std::min(data_rank, static_cast<uint8_t>(data_rank));
        std::vector<int64_t> segment_ids_shape;
        for (int i = 0; i < segment_ids_rank; ++i) {
            segment_ids_shape.push_back(data_shape[i]);
        }
        ...
        if (num_segments_dtype == tensorflow::DT_INT32) {
            if (offset + sizeof(int32_t) <= size) {
                ...
                std::memcpy(&val, data + offset, sizeof(int32_t));
                offset += sizeof(int32_t);
                num_segments_tensor.scalar<int32_t>()() = std::abs(val) % 20 + 1;
            } else {
                num_segments_tensor.scalar<int32_t>()() = 10;
            }
        } else {
            if (offset + sizeof(int64_t) <= size) {
                ...
                std::memcpy(&val, data + offset, sizeof(int64_t));
                offset += sizeof(int64_t);
                num_segments_tensor.scalar<int64_t>()() = std::abs(val) % 20 + 1;
            } else {
                num_segments_tensor.scalar<int64_t>()() = 10;
            }
        }
        ...
        auto unsorted_segment_prod = tensorflow::ops::UnsortedSegmentProd(
            root, data_input, segment_ids_input, num_segments_input);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({unsorted_segment_prod}, &outputs);
