...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        fillTensorWithData<float>(min_object_covered_tensor, data, offset, size);
        
        auto min_obj_scalar = min_object_covered_tensor.scalar<float>();
        float min_obj_val = min_obj_scalar();
        if (std::isnan(min_obj_val) || std::isinf(min_obj_val)) min_obj_val = 0.0f;
        if (min_obj_val < 0.0f) min_obj_val = 0.0f;
        if (min_obj_val > 1.0f) min_obj_val = 1.0f;
        min_obj_scalar() = min_obj_val;
        ...
        auto sample_op = tensorflow::ops::SampleDistortedBoundingBoxV2(
            root,
            image_size_input,
            bounding_boxes_input,
            min_object_covered_input,
            tensorflow::ops::SampleDistortedBoundingBoxV2::Seed(seed)
                .Seed2(seed2)
