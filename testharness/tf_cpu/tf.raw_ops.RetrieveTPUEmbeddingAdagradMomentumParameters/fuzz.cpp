...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            size_t name_len = std::min(static_cast<size_t>(data[offset] % 20), size - offset - 1);
            offset++;
        ...
        std::string config = "";
        if (offset < size) {
            size_t config_len = std::min(static_cast<size_t>(data[offset] % 20), size - offset - 1);
            offset++;
        ...
        // Use raw_ops instead of ops::RetrieveTPUEmbeddingAdagradMomentumParameters
        ...
        tensorflow::NodeBuilder node_builder(
            root.WithOpName("RetrieveTPUEmbeddingAdagradMomentumParameters").unique_name("RetrieveTPUEmbeddingAdagradMomentumParameters"),
            "RetrieveTPUEmbeddingAdagradMomentumParameters");
        
        node_builder.Input(num_shards_tensor.node());
        node_builder.Input(shard_id_tensor.node());
        node_builder.Attr("table_id", table_id);
        node_builder.Attr("table_name", table_name);
        node_builder.Attr("config", config);
        ...
        if (output_tensors.size() >= 3) {
            std::cout << "Parameters shape: " << output_tensors[0].shape().DebugString() << std::endl;
            std::cout << "Accumulators shape: " << output_tensors[1].shape().DebugString() << std::endl;
            std::cout << "Momenta shape: " << output_tensors[2].shape().DebugString() << std::endl;
        }
