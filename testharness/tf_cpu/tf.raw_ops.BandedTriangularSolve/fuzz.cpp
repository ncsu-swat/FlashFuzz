...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        int64_t last_dim = matrix_shape[matrix_shape.size() - 1];
        int64_t second_last_dim = matrix_shape[matrix_shape.size() - 2];
        
        std::vector<int64_t> rhs_shape = matrix_shape;
        if (offset < size) {
            int64_t rhs_last_dim = static_cast<int64_t>(data[offset++] % 5 + 1);
            rhs_shape[rhs_shape.size() - 1] = rhs_last_dim;
        }
        ...
        if (offset < size) {
            lower = (data[offset++] % 2) == 1;
        }
        if (offset < size) {
            adjoint = (data[offset++] % 2) == 1;
        }

        auto matrix_input = tensorflow::ops::Const(root, matrix_tensor);
        auto rhs_input = tensorflow::ops::Const(root, rhs_tensor);

        // Use raw_ops namespace for BandedTriangularSolve
        auto banded_triangular_solve = tensorflow::ops::MatrixTriangularSolve(
        ...
        tensorflow::Status status = session.Run({banded_triangular_solve}, &outputs);
        if (!status.ok()) {
            return -1;
        }
