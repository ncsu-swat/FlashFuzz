...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::vector<int64_t> bias_shape = {filter_shape[2]};
        std::vector<int64_t> summand_shape = {input_shape[0], 
                                             (input_shape[1] + 1) / 2, 
                                             (input_shape[2] + 1) / 2, 
                                             filter_shape[2]};
        ...
        tensorflow::TensorShape summand_tensor_shape(summand_shape);
        ...
        tensorflow::Tensor summand_tensor(tensorflow::DT_FLOAT, summand_tensor_shape);
        ...
        fillTensorWithDataByType(summand_tensor, tensorflow::DT_FLOAT, data, offset, size);
        ...
        // Use raw_ops namespace for QuantizedConv2DWithBiasSumAndRelu
        auto quantized_conv2d_op = tensorflow::ops::Raw(
            root.WithOpName("QuantizedConv2DWithBiasSumAndRelu"),
            {input_op.output, filter_op.output, bias_op.output, 
             min_input_op.output, max_input_op.output, 
             min_filter_op.output, max_filter_op.output, 
             summand_op.output},
            {tensorflow::DT_QINT8, tensorflow::DT_FLOAT, tensorflow::DT_FLOAT},
        ...
        tensorflow::Status status = session.Run({quantized_conv2d_op.output[0], 
                                                quantized_conv2d_op.output[1], 
                                                quantized_conv2d_op.output[2]}, &outputs);
