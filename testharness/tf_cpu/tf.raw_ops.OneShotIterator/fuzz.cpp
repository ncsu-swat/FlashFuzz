...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            uint8_t container_len = data[offset] % 5;
            offset++;
            for (uint8_t i = 0; i < container_len && offset < size; ++i) {
                container += static_cast<char>((data[offset] % 26) + 'a');
                offset++;
            }
        }
        
        if (offset < size) {
            uint8_t shared_name_len = data[offset] % 5;
            offset++;
            for (uint8_t i = 0; i < shared_name_len && offset < size; ++i) {
                shared_name += static_cast<char>((data[offset] % 26) + 'a');
                offset++;
            }
        }

        tensorflow::NameAttrList dataset_factory;
        dataset_factory.set_name("simple_dataset_factory");
        
        auto one_shot_iterator_op = tensorflow::ops::OneShotIterator(
            root,
            dataset_factory,
            output_types,
            output_shapes);
            
        // Apply attributes after construction
        tensorflow::ops::OneShotIterator::Attrs attrs;
        if (!container.empty()) {
            attrs = attrs.Container(container);
        }
        if (!shared_name.empty()) {
            attrs = attrs.SharedName(shared_name);
        }
        
        one_shot_iterator_op.operation.node()->AddAttr("container", container);
        one_shot_iterator_op.operation.node()->AddAttr("shared_name", shared_name);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({one_shot_iterator_op}, &outputs);
