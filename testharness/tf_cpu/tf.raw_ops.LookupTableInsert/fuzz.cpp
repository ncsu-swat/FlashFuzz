...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto table = tensorflow::ops::HashTable(root, key_dtype, value_dtype);
        
        auto table_init = tensorflow::ops::InitializeTable(root, table.table_handle, 
                                                          tensorflow::ops::Const(root, keys_tensor),
                                                          tensorflow::ops::Const(root, values_tensor));
        
        auto insert_op = tensorflow::ops::LookupTableInsert(root, table.table_handle,
                                                           tensorflow::ops::Const(root, keys_tensor),
                                                           tensorflow::ops::Const(root, values_tensor));
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Operation> ops_to_run = {table_init.operation};
        tensorflow::Status init_status = session.Run({}, {}, ops_to_run, nullptr);
        if (!init_status.ok()) {
            return -1;
        }
        
        ops_to_run = {insert_op.operation};
