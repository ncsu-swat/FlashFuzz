...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto unstage_op = tensorflow::ops::Unstage(
            root,
            dtypes,
            tensorflow::ops::Unstage::Capacity(capacity)
        ...
        tensorflow::Status status = session.Run({}, unstage_op.values, &outputs);
        
        if (status.ok()) {
            std::cout << "Unstage operation completed successfully" << std::endl;
            std::cout << "Number of output tensors: " << outputs.size() << std::endl;
            for (size_t i = 0; i < outputs.size(); ++i) {
                std::cout << "Output tensor " << i << " shape: " << outputs[i].shape().DebugString() << std::endl;
            }
        }
