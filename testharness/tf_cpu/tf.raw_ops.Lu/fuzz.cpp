...
tensorflow::DataType parseOutputIdxType(uint8_t selector) {
...
std::vector<int64_t> parseShape(const uint8_t* data, size_t& offset, size_t total_size, uint8_t rank) {
    ...
    if (rank >= 2) {
        int64_t matrix_size = shape[rank-1];
        shape[rank-2] = matrix_size;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::DataType output_idx_type = parseOutputIdxType(data[offset++]);
        ...
        std::cout << "Output idx type: " << tensorflow::DataTypeString(output_idx_type) << std::endl;

        auto input_placeholder = tensorflow::ops::Placeholder(root, input_dtype);
        
        tensorflow::ops::Lu::Attrs attrs;
        attrs = attrs.OutputIdxType(output_idx_type);
        
        auto lu_op = tensorflow::ops::Lu(root, input_placeholder, attrs);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;

        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}}, 
                                               {lu_op.lu, lu_op.p}, &outputs);
        ...
        std::cout << "LU decomposition completed successfully" << std::endl;
        std::cout << "LU tensor shape: ";
        for (int i = 0; i < outputs[0].shape().dims(); ++i) {
            std::cout << outputs[0].shape().dim_size(i) << " ";
        }
        std::cout << std::endl;
        std::cout << "P tensor shape: ";
