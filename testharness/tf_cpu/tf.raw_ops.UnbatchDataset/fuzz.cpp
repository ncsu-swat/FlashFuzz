...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto batch_dataset = tensorflow::ops::BatchDataset(
            root,
            range_dataset,
            tensorflow::ops::Const(root, 2),
            {tensorflow::DT_INT64},
            {{-1}}
        );

        // Now unbatch the dataset
        auto unbatch_dataset = tensorflow::ops::UnbatchDataset(
            root,
            batch_dataset,
            output_types,
            output_shapes
        ...
        tensorflow::Status status = session.Run({unbatch_dataset}, &outputs);
        if (!status.ok()) {
            return -1;
        }
