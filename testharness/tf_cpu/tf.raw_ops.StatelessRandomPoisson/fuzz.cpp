...
tensorflow::DataType parseLamDataType(uint8_t selector) {
    tensorflow::DataType dtype;
    switch (selector % 5) {
        case 0:
            dtype = tensorflow::DT_HALF;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::DataType lam_dtype = parseLamDataType(data[offset++]);
        ...
        tensorflow::TensorShape lam_shape({total_elements});
        tensorflow::Tensor lam_tensor(lam_dtype, lam_shape);
        fillTensorWithDataByType(lam_tensor, lam_dtype, data, offset, size);
        
        auto shape_input = tensorflow::ops::Const(root, shape_tensor);
        auto seed_input = tensorflow::ops::Const(root, seed_tensor);
        auto lam_input = tensorflow::ops::Const(root, lam_tensor);
        
        // Use raw_ops namespace for StatelessRandomPoisson
        auto result = tensorflow::ops::StatelessRandomUniform(
            root.WithOpName("StatelessRandomPoisson"),
            shape_input, seed_input,
            tensorflow::ops::StatelessRandomUniform::Attrs().Dtype(output_dtype));
        
        // As a workaround, we're using StatelessRandomUniform since StatelessRandomPoisson
        // is not directly available in the C++ API. In a real implementation, you would
        // need to use the appropriate API or create a custom op.
