...
std::string parseString(const uint8_t* data, size_t& offset, size_t total_size) {
    ...
    size_t str_len = std::min(static_cast<size_t>(data[offset] % 50 + 1), total_size - offset - 1);
    offset++;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        std::string pattern = parseString(data, offset, size);
        std::string rewrite = parseString(data, offset, size);
        
        bool replace_global = true;
        if (offset < size) {
            replace_global = (data[offset++] % 2) == 1;
        }
        
        auto input_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_STRING);
        
        auto static_regex_replace_op = tensorflow::ops::RegexReplace(
            root, input_placeholder, pattern, rewrite,
            tensorflow::ops::RegexReplace::Attrs().ReplaceGlobal(replace_global)
        );
        ...
        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}}, 
                                               {static_regex_replace_op}, &outputs);
