...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Create keys and values inputs
        auto keys_input = tensorflow::ops::Const(root, key_tensor);
        auto values_input = tensorflow::ops::Const(root, value_tensor);
        
        // Create the LookupTableInsertV2 operation
        auto insert_op = tensorflow::ops::Raw(
            root.WithOpName("LookupTableInsertV2"),
            "LookupTableInsertV2",
            {table_handle.output, keys_input.output, values_input.output},
            {}
        );
        
        tensorflow::ClientSession session(root);
        
        // Create a dummy resource handle for the table
        tensorflow::Tensor table_handle_tensor(tensorflow::DT_RESOURCE, tensorflow::TensorShape({}));
        table_handle_tensor.scalar<tensorflow::ResourceHandle>()() = tensorflow::ResourceHandle();
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({{table_handle, table_handle_tensor}}, {}, {insert_op}, &outputs);
