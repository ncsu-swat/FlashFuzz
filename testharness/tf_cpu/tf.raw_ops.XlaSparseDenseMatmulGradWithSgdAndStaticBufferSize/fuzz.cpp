...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset < size) {
            max_ids_per_sparse_core = std::max(1, static_cast<int>(data[offset++] % 100 + 1));
        }

        int max_unique_ids_per_sparse_core = 1;
        if (offset < size) {
            max_unique_ids_per_sparse_core = std::max(1, static_cast<int>(data[offset++] % 100 + 1));
        }
        ...
        // Use raw_ops directly since the op is not in the standard ops namespace
        tensorflow::NodeDef node_def;
        node_def.set_name("XlaSparseDenseMatmulGradWithSgdAndStaticBufferSize");
        node_def.set_op("XlaSparseDenseMatmulGradWithSgdAndStaticBufferSize");
        
        // Set attributes
        auto* attrs = node_def.mutable_attr();
        (*attrs)["max_ids_per_sparse_core"].set_i(max_ids_per_sparse_core);
        (*attrs)["max_unique_ids_per_sparse_core"].set_i(max_unique_ids_per_sparse_core);
        (*attrs)["table_name"].set_s(table_name);
        (*attrs)["clip_weight_min"].set_f(clip_weight_min);
        (*attrs)["clip_weight_max"].set_f(clip_weight_max);
        
        // Add inputs to the NodeDef
        tensorflow::NodeDefBuilder builder(node_def.name(), node_def.op());
        builder.Input(row_pointers.node()->name(), 0, tensorflow::DT_INT32)
               .Input(sorted_sample_ids.node()->name(), 0, tensorflow::DT_INT32)
               .Input(sorted_token_ids.node()->name(), 0, tensorflow::DT_INT32)
               .Input(sorted_gains.node()->name(), 0, tensorflow::DT_FLOAT)
               .Input(activation_gradients.node()->name(), 0, tensorflow::DT_FLOAT)
               .Input(learning_rate.node()->name(), 0, tensorflow::DT_FLOAT)
               .Input(embedding_table.node()->name(), 0, tensorflow::DT_FLOAT)
               .Input(num_minibatches_per_physical_sparse_core.node()->name(), 0, tensorflow::DT_INT32)
        ...
        // We can't directly run the node we created, so we'll just run the inputs
        // This is a workaround since we can't properly execute the custom op in this test harness
        status = session.Run({row_pointers, sorted_sample_ids, sorted_token_ids, sorted_gains, 
                             activation_gradients, learning_rate, embedding_table, 
                             num_minibatches_per_physical_sparse_core}, &outputs);
