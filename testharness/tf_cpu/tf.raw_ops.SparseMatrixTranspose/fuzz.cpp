...
tensorflow::Tensor createCSRSparseMatrix(const uint8_t* data, size_t& offset, size_t total_size, tensorflow::DataType dtype) {
    ...
    if (offset + sizeof(int64_t) * 3 <= total_size) {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        bool conjugate = (data[offset++] % 2) == 1;
        
        tensorflow::Tensor input_tensor = createCSRSparseMatrix(data, offset, size, dtype);
        
        auto input_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_VARIANT);
        
        // Use raw_ops namespace for SparseMatrixTranspose
        auto transpose_op = tensorflow::ops::Operation(
            root.WithOpName("SparseMatrixTranspose"),
        ...
        tensorflow::Status status = session.Run(
            ...
            {tensorflow::Output(transpose_op, 0)},
            &outputs
        );
