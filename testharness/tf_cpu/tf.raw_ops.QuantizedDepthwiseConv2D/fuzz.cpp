...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto attrs = tensorflow::ops::Raw::WithAttr("out_type", out_dtype);
        attrs = attrs.WithAttr("dilations", dilations);
        attrs = attrs.WithAttr("strides", strides);
        attrs = attrs.WithAttr("padding", padding);
        
        tensorflow::ops::Raw::QuantizedDepthwiseConv2D(
            root.WithOpName("QuantizedDepthwiseConv2D"),
            input_op,
            filter_op,
            min_input_op,
            max_input_op,
        ...
        outputs.assign(output_list.begin(), output_list.end());
