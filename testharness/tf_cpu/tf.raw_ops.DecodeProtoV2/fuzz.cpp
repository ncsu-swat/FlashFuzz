...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        uint8_t bytes_rank = parseRank(data[offset++]);
        std::vector<int64_t> bytes_shape = parseShape(data, offset, size, bytes_rank);
        
        tensorflow::Tensor bytes_tensor(tensorflow::DT_STRING, tensorflow::TensorShape(bytes_shape));
        fillStringTensor(bytes_tensor, data, offset, size);

        if (offset >= size) return 0;
        
        std::string message_type = "tensorflow.Example";
        
        uint8_t num_fields = (data[offset++] % 3) + 1;
        ...
        for (uint8_t i = 0; i < num_fields; ++i) {
            ...
            switch (i % 3) {
                case 0:
                    field_names.push_back("feature");
                    output_types.push_back(tensorflow::DT_STRING);
                    break;
                case 1:
                    field_names.push_back("context");
                    output_types.push_back(tensorflow::DT_STRING);
                    break;
                case 2:
                    field_names.push_back("example");
                    output_types.push_back(tensorflow::DT_STRING);
                    break;
            }
            offset++;
