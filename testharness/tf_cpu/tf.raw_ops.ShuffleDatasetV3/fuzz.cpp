...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor dummy_data(output_dtype, dataset_shape);
        ...
        auto seed_generator = tensorflow::ops::DummySeedGenerator(root);
        ...
        // Use raw op to create ShuffleDatasetV3
        auto shuffle_dataset = tensorflow::ops::ShuffleDataset(
            root,
            range_dataset,
            buffer_size,
            tensorflow::ops::ShuffleDataset::Seed(seed_val)
                .Seed2(seed2_val)
                .ReshuffleEachIteration(reshuffle_each_iteration)
        );

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({shuffle_dataset}, &outputs);
        if (!status.ok()) {
            return -1;
        }
