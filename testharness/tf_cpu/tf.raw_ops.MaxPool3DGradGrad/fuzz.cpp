...
std::vector<int> parseKsize(const uint8_t* data, size_t& offset, size_t total_size) {
    ...
    for (int i = 1; i < 4; ++i) {
        if (offset + sizeof(int) <= total_size) {
            ...
            offset += sizeof(int);
            ksize[i] = std::max(1, std::abs(val) % 5 + 1);
        } else {
            ksize[i] = 1;
        }
    }
...
std::vector<int> parseStrides(const uint8_t* data, size_t& offset, size_t total_size) {
    ...
    for (int i = 1; i < 4; ++i) {
        if (offset + sizeof(int) <= total_size) {
            ...
            std::memcpy(&val, data + offset, sizeof(int));
            offset += sizeof(int);
            strides[i] = std::max(1, std::abs(val) % 3 + 1);
        } else {
            strides[i] = 1;
        }
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor orig_input_tensor(dtype, input_tensor_shape);
        fillTensorWithDataByType(orig_input_tensor, dtype, data, offset, size);
        
        tensorflow::Tensor orig_output_tensor(dtype, input_tensor_shape);
        fillTensorWithDataByType(orig_output_tensor, dtype, data, offset, size);
        
        tensorflow::Tensor grad_tensor(dtype, input_tensor_shape);
        ...
        // Use raw_ops namespace to access MaxPool3DGradGrad
        auto max_pool_3d_grad_grad = tensorflow::ops::MaxPool3DGradGrad(
            root, orig_input, orig_output, grad,
            ksize, strides, padding, data_format);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({max_pool_3d_grad_grad}, &outputs);
