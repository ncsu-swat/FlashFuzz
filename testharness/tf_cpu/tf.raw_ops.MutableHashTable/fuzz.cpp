...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto mutable_hash_table_attrs = tensorflow::ops::MutableHashTable::Attrs()
            .Container(container)
            .SharedName(shared_name)
            .UseNodeNameSharing(use_node_name_sharing);

        auto hash_table = tensorflow::ops::MutableHashTable(
            root.WithOpName("hash_table"),
            key_dtype,
            value_dtype,
            mutable_hash_table_attrs
        );

        std::cout << "Created MutableHashTable operation" << std::endl;

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({hash_table.table_handle}, &outputs);
        if (!status.ok()) {
        ...
        if (!outputs.empty()) {
            std::cout << "Hash table created successfully, handle shape: " 
