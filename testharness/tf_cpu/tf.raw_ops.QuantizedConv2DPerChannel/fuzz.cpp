...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor min_filter_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({filter_shape[3]}));
        tensorflow::Tensor max_filter_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({filter_shape[3]}));
        ...
        auto attrs = tensorflow::ops::Raw::QuantizedConv2DPerChannel::Attrs()
            .OutType(out_dtype)
            .Dilations(dilations);
            
        auto conv_op = tensorflow::ops::Raw::QuantizedConv2DPerChannel(
            root,
            input_op,
            filter_op,
            min_input_op,
        ...
        tensorflow::Status status = session.Run({conv_op.output, conv_op.min_output, conv_op.max_output}, &outputs);
        if (!status.ok()) {
            return -1;
        }
