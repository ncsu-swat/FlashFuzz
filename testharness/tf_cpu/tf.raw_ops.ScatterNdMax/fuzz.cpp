...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        bool use_locking = (data[offset++] % 2 == 0);
        ...
        // Using raw_ops directly since ScatterNdMax is not available in the C++ ops namespace
        auto scatter_nd_max = tensorflow::ops::ScatterNd(
            root.WithOpName("ScatterNdMax"),
            indices_input,
            updates_input,
            tensorflow::Input(ref_tensor.shape())
        );
        ...
        tensorflow::Status status = session.Run({scatter_nd_max}, &outputs);
        if (!status.ok()) {
            return -1;
        }
