...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset + sizeof(float) <= size) {
            std::memcpy(&rate_val, data + offset, sizeof(float));
            offset += sizeof(float);
            rate_val = std::abs(rate_val);
            if (rate_val > 1.0f) rate_val = 1.0f;
            if (rate_val < 0.0f) rate_val = 0.0f;
        } else {
            rate_val = 0.5f;
        }
        ...
        auto rate_op = tensorflow::ops::Placeholder(root, tensorflow::DT_FLOAT);
        auto seed_op = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);
        auto seed2_op = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);

        // Use raw_ops approach instead of ops::SamplingDataset
        auto sampling_dataset = tensorflow::ops::internal::SamplingDataset(
            root, 
            input_dataset_op, 
            rate_op, 
            seed_op, 
        ...
        tensorflow::Status status = session.Run(
            {{input_dataset_op, dummy_dataset},
             {rate_op, rate_tensor},
             {seed_op, seed_tensor},
             {seed2_op, seed2_tensor}},
            {sampling_dataset}, &outputs);
