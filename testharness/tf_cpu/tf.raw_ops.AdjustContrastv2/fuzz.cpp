...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        float contrast_factor_value = 1.0f;
        if (offset + sizeof(float) <= size) {
            std::memcpy(&contrast_factor_value, data + offset, sizeof(float));
            offset += sizeof(float);
        }
        
        tensorflow::Tensor contrast_factor_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({}));
        contrast_factor_tensor.scalar<float>()() = contrast_factor_value;
        ...
        tensorflow::Status status = session.Run({{images_placeholder, images_tensor}, 
                                                 {contrast_factor_placeholder, contrast_factor_tensor}}, 
                                                {adjust_contrast_op}, &outputs);
