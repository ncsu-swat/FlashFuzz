...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        uint8_t num_tensors = (data[offset] % MAX_NUM_TENSORS) + 1;
        offset++;
        ...
        // Create OutfeedEnqueueTuple operation using raw_ops
        tensorflow::NodeDef node_def;
        node_def.set_name("OutfeedEnqueueTuple");
        node_def.set_op("OutfeedEnqueueTuple");
        
        // Add inputs to the node
        for (size_t i = 0; i < inputs.size(); ++i) {
            node_def.add_input(inputs[i].node()->name());
        }
        ...
        if (!status.ok()) {
            std::cout << "Error creating OutfeedEnqueueTuple node: " << status.ToString() << std::endl;
            return -1;
        }
        ...
        for (size_t i = 0; i < inputs.size(); ++i) {
            tensorflow::DataType dtype = parseDataType(data[1 + i * 2]);
            uint8_t rank = parseRank(data[2 + i * 2]);
            size_t temp_offset = 3 + i * 2;
            ...
            tensorflow::Tensor tensor(dtype, tensor_shape);
            fillTensorWithDataByType(tensor, dtype, data, temp_offset, size);
            
            feed_dict.push_back({inputs[i].node()->name(), tensor});
        }

        status = session.Run(feed_dict, {}, {op.node()->name()}, nullptr);
        if (!status.ok()) {
