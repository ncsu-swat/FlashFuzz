...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset + sizeof(int64_t) <= size) {
            ...
            buffer_size = std::abs(buffer_size) % 1000 + 1;
            buffer_size_tensor.scalar<int64_t>()() = buffer_size;
        } else {
            buffer_size_tensor.scalar<int64_t>()() = 100;
        }
        ...
        if (offset + sizeof(int64_t) <= size) {
            ...
            offset += sizeof(int64_t);
            seed_tensor.scalar<int64_t>()() = seed;
        } else {
            seed_tensor.scalar<int64_t>()() = 42;
        }
        ...
        if (offset + sizeof(int64_t) <= size) {
            ...
            offset += sizeof(int64_t);
            seed2_tensor.scalar<int64_t>()() = seed2;
        } else {
            seed2_tensor.scalar<int64_t>()() = 123;
        }
        
        bool reshuffle_each_iteration = true;
        if (offset < size) {
            reshuffle_each_iteration = (data[offset] % 2) == 1;
            offset++;
        }
        ...
        auto buffer_size = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);
        auto seed = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);
        auto seed2 = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);
        
        auto shuffle_dataset_op = tensorflow::ops::ShuffleDatasetV3(
            root, 
            input_dataset, 
            buffer_size, 
            seed, 
        ...
        std::vector<std::pair<std::string, tensorflow::Tensor>> feed_dict = {
            {input_dataset.node()->name(), dummy_dataset},
            {buffer_size.node()->name(), buffer_size_tensor},
            {seed.node()->name(), seed_tensor},
            {seed2.node()->name(), seed2_tensor}
        };
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run(feed_dict, {shuffle_dataset_op}, &outputs);
