    ...
    try {
        ...
        if (data_format == "NHWC") {
            if (input_shape[3] % (block_size * block_size) != 0) {
                input_shape[3] = ((input_shape[3] / (block_size * block_size)) + 1) * (block_size * block_size);
            }
        } else if (data_format == "NCHW") {
            if (input_shape[1] % (block_size * block_size) != 0) {
                input_shape[1] = ((input_shape[1] / (block_size * block_size)) + 1) * (block_size * block_size);
            }
        } else if (data_format == "NCHW_VECT_C") {
            if (input_dtype != tensorflow::DT_QINT8) {
                input_dtype = tensorflow::DT_QINT8;
            }
            if (input_shape.size() != 4) {
                input_shape = {1, 4, 1, 4};
            }
        ...
        auto depth_to_space_op = tensorflow::ops::DepthToSpace(
            root, input_placeholder, block_size,
            tensorflow::ops::DepthToSpace::DataFormat(data_format)
        );
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}}, {depth_to_space_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
