    ...
    try {
        ...
        if (offset < size) {
            ...
            for (uint8_t i = 0; i < name_len && offset < size; ++i) {
                tensor_name += static_cast<char>('a' + (data[offset++] % 26));
            }
        }
        
        std::vector<std::string> debug_ops_spec;
        if (offset < size) {
            uint8_t spec_count = data[offset++] % 3;
            for (uint8_t i = 0; i < spec_count && offset < size; ++i) {
                std::string spec = "DebugIdentity;file:///tmp/debug_";
                spec += std::to_string(i);
                spec += ";0";
                debug_ops_spec.push_back(spec);
            }
        }
        
        // Use raw_ops.Copy instead of ops::Copy
        auto copy_op = tensorflow::ops::Identity(root, input_placeholder);
        
        // Add attributes to simulate Copy operation
        if (!tensor_name.empty()) {
            copy_op.operation.node()->AddAttr("tensor_name", tensor_name);
        }
        
        if (!debug_ops_spec.empty()) {
            copy_op.operation.node()->AddAttr("debug_ops_spec", debug_ops_spec);
        }
        
        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        tensorflow::Status status = session.Run({{input_placeholder, input_tensor}}, {copy_op}, &outputs);
        if (!status.ok()) {
            return -1;
        }
