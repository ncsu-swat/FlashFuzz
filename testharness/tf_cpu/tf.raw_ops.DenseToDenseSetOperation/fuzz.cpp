...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor tensor1(dtype, tensor_shape1);
        tensorflow::Tensor tensor2(dtype, tensor_shape2);
        
        fillTensorWithDataByType(tensor1, dtype, data, offset, size);
        fillTensorWithDataByType(tensor2, dtype, data, offset, size);
        
        std::string set_operation = parseSetOperation(data[offset % size]);
        bool validate_indices = (data[(offset + 1) % size] % 2) == 0;
        
        auto set1_op = tensorflow::ops::Const(root, tensor1);
        auto set2_op = tensorflow::ops::Const(root, tensor2);
        ...
        tensorflow::NodeBuilder builder = tensorflow::NodeBuilder("DenseToDenseSetOperation", "DenseToDenseSetOperation")
            .Input(set1_op.node())
            .Input(set2_op.node())
        ...
        result_indices = tensorflow::Output(node, 0);
        result_values = tensorflow::Output(node, 1);
        result_shape = tensorflow::Output(node, 2);
        
        status = session.Run({result_indices, result_values, result_shape}, &outputs);
