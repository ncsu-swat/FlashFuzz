...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (dtype == tensorflow::DT_VARIANT) return 0;
        ...
        for (uint8_t i = 0; i < num_inputs; ++i) {
            tensorflow::ops::Placeholder input_placeholder = tensorflow::ops::Placeholder(root, dtype);
        ...
        tensorflow::Status status = session.Run(feed_dict, {add_n_op}, &outputs);
