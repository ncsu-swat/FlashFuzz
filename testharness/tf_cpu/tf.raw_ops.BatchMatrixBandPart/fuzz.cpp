...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        int64_t num_lower_val = 0;
        int64_t num_upper_val = 0;
        
        if (offset + sizeof(int64_t) <= size) {
            std::memcpy(&num_lower_val, data + offset, sizeof(int64_t));
            offset += sizeof(int64_t);
            num_lower_val = std::abs(num_lower_val) % 100;
        }
        
        if (offset + sizeof(int64_t) <= size) {
            std::memcpy(&num_upper_val, data + offset, sizeof(int64_t));
            offset += sizeof(int64_t);
            num_upper_val = std::abs(num_upper_val) % 100;
        }
        
        tensorflow::Tensor num_lower_tensor(tensorflow::DT_INT64, tensorflow::TensorShape({}));
        num_lower_tensor.scalar<int64_t>()() = num_lower_val;
        
        tensorflow::Tensor num_upper_tensor(tensorflow::DT_INT64, tensorflow::TensorShape({}));
        num_upper_tensor.scalar<int64_t>()() = num_upper_val;
        ...
        tensorflow::Status status = session.Run(
            ...
            {batch_matrix_band_part}, &outputs);
