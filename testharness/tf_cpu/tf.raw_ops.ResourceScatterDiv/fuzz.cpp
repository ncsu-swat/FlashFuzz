...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto scatter_div_op = tensorflow::ops::ResourceScatterDiv(root, var, indices_input, updates_input);
        ...
        std::vector<tensorflow::Operation> scatter_ops = {scatter_div_op};
        tensorflow::Status status = session.Run(scatter_ops, nullptr);
        if (!status.ok()) {
            return -1;
        }
