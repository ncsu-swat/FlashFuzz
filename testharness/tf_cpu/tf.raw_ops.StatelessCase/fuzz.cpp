...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::NameAttrList branch_func;
        branch_func.set_name("identity_branch");
        
        std::vector<tensorflow::NameAttrList> branches = {branch_func, branch_func, branch_func};
        
        tensorflow::Node* case_node;
        tensorflow::NodeBuilder builder("StatelessCase", "StatelessCase");
        
        // Convert input_tensors to a vector of NodeBuilder::NodeOut
        std::vector<tensorflow::NodeBuilder::NodeOut> node_outs;
        for (const auto& output : input_tensors) {
            node_outs.push_back(tensorflow::NodeBuilder::NodeOut(output.node(), output.index()));
        }
        
        builder.Input(branch_index.node())
               .Input(node_outs)
               .Attr("Tout", output_types)
               .Attr("branches", branches);
        
        tensorflow::Status build_status = builder.Finalize(root.graph(), &case_node);
        if (!build_status.ok()) {
            return 0;
        }
