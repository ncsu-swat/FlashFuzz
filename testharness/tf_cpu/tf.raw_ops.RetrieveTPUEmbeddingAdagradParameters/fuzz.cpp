...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Use raw_ops API instead of ops::RetrieveTPUEmbeddingAdagradParameters
        auto num_shards_tensor = tensorflow::ops::Const(root, num_shards);
        auto shard_id_tensor = tensorflow::ops::Const(root, shard_id);
        auto table_id_tensor = tensorflow::ops::Const(root, table_id);
        auto table_name_tensor = tensorflow::ops::Const(root, table_name);
        auto config_tensor = tensorflow::ops::Const(root, config);

        tensorflow::NodeDef node_def;
        node_def.set_name("RetrieveTPUEmbeddingAdagradParameters");
        node_def.set_op("RetrieveTPUEmbeddingAdagradParameters");
        
        // Add inputs
        node_def.add_input(num_shards_tensor.node()->name());
        node_def.add_input(shard_id_tensor.node()->name());
        
        // Add attributes
        auto* attrs = node_def.mutable_attr();
        (*attrs)["table_id"].set_i(table_id);
        (*attrs)["table_name"].set_s(table_name);
        (*attrs)["config"].set_s(config);
        ...
        // Run the session with the operation outputs
        status = session.Run({tensorflow::Output(op, 0), tensorflow::Output(op, 1)}, &outputs);
        ...
        if (outputs.size() >= 2) {
            ...
            std::cout << "Accumulators shape: " << outputs[1].shape().DebugString() << std::endl;
        }
