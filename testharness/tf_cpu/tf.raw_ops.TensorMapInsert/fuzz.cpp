...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        // Create an empty tensor map using raw ops
        auto empty_map = tensorflow::ops::EmptyTensorList(
            root, 
            tensorflow::ops::Const(root, std::vector<int64_t>{}),
        ...
        // Use raw op directly
        tensorflow::Output tensor_map_insert = tensorflow::Operation(
            root.WithOpName("TensorMapInsert"),
            "TensorMapInsert",
            {empty_map, key_input, value_input},
            {tensorflow::DT_VARIANT}
        ).output(0);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({tensor_map_insert}, &outputs);
        if (!status.ok()) {
            return -1;
        }
