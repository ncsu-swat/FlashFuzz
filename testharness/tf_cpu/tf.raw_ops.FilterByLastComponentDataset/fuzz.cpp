...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        // Create a node for FilterByLastComponentDataset using raw ops
        auto input_dataset = tensorflow::ops::Const(root, variant_tensor);
        
        tensorflow::NodeDef node_def;
        node_def.set_name("FilterByLastComponentDataset");
        node_def.set_op("FilterByLastComponentDataset");
        ...
        // Connect the input node to our new node
        root.graph()->AddEdge(input_dataset.node(), 0, node, 0);
        
        // Create an output for the node
        auto filter_dataset = tensorflow::Output(node, 0);

        tensorflow::ClientSession session(root);
        std::vector<tensorflow::Tensor> outputs;
        
        status = session.Run({filter_dataset}, &outputs);
        if (!status.ok()) {
            return -1;
        }
