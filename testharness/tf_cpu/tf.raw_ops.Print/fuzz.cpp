...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        uint8_t num_data_tensors = (data[offset++] % 3) + 1;
        ...
        for (uint8_t i = 0; i < num_data_tensors && offset < size; ++i) {
            ...
            data_outputs.push_back(data_placeholder.output);
        }
        ...
        if (offset < size) {
            first_n = static_cast<int64_t>(data[offset++]) - 128;
        }
        if (offset < size) {
            summarize = static_cast<int64_t>(data[offset++] % 10) + 1;
        }
        
        tensorflow::ops::Print::Attrs attrs;
        attrs = attrs.Message(message).FirstN(first_n).Summarize(summarize);
        
        // Convert vector of outputs to InputList
        tensorflow::InputList data_inputs(data_outputs);
        
        auto print_op = tensorflow::ops::Print(root, input_placeholder, data_inputs, attrs);
        
        tensorflow::ClientSession session(root);
        
        std::vector<std::pair<std::string, tensorflow::Tensor>> feed_dict;
        feed_dict.push_back({input_placeholder.node()->name(), input_tensor});
        
        for (size_t i = 0; i < data_placeholders.size(); ++i) {
            feed_dict.push_back({data_placeholders[i].node()->name(), data_tensors[i]});
        }
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run(feed_dict, {print_op}, &outputs);
