...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        for (uint8_t i = 0; i < num_output_types; ++i) {
            ...
            output_types.push_back(output_dtype);
            
            uint8_t output_rank = parseRank(data[offset++]);
            std::vector<int64_t> output_shape = parseShape(data, offset, size, output_rank);
            
            tensorflow::PartialTensorShape output_tensor_shape;
            if (!output_shape.empty()) {
                output_tensor_shape = tensorflow::PartialTensorShape(output_shape);
            }
            output_shapes.push_back(output_tensor_shape);
        }
        
        auto input_dataset = tensorflow::ops::Placeholder(root, tensorflow::DT_VARIANT);
        auto other_arguments_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_FLOAT, 
                                                                       tensorflow::ops::Placeholder::Shape({}));
        auto num_parallel_calls_placeholder = tensorflow::ops::Placeholder(root, tensorflow::DT_INT64);
        
        tensorflow::NameAttrList func;
        func.set_name("identity_func");
        
        std::vector<tensorflow::Output> other_arguments = {other_arguments_placeholder};
        
        // Use raw_ops API to create ParallelMapDatasetV2
        auto parallel_map_dataset = tensorflow::ops::experimental::ParallelMapDataset(
            root,
            input_dataset,
            other_arguments,
        ...
        tensorflow::Tensor other_args_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({}));
        other_args_tensor.scalar<float>()() = 1.0f;
        
        std::vector<std::pair<std::string, tensorflow::Tensor>> feed_dict = {
            {input_dataset.node()->name(), input_dataset_tensor},
            {other_arguments_placeholder.node()->name(), other_args_tensor},
            {num_parallel_calls_placeholder.node()->name(), num_parallel_calls_tensor}
        };
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run(feed_dict, {parallel_map_dataset}, {}, &outputs);
