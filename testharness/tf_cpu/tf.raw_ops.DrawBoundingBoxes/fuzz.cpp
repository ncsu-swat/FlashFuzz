...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        if (offset + 4 * sizeof(int64_t) <= size) {
            images_shape = parseShape(data, offset, size, images_rank);
        ...
        int64_t batch_size = images_shape[0];
        int64_t num_boxes = 1;
        if (offset < size) {
            num_boxes = (data[offset++] % 3) + 1;
        }
        
        std::vector<int64_t> boxes_shape = {batch_size, num_boxes, 4};
        ...
        for (int64_t i = 0; i < boxes_flat.size(); ++i) {
            if (offset + sizeof(float) <= size) {
                ...
                value = std::max(0.0f, std::min(1.0f, std::abs(value)));
                boxes_flat(i) = value;
            } else {
                boxes_flat(i) = 0.0f;
            }
