...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        axis_val = axis_val % static_cast<int32_t>(value_rank);
        if (axis_val < 0) {
            axis_val += static_cast<int32_t>(value_rank);
        }
        ...
        int32_t num_split;
        if (offset + sizeof(int32_t) <= size) {
            std::memcpy(&num_split, data + offset, sizeof(int32_t));
            offset += sizeof(int32_t);
        } else {
            num_split = 2;
        }
        
        num_split = std::abs(num_split) % 8 + 1;
        
        if (axis_val >= 0 && axis_val < static_cast<int32_t>(value_shape.size())) {
            int64_t dim_size = value_shape[axis_val];
            if (dim_size % num_split != 0) {
                num_split = 1;
                for (int i = 2; i <= dim_size && i <= 8; ++i) {
                    if (dim_size % i == 0) {
                        num_split = i;
                        break;
                    }
        ...
        auto split_op = tensorflow::ops::Split(root, axis_input, value_input, num_split);
        
        tensorflow::ClientSession session(root);
        
        std::vector<tensorflow::Tensor> outputs;
        tensorflow::Status status = session.Run({split_op.output}, &outputs);
        if (!status.ok()) {
            return -1;
        }
