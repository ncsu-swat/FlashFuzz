...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        auto data_input = tensorflow::ops::Const(root, input_tensor);
        
        if (offset >= size) return 0;
        uint8_t frame_name_len = data[offset++] % 20 + 1;
        std::string frame_name = "frame_";
        for (uint8_t i = 0; i < frame_name_len && offset < size; ++i) {
            frame_name += static_cast<char>('a' + (data[offset++] % 26));
        }
        
        if (offset >= size) return 0;
        bool is_constant = (data[offset++] % 2) == 1;
        ...
        std::cout << "Parallel iterations: " << parallel_iterations << std::endl;
