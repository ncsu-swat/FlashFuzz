...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    try {
        ...
        tensorflow::Tensor velocities_tensor(tensorflow::DT_FLOAT, tensor_shape3);
        fillTensorWithDataByType(velocities_tensor, tensorflow::DT_FLOAT, data, offset, size);
        ...
        auto velocities_input = tensorflow::ops::Const(root, velocities_tensor);

        // Use raw_ops directly instead of the missing tpu_ops.h
        auto load_op = tensorflow::ops::Operation(
            root.WithOpName("LoadTPUEmbeddingADAMParameters"),
            "LoadTPUEmbeddingADAMParameters",
            {parameters_input.node(), momenta_input.node(), velocities_input.node()},
            {
