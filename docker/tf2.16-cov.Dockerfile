...
RUN bazel build --copt=-g --copt=-O0 --copt="-Wno-error=c23-extensions" --copt=-fsanitize=fuzzer-no-link --copt=-fprofile-instr-generate --copt=-fcoverage-mapping --linkopt=-fsanitize=fuzzer-no-link --linkopt=-fprofile-instr-generate --linkopt=-fcoverage-mapping --linkopt=-L/usr/lib/clang/19/lib/linux --linkopt=-lclang_rt.fuzzer-x86_64  //tensorflow:tensorflow_cc
